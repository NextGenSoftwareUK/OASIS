...
export async function getCreditBalance(walletAddress: string): Promise<number> {
  try {
    ...
    if (!response.ok) {
      // Silently return 0 if API is not available or returns error
      return 0;
    }
    ...
    // Silently fail - API might not be running or network error
    // Return 0 instead of throwing to prevent UI errors
    console.debug('Credit balance API not available, returning 0:', err);
    return 0;
  }
}
