...
| **[NextGenSoftwareUK/OASIS-Omniverse-Hub](https://github.com/NextGenSoftwareUK/OASIS-Omniverse-Hub)** | OASIS Omniverse HUB — the Unity-based Omniverse OS and Kernel. The persistent cross-game space players teleport through between all connected OGames. Contains `OmniverseKernel.cs`, the portal rendering system, the shared cross-game HUD, login screen, and the Web4/Web5 gateway client. Portal triggers carry `TargetMap` / spawn coordinates; `EnterPortalAsync` writes the arrive IPC file on walk-in. `TickAvatarStateIpc` polls the state file written by `OGEngineClient` every second; if the file is absent or its `updatedUtc` timestamp is more than 5 seconds old (game crashed or exited cleanly) the stale file is deleted and live XP/karma/activeGame are cleared immediately so the HUD never shows dead-game values. Live state feeds into `GetRuntimeHealthSnapshot` and the status strip. | Core of the OASIS Omniverse itself — the most strategically sensitive IP in the platform |
