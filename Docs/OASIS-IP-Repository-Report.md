...
|---|---|---|
| **[NextGenSoftwareUK/OASIS-API-Core](https://github.com/NextGenSoftwareUK/OASIS-API-Core)** | Core OASIS API interfaces, base classes and provider contracts. Contains the **COSMIC ORM** — a universal data abstraction layer across 206 Web2 and Web3 providers that lets developers migrate from traditional databases to decentralised storage with minimal code changes. Existing models can be reused as-is; swapping from Entity Framework or NHibernate is a matter of a few lines. The same codebase then spans SQL, NoSQL, IPFS, Holochain, Solana, Ethereum and 122+ more — without rewriting business logic. Also contains the full HyperDrive backend (auto-failover, auto-replication, auto-load-balancing across every provider) and the STARNET holon graph — all deeply integrated as the unified WEB4 engine | The single most critical piece of IP; the entire provider ecosystem, zero-downtime system, and the most frictionless Web2→Web3 migration path in existence |
...
| **[NextGenSoftwareUK/OASIS-WEB6](https://github.com/NextGenSoftwareUK/OASIS-WEB6)** | WEB6 AI layer — FAHRN multi-agent orchestration (5 dispatch modes, ML.NET classifier), Holonic BRAID shared reasoning graph, SkillOpt self-evolving skills, 259 MCP tools, 56 REST endpoints, DID/VC identity, 100 AI model providers (OpenAI, Anthropic, Gemini, Groq, Mistral, DeepSeek, xAI, Cohere, Ollama, Azure OpenAI, AWS Bedrock, ElevenLabs, Runway ML, Black Forest Labs, Alibaba Qwen, Doubao, MiniMax, Zhipu AI, Baidu ERNIE, Naver HyperCLOVA X, IBM WatsonX, Snowflake Cortex, Bittensor, GaiaNet, Venice AI + 75 more), 22 orchestrator protocols (MCP, A2A, ACP, ANP, LangGraph, OpenAI Agents SDK, Nostr NIP-90, LangChain, AutoGen, CrewAI, SemanticKernel, BeeAgent, Temporal, Dapr, NATSJetStream, gRPC, GraphQL, Kafka, AMQP, MQTT, Webhook + 1 more) | Next-generation AI platform IP; primary revenue growth engine |
...
| **[NextGenSoftwareUK/OGEngineClient](https://github.com/NextGenSoftwareUK/OGEngineClient)** | Native AOT .NET HTTP client for both the WEB4 OASIS API and WEB5 STAR API, plus P/Invoke interop bridge that exposes OASIS/STAR functionality directly into OGEngine native games (OQuake, ODOOM and others). Includes `OmniverseHubBridge` — real-time IPC layer that pushes live avatar XP/karma to the Hub status strip, lets hosted games request cross-game portal transitions, and provides `ConsumeArriveFile` / `ArrivePayload` so games can read the spawn map and coordinates the Hub wrote on portal activation. Native exports `ogengine_hub_notify_avatar_state`, `ogengine_hub_request_teleport`, and `ogengine_hub_consume_arrive_file` give C/C++ game binaries full access to all three IPC channels without managed interop overhead. Covered by `OGEngine.Client.Tests` (17 xunit tests). | Game engine integration layer; bridges the entire OASIS platform into proprietary native game runtimes |
...
| **[NextGenSoftwareUK/OASIS-Omniverse-Hub](https://github.com/NextGenSoftwareUK/OASIS-Omniverse-Hub)** | OASIS Omniverse HUB — the Unity-based Omniverse OS and Kernel. The persistent cross-game space players teleport through between all connected OGames. Contains `OmniverseKernel.cs`, the portal rendering system, the shared cross-game HUD, login screen, and the Web4/Web5 gateway client. Portal triggers carry `TargetMap` / spawn coordinates; both `EnterPortalAsync` and `TickTeleportIpc` write arrive JSON using `InvariantCulture` floats to prevent locale-sensitive corruption on non-English OS targets. `TickAvatarStateIpc` polls the state file written by `OGEngineClient` every second; if the file is absent or its `updatedUtc` timestamp is more than 5 seconds old the stale file is deleted and live XP/karma/activeGame are cleared immediately. The status strip live-state row triggers on `liveActiveGame` being non-empty (not XP/karma > 0), so a brand-new avatar with 0 XP still gets the live display. | Core of the OASIS Omniverse itself — the most strategically sensitive IP in the platform |
