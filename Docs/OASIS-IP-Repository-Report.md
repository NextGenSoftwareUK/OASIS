...
| **[NextGenSoftwareUK/OGEngineClient](https://github.com/NextGenSoftwareUK/OGEngineClient)** | Native AOT .NET HTTP client for both the WEB4 OASIS API and WEB5 STAR API, plus P/Invoke interop bridge that exposes OASIS/STAR functionality directly into OGEngine native games (OQuake, ODOOM and others). Includes `OmniverseHubBridge` — real-time IPC layer that pushes live avatar XP/karma to the Hub status strip and lets hosted games request cross-game portal transitions via the same file-based IPC the OmniverseKernel already polls. Exports `ogengine_hub_notify_avatar_state` and `ogengine_hub_request_teleport` so C/C++ game binaries can signal the Hub without any managed interop overhead. | Game engine integration layer; bridges the entire OASIS platform into proprietary native game runtimes |
...
| **[NextGenSoftwareUK/OASIS-Omniverse-Hub](https://github.com/NextGenSoftwareUK/OASIS-Omniverse-Hub)** | OASIS Omniverse HUB — the Unity-based Omniverse OS and Kernel. The persistent cross-game space players teleport through between all connected OGames. Contains `OmniverseKernel.cs`, the portal rendering system, the shared cross-game HUD, login screen, and the Web4/Web5 gateway client. Portal triggers now carry `TargetMap` / spawn coordinates so each portal can drop players into a specific level; `EnterPortalAsync` writes the arrive IPC file immediately on walk-in (not just on game-initiated teleports). `OmniverseKernel.TickAvatarStateIpc` polls the state file written by `OGEngineClient.OmniverseHubBridge` every second and feeds live XP, karma and active-game identity into `GetRuntimeHealthSnapshot`; the status strip renders them in real time alongside API circuit and host diagnostics. The beating heart of the entire OASIS Omniverse. | Core of the OASIS Omniverse itself — the most strategically sensitive IP in the platform |
