...
| API Core | 51842d3f35e16d251aee688dadeb100b110dc2c5 |
...
Core Development subsequently introduced ProviderCategories → ProviderCapabilities changes. The tested parent graph still contains 78 references to the earlier contract. Those remote commits have not been overwritten or reverted. Component PRs and the parent must be integrated with that separate provider migration and retested together before changing a deployment branch. No compatibility shim was added.
...
| Full ONODE WebAPI.UnitTests project | 87 passed; no skips |
...
- Historical Holon/Mongo/provider/Stripe exports and an approved signed migration manifest have not been supplied or imported. Empty provider receipt arrays in historical records remain explicitly unverified; an opening balance does not invent provider evidence.
