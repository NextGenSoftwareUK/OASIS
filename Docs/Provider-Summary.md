# OASIS Provider Summary

_184 fully implemented providers across 7 categories — last updated: 2026-09-19_

---

## Category Breakdown

| Category | Count | Examples |
|---|---|---|
| Blockchain / L1 / L2 | 51 | Ethereum, Solana, Bitcoin, Polygon, Arbitrum, Base, BNB Chain, Cardano, Polkadot, NEAR, Cosmos, Sui, Aptos, TON, Stellar, Monad, Abstract, Berachain, Sei, Axelar, Wormhole, Fhenix, Web3Core, Chainflip, deBridge, Stargate, Synapse… |
| Decentralised Social / Network | 30 | Farcaster, Nostr, Lens, Lens v2, BlueSky, Matrix, Discord, ActivityPub, Scuttlebutt, Urbit, SEEDS, Telegram, Waku, Livepeer, Akash, Tor/Onion, Push Protocol, Celestia, Ceramic… |
| Storage / Database / Cloud | 81 | MongoDB, IPFS, Arweave, Neo4j, SQL Server, Oracle DB, SQLite, Azure Blob, Azure Cosmos DB, AWS, Google Cloud, ThreeFold, SOLID, Moralis, PostgreSQL, Firebase, Supabase, Fastly, Deno Deploy, Vercel KV, Netlify Blobs, Fly.io… |
| Spatial / Gaming / AR | 8 | GO Map, Mapbox, WRLD 3D, Cargo, Google Maps, HERE Maps, MapLibre, Niantic Lightship |
| Web3 API / Indexing | 4 | The Graph, ENS, Alchemy, Infura |
| Identity / Encryption / IP | 8 | World ID, Lit Protocol, Story Protocol, Civic, Reclaim Protocol, Polygon ID, zkPass, Holonym |
| Infrastructure / On-Chain | 2 | Safe (Gnosis), Sei Network |

**Total: 184 providers** (51 + 30 + 81 + 8 + 4 + 8 + 2 = 184)

> **Added 2026-09-19 (+10):** Blockchain ×4 (Chainflip, deBridge, Stargate, Synapse), Storage ×3 (Vercel KV, Netlify Blobs, Fly.io), Identity ×3 (Polygon ID, zkPass, Holonym). Previous total: 174.
> **Added 2026-09-15 (+1):** Web3Core (EVM-compatible blockchain universal adapter). Previous total: 173.
> **Added 2026-09-13 (+12):** Maps ×4 (Google Maps, HERE Maps, MapLibre, Niantic Lightship), Blockchain ×3 (Axelar, Wormhole, Fhenix FHE L2), Network ×1 (Ceramic), Identity ×2 (Civic, Reclaim Protocol), Storage ×2 (Fastly, Deno Deploy). Previous total: 161.

---

## Full Provider List

### Blockchain / L1 / L2 (51)

| Provider | ProviderType |
|---|---|
| Ethereum | EthereumOASIS |
| Bitcoin | BitcoinOASIS |
| Solana | SOLANAOASIS |
| BNB Chain | BNBChainOASIS |
| Polygon | PolygonOASIS |
| Arbitrum | ArbitrumOASIS |
| Optimism | OptimismOASIS |
| Avalanche | AvalancheOASIS |
| Base | BaseOASIS |
| Cardano | CardanoOASIS |
| Polkadot | PolkadotOASIS |
| NEAR | NEAROASIS |
| Cosmos | CosmosBlockChainOASIS |
| TRON | TRONOASIS |
| XRP Ledger | XRPLOASIS |
| EOS | EOSIOOASIS |
| Sui | SuiOASIS |
| Aptos | AptosOASIS |
| Hedera Hashgraph | HashgraphOASIS |
| MultiversX (Elrond) | ElrondOASIS |
| Fantom | FantomOASIS |
| zkSync | ZkSyncOASIS |
| Scroll | ScrollOASIS |
| Linea | LineaOASIS |
| Rootstock (RSK) | RootstockOASIS |
| Telos | TelosOASIS |
| Stacks (BlockStack) | BlockStackOASIS |
| Zcash | ZcashOASIS |
| Miden | MidenOASIS |
| Aztec | AztecOASIS |
| Starknet | StarknetOASIS |
| Radix | RadixOASIS |
| TON | TONOASIS |
| Stellar | StellarOASIS |
| Monad | MonadOASIS |
| ChainLink | ChainLinkOASIS |
| Loom (video messaging) | LoomOASIS |
| Algorand | AlgorandOASIS |
| Filecoin | FilecoinOASIS |
| Ceramic / ComposeDB | CeramicOASIS |
| Basechain (Loom Network EVM) | BasechainOASIS |
| Abstract | AbstractOASIS |
| Berachain | BerachainOASIS |
| Axelar (cross-chain interoperability) | AxelarOASIS |
| Wormhole (cross-chain bridge) | WormholeOASIS |
| Fhenix (FHE L2) | FhenixOASIS |
| Web3Core (EVM universal adapter) | Web3CoreOASIS |
| Chainflip (native cross-chain swap) | ChainflipOASIS |
| deBridge (cross-chain liquidity) | deBridgeOASIS |
| Stargate (LayerZero liquidity bridge) | StargateOASIS |
| Synapse (cross-chain bridge) | SynapseOASIS |

### Storage / Database / Cloud (81)

| Provider | ProviderType |
|---|---|
| MongoDB | MongoOASIS |
| Neo4j | Neo4jOASIS |
| SQL Server | SQLServerDBOASIS |
| Oracle DB | OracleDBOASIS |
| SQLite | SQLLiteDBOASIS |
| Local File | LocalFileOASIS |
| IPFS | IPFSOASIS |
| Pinata (IPFS pinning) | PinataOASIS |
| Arweave (permanent) | ArweaveOASIS |
| SOLID (Tim Berners-Lee) | SOLIDOASIS |
| ThreeFold | ThreeFoldOASIS |
| Azure Blob Storage | AzureStorageOASIS |
| Azure Cosmos DB | AzureCosmosDBOASIS |
| AWS | AWSOASIS |
| Google Cloud | GoogleCloudOASIS |
| Moralis (Web3 data API) | MoralisOASIS |
| Tableland (on-chain SQL) | TablelandOASIS |
| PostgreSQL | PostgreSQLOASIS |
| Firebase | FirebaseOASIS |
| Supabase | SupabaseOASIS |
| Cloudflare Workers / KV | CloudflareOASIS |
| PocketBase | PocketBaseOASIS |
| Turso | TursoOASIS |
| Appwrite | AppwriteOASIS |
| PlanetScale | PlanetScaleOASIS |
| OrbitDB | OrbitDBOASIS |
| GUN | GUNOASIS |
| CockroachDB | CockroachDBOASIS |
| Neon | NeonOASIS |
| SurrealDB | SurrealDBOASIS |
| RavenDB | RavenDBOASIS |
| Cassandra | CassandraOASIS |
| Cloudflare D1 | CloudflareD1OASIS |
| Convex | ConvexOASIS |
| Fauna | FaunaOASIS |
| Qdrant | QdrantOASIS |
| Weaviate | WeaviateOASIS |
| InfluxDB | InfluxDBOASIS |
| TimescaleDB | TimescaleDBOASIS |
| Elasticsearch | ElasticsearchOASIS |
| DynamoDB | DynamoDBOASIS |
| Couchbase | CouchbaseOASIS |
| Upstash | UpstashOASIS |
| ScyllaDB | ScyllaDBOASIS |
| Litestream | LitestreamOASIS |
| MinIO | MinIOOASIS |
| Cloudinary | CloudinaryOASIS |
| PouchDB | PouchDBOASIS |
| Meilisearch | MeilisearchOASIS |
| Typesense | TypesenseOASIS |
| ClickHouse | ClickHouseOASIS |
| Redis | RedisOASIS |
| Memcached | MemcachedOASIS |
| KeyDB | KeyDBOASIS |
| OpenSearch | OpenSearchOASIS |
| Algolia | AlgoliaOASIS |
| Solr | SolrOASIS |
| ArangoDB | ArangoDBOASIS |
| QuestDB | QuestDBOASIS |
| CouchDB | CouchDBOASIS |
| Xata | XataOASIS |
| Pinecone | PineconeOASIS |
| Milvus | MilvusOASIS |
| Chroma | ChromaOASIS |
| LanceDB | LanceDBOASIS |
| pgvector | PgVectorOASIS |
| Marqo | MarqoOASIS |
| Zilliz | ZillizOASIS |
| ArcadeDB | ArcadeDBOASIS |
| DuckDB | DuckDBOASIS |
| MotherDuck | MotherDuckOASIS |
| Snowflake | SnowflakeOASIS |
| BigQuery | BigQueryOASIS |
| Redshift | RedshiftOASIS |
| Databricks | DatabricksOASIS |
| Apache Druid | DruidOASIS |
| Apache Pinot | PinotOASIS |
| Dragonfly | DragonflyOASIS |
| ValKey | ValKeyOASIS |
| Garnet | GarnetOASIS |
| Fastly (edge CDN / KV) | FastlyOASIS |
| Deno Deploy (edge functions) | DenoDeployOASIS |
| Vercel KV (edge key-value store) | VercelKVOASIS |
| Netlify Blobs (edge object store) | NetlifyBlobsOASIS |
| Fly.io (distributed app platform) | FlyIOOASIS |

### Decentralised Social / Network (30)

| Provider | ProviderType |
|---|---|
| ActivityPub | ActivityPubOASIS |
| Holochain | HoloOASIS |
| HoloWeb | HoloWebOASIS |
| Scuttlebutt | ScuttlebuttOASIS |
| Urbit | UrbitOASIS |
| SEEDS | SEEDSOASIS |
| Telegram | TelegramOASIS |
| Farcaster | FarcasterOASIS |
| Nostr | NostrOASIS |
| Lens Protocol | LensOASIS |
| BlueSky (AT Protocol) | BlueSkyOASIS |
| Matrix | MatrixOASIS |
| Discord | DiscordOASIS |
| Waku (P2P messaging) | WakuOASIS |
| Livepeer (video) | LivepeerOASIS |
| Akash (decentralised compute) | AkashOASIS |
| Tor / Onion | OnionOASIS |
| Orion Protocol (DEX aggregator) | OrionProtocolOASIS |
| PLAN | PLANOASIS |
| Push Protocol (Web3 notifications) | PushProtocolOASIS |
| Celestia (DA layer) | CelestiaOASIS |
| Eclipse (SVM L2) | EclipseOASIS |
| Lens v2 | LensV2OASIS |
| Privy (embedded wallets) | PrivyOASIS |
| LayerZero (cross-chain messaging) | LayerZeroOASIS |
| Gitcoin Passport (identity scoring) | GitcoinPassportOASIS |
| Polybase (decentralised DB) | PolybaseOASIS |
| Sui zkLogin (social login) | SuiZkLoginOASIS |
| ZKsync SSO (smart account SSO) | ZkSyncSSOOASIS |
| Ceramic / ComposeDB (decentralised data streams) | CeramicOASIS |

### Web3 API / Indexing / RPC (4)

| Provider | ProviderType |
|---|---|
| The Graph (GraphQL subgraph indexing) | TheGraphOASIS |
| ENS (Ethereum Name Service) | ENSOASIS |
| Alchemy (RPC gateway) | AlchemyOASIS |
| Infura (RPC gateway) | InfuraOASIS |

### Spatial / Gaming / AR (8)

| Provider | ProviderType |
|---|---|
| GO Map (Unity AR) | GOMapOASIS |
| Mapbox (geospatial) | MapboxOASIS |
| WRLD 3D (metaverse) | WRLD3DOASIS |
| Cargo (NFT marketplace) | CargoOASIS |
| Google Maps | GoogleMapsOASIS |
| HERE Maps | HEREMapsOASIS |
| MapLibre (open-source maps) | MapLibreOASIS |
| Niantic Lightship (AR platform) | NianticLightshipOASIS |

### Identity / Security / IP (8)

| Provider | ProviderType |
|---|---|
| World ID (Worldcoin ZK proof-of-humanity) | WorldIDOASIS |
| Lit Protocol (threshold access control) | LitProtocolOASIS |
| Story Protocol (programmable IP) | StoryProtocolOASIS |
| Civic (decentralised identity) | CivicOASIS |
| Reclaim Protocol (zk identity proofs) | ReclaimProtocolOASIS |
| Polygon ID (ZK decentralised identity) | PolygonIDOASIS |
| zkPass (ZK data verification) | zkPassOASIS |
| Holonym (ZK proof of unique humanity) | HolonymOASIS |

### Infrastructure / Multisig (2)

| Provider | ProviderType |
|---|---|
| Safe (Gnosis multisig) | SafeOASIS |
| Sei Network | SeiOASIS |

---

## Competitive Moat

No competitor — Web3 or traditional — comes close to 184 implemented providers under a single unified interface:

| Platform | Providers |
|---|---|
| **OASIS** | **184** |
| Moralis | ~15 (read-only Web3 data) |
| Alchemy | ~10 (EVM chains only) |
| Infura | ~8 (EVM chains only) |
| The Graph | ~50 (indexing only, no write) |
| Chainlink | ~20 (oracle data only) |

OASIS is the only platform that provides **read + write + identity + social + spatial** across all categories through a single hot-swappable provider interface (OASIS HyperDrive).
