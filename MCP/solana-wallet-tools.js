...
export class SolanaWalletMCPTools {
    ...
    async generateKeypair() {
        try {
            const response = await this.axiosInstance.post('/api/keys/generate_keypair_with_wallet_address_for_provider/SolanaOASIS');
    ...
    async setDefaultWallet(avatarId, walletId) {
        try {
            await this.axiosInstance.post(`/api/wallet/avatar/${avatarId}/default-wallet/${walletId}?providerType=SolanaOASIS`);
        }
        catch (error) {
     ...
     */
    async createSolanaWallet(options) {
 ...
 * ```
 */
//# sourceMappingURL=solana-wallet-tools.js.map