...
export class EthereumWalletMCPTools {
  ...
  private async generateKeypair(): Promise<{
    ...
    try {
      const response = await this.axiosInstance.post(
        '/api/keys/generate_keypair_with_wallet_address_for_provider/EthereumOASIS'
      );
  ...
  private async setDefaultWallet(
    ...
    try {
      await this.axiosInstance.post(
        `/api/wallet/avatar/${avatarId}/default-wallet/${walletId}?providerType=EthereumOASIS`
      );
    } catch (error: any) {
  ...
  async createEthereumWallet(
    options: EthereumWalletCreationOptions
  ): Promise<EthereumWalletCreationResult> {
