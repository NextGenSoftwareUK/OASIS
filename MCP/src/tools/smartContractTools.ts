...
export const smartContractTools: Tool[] = [
  ...
  },
  {
    ...
    description: 'Deploy a compiled smart contract to the blockchain. For Solana, uses @solana/web3.js for reliable deployment (bypasses Solana CLI bugs). Supports large programs with chunked buffer writes. Can use OASIS avatar wallet via JWT token, or provide manual keypair/schema file.',
    inputSchema: {
      type: 'object',
      properties: {
        compiledContractPath: {
          type: 'string',
          description: 'Path to compiled contract file (.so for Solana, ZIP for Radix, .bin for Ethereum). For Solana, can be direct .so file path.',
        },
        blockchain: {
          type: 'string',
          enum: ['Ethereum', 'Solana', 'Radix'],
          description: 'Target blockchain platform. Solana deployment uses web3.js for reliability.',
        },
        abiPath: {
          type: 'string',
          description: 'Path to ABI/Schema file (required for Ethereum/Radix if not using OASIS). For Solana, can be empty JSON {}.',
        },
        walletKeypairPath: {
          type: 'string',
          description: 'Path to wallet keypair file (required for Solana if not using OASIS). JSON format with 64-byte array.',
        },
        oasisJwtToken: {
          type: 'string',
