...
export class SmartContractClient {
   ...
   * Supports large programs with chunked buffer writes
   */
  async deployContract(
    ...
    const response = await this.client.post('/api/v1/contracts/deploy', formData, {
      headers: formData.getHeaders(),
      timeout: 300000, // 5 minutes for deployment (Solana can take time for large programs)
    });
    ...
    return {
      contractAddress: data.contractAddress || data.programId || data.address || '',
      programId: data.programId || data.contractAddress, // Solana-specific
      transactionHash: data.transactionHash || data.signature || data.txHash || '',
      success: data.success !== false && (data.isSuccess !== false),
    };
  }
