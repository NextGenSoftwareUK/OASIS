...
ECS_SERVICE_NAME="oasis-api-service-devnet"
TASK_DEFINITION_FILE="oasis-api-task-definition-devnet.json"

echo "🚀 Deploying OASIS API Devnet Instance..."
...
# 2. Build the Docker image with devnet configuration
...
# 5. Create CloudWatch log group for devnet
echo "Creating CloudWatch log group for devnet..."
aws logs create-log-group --log-group-name "/ecs/oasis-api-devnet" --region us-east-1 2>/dev/null || echo "Log group already exists"

# 6. Create ECS service for devnet
...
if [ $? -ne 0 ]; then
    ...
    echo "✅ OASIS API Devnet deployment completed!"
    echo "🔗 Devnet endpoint: https://oasisweb4-devnet.one (when ALB is configured)"
    echo "🔗 Solana Devnet RPC: https://api.devnet.solana.com"
    echo ""
