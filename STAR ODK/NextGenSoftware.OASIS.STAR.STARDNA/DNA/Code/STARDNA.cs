...
namespace NextGenSoftware.OASIS.STAR.DNA
{
    public class STARDNA
    {
        ...
        public string BaseSTARPath { get; set; } = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "STAR_ODK_v3.0.0"); //If BaseSTARPath is blank then all other paths below are absolute otherwise they are relative to STARBasePath.
        ...
        public string BaseSTARNETPath { get; set; } = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "STAR_ODK_v3.0.0", "STARNET");
