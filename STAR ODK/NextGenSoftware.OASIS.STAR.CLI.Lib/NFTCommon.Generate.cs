...
namespace NextGenSoftware.OASIS.STAR.CLI.Lib
{
    public partial class NFTCommon
    {
        public async Task<IMintWeb4NFTRequest> GenerateNFTRequestAsync(string web3JSONMetaDataFile = "")
        {
            ...
            {
                object offChainMetaDataTypeObj = CLIEngine.GetValidInputForEnum("How do you wish to store the offchain meta data/image? OASIS, IPFS, Pinata, Arweave or External JSON URI (for the last option you will need to generate the meta data yourself and host somewhere like Pinata or Arweave and then enter the URI, for the first four options the metadata will be generated automatically)? If you choose OASIS, it will automatically auto-replicate to other providers across the OASIS through the auto-replication feature in the OASIS HyperDrive. If you choose OASIS and then IPFSOASIS for the next question for the OASIS Provider it will store it on IPFS via The OASIS and then benefit from the OASIS HyperDrive feature to provide more reliable service and up-time etc. If you choose IPFS or Pinata for this question then it will store it directly on IPFS/Pinata without any additional benefits of The OASIS. If you choose Arweave it will store it permanently on the Arweave network — data written to Arweave cannot be deleted and is stored forever.", typeof(NFTOffChainMetaType));
