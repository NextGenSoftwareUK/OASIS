    ...
    public class OAPPTemplates : STARNETUIBase<OAPPTemplate, DownloadedOAPPTemplate, InstalledOAPPTemplate, STARNETDNA>
    {
        public OAPPTemplates(Guid avatarId, STARDNA STARDNA) : base(new OAPPTemplateManager(avatarId, STARDNA),
            "Welcome to the OAPP Template Wizard", new List<string> 
            {
                ...
                "etc...",
                "",
                "Library Integration Tags (for use with imported libraries):",
                "{LIBRARYUSINGSTATEMENTS} = Automatically generates using statements for imported library proxy classes (e.g., 'using GeneratedProxies;', 'using NextGenSoftware.OASIS.API.ONODE.Core.Managers.Interop;'). This tag is only replaced if the OAPP has library dependencies with generated proxy classes in the GeneratedProxies folder.",
                "{LIBRARYMETHODSTUBS} = Automatically generates example method stubs demonstrating how to load and use each imported library. Each stub includes commented example code showing how to create an interop manager, load the library, instantiate the proxy, and use proxy methods. This tag is only replaced if the OAPP has library dependencies with generated proxy classes in the GeneratedProxies folder.",
                "",
                "More to come soon! ;-)",
                //"",
