...
namespace NextGenSoftware.OASIS.STAR.WebAPI.Controllers
{
    ...
    public class ZomesMetaDataController : STARControllerBase
    {
        ...
        public async Task<IActionResult> SearchZomesMetaData([FromQuery] string searchTerm, [FromQuery] bool showAllVersions = false, [FromQuery] int version = 0)
        {
            try
            {
                var result = await _starAPI.ZomesMetaDataDNA.SearchAsync<ZomeMetaDataDNA>(avatarId: AvatarId, searchTerm: searchTerm, searchOnlyForCurrentAvatar: true, showAllVersions: showAllVersions, version: version);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> SearchZomesMetaData([FromBody] SearchRequest request)
        {
            try
            {
                var result = await _starAPI.ZomesMetaDataDNA.SearchAsync<ZomeMetaDataDNA>(avatarId: AvatarId, searchTerm: request.SearchTerm, searchOnlyForCurrentAvatar: true);
                return Ok(result);
            }
            catch (Exception ex)
            {
