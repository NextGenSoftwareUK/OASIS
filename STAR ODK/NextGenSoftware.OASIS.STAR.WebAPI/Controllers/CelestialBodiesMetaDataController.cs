...
namespace NextGenSoftware.OASIS.STAR.WebAPI.Controllers
{
    ...
    public class CelestialBodiesMetaDataController : STARControllerBase
    {
        ...
        public async Task<IActionResult> SearchCelestialBodiesMetaData([FromQuery] string searchTerm, [FromQuery] bool showAllVersions = false, [FromQuery] int version = 0)
        {
            try
            {
                var result = await _starAPI.CelestialBodiesMetaDataDNA.SearchAsync<CelestialBodyMetaDataDNA>(avatarId: AvatarId, searchTerm: searchTerm, searchOnlyForCurrentAvatar: true, showAllVersions: showAllVersions, version: version);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> SearchCelestialBodiesMetaData([FromBody] SearchRequest request)
        {
            try
            {
                var result = await _starAPI.CelestialBodiesMetaDataDNA.SearchAsync<CelestialBodyMetaDataDNA>(avatarId: AvatarId, searchTerm: request.SearchTerm, searchOnlyForCurrentAvatar: true);
                return Ok(result);
            }
            catch (Exception ex)
            {
