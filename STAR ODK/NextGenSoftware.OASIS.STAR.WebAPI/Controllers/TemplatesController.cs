...
namespace NextGenSoftware.OASIS.STAR.WebAPI.Controllers
{
    ...
    public class TemplatesController : STARControllerBase
    {
        ...
        public async Task<IActionResult> SearchTemplates([FromBody] SearchRequest request)
        {
            try
            {
                var result = await _starAPI.OAPPTemplates.SearchAsync<OAPPTemplate>(avatarId: AvatarId, searchTerm: request.SearchTerm, searchOnlyForCurrentAvatar: true, showAllVersions: false, version: 0);
                return Ok(result);
            }
            catch (Exception ex)
            {
