...
namespace NextGenSoftware.OASIS.API.ONODE.Core.Managers
{
    public partial class NFTManager : COSMICManagerBase, INFTManager
    {
        ...
        public async Task<OASISResult<IWeb4NFT>> MintOnChainCollectionNFTAsync(IMintOnChainCollectionNFTRequest request, ProviderType providerType = ProviderType.SolanaOASIS, ResponseFormatType responseFormatType = ResponseFormatType.FormattedText)
        {
            ...
            {
                OASISErrorHandling.HandleError(ref result, $"{errorMessage} MintNftAsync succeeded but no NFTTokenAddress was returned on the minted Web3 NFT â€” cannot call SetCollectionSizeAsync.");
                return result;
            }
        ...
        private IMintWeb4NFTRequest CloneWeb4NFTRequest(IMintWeb4NFTRequest request)
        {
            return new MintWeb4NFTRequest()
            {
                ...
                WaitForNFTToSendInSeconds = request.WaitForNFTToSendInSeconds,
                WaitTillNFTSent = request.WaitTillNFTSent,
                // DISABLED: see SolanaService.cs â€” RevokeTokenAuthorities is a no-op on Metaplex NFTs.
                ...
                if (!string.IsNullOrEmpty(currentWeb3NFT.MintTransactionHash))
                {
                    ...
                    if (!currentWeb3NFT.MintTransactionHash.ToLower().Contains("error"))
                    {
                        ...
                        if (saveHolonResult != null && saveHolonResult.Result != null && !saveHolonResult.IsError)
                            ...
                            // Non-fatal in a batch â€” log as warning so IsError is not set and the batch continues; error is captured in InnerMessages and included in the final response.
                    ...
                    if (isLastWeb3NFT)
                    {
                        ...
                        if (result.SavedCount > 0)
                        {
                            ...
                            if (saveHolonResult != null && saveHolonResult.Result != null && !saveHolonResult.IsError)
                            {
                                ...
                                // Preserve any prior IsError/IsWarning from Web3 holon saves â€” don't reset to false.
                            ...
                            {
                                // Non-fatal: Web3 NFTs minted OK but Web4 holon failed â€” log as warning to preserve batch results.
                        ...
                        {
                            // Nothing was minted â€” preserve any accumulated errors and mark as error.
                            result.IsError = result.IsError || result.SavedCount == 0;
                            result.Message = FormatSuccessMessage(mergedRequest, result, metaDataProviderType, newlyMintedNFTs, responseFormatType);
                        }
