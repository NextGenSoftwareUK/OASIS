...
namespace NextGenSoftware.OASIS.API.ONODE.WebAPI.UnitTests.Subscription
{
    ...
    {
        [Fact]
        public async Task AuthorizeUsage_ForwardsBearerAndIndependentServiceIdentityToWeb4()
        {
            ...
            var client = new Web4SubscriptionAuthorizationClient(new HttpClient(handler), "https://web4.example/", "WEB8", "service-secret");

            var result = await client.AuthorizeUsageAsync("jwt-value", new UsageAuthorizationRequest { OperationId = Guid.NewGuid().ToString("D"), ConsumingService = "WEB8", RequestFingerprint = new string('a', 64), MeterCategory = "api.request", Endpoint = "POST /operation" });

            captured!.RequestUri!.ToString().Should().Be("https://web4.example/api/subscription/usage/authorize");
            captured.Headers.Authorization!.Scheme.Should().Be("Bearer");
            captured.Headers.Authorization.Parameter.Should().Be("jwt-value");
            captured.Headers.GetValues("X-OASIS-Service").Single().Should().Be("WEB8");
            captured.Headers.GetValues("X-OASIS-Service-Key").Single().Should().Be("service-secret");
        ...
        [Fact]
        public async Task AuthorizeUsage_Web4Failure_IsSurfaced()
        {
            ...
            var client = new Web4SubscriptionAuthorizationClient(new HttpClient(handler), "https://web4.example", "WEB10", "service-secret");

            Func<Task> action = () => client.AuthorizeUsageAsync("jwt-value", new UsageAuthorizationRequest { ConsumingService = "WEB10" });

            await action.Should().ThrowAsync<HttpRequestException>();
        }

        [Fact]
        public async Task UsageAuthorizeAndSettle_UseBoundOperationAndTokenFreeSettlement()
        {
            ...
            var handler = new StubHandler(async request =>
            {
                ...
                request.Headers.GetValues("X-OASIS-Service").Single().Should().Be("WEB6");
            ...
            var client = new Web4SubscriptionAuthorizationClient(new HttpClient(handler), "https://web4.example", "WEB6", "service-secret");
            ...
            var settle = await client.SettleUsageAsync(new UsageSettlementRequest { OperationId=op, UserId=Guid.NewGuid().ToString("D"), ConsumingService="WEB6", Outcome="succeeded", PromptTokens=10, CompletionTokens=5 });

            auth.OperationId.Should().Be(op);
            settle.Settled.Should().BeTrue();
            requests.Should().OnlyContain(x => x.Body.Contains(op));
            requests[0].Bearer.Should().Be("jwt");
            requests[1].Bearer.Should().BeNull();
        ...
        public async Task AuthorizeUsage_PolicyDenialIsReturnedAsDecision()
        {
            var client = new Web4SubscriptionAuthorizationClient(new HttpClient(new StubHandler(_ =>
                ...
                }))), "https://web4.example", "WEB6", "service-secret");
