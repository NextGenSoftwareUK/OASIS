            ...
            try
            {
                ...
                if (subscription == null || !IsActive(subscription))
                {
                    ...
                    if (subscription == null)
                        ...
                        await WriteJson(context, 402, new { error = "Inactive Subscription", code = "INACTIVE_SUBSCRIPTION", message = "Your subscription is not active. Please update your payment method.", planId = subscription.PlanId, status = subscription.Status, renewUrl = "/subscription/manage" });
                    return;
                }
                ...
                if (limit >= 0 && usage.RequestCount >= limit)
                {
                    ...
                    else
                    {
                        await WriteJson(context, 429, new
                        {
                            ...
                            usage = new { current = usage.RequestCount, limit, remaining = 0 },
                            options = new { upgradeUrl = "/subscription/plans", payAsYouGoUrl = "/subscription/manage", pricePerRequest = OveragePricing.GetValueOrDefault(planId, 0m) },
                            nextResetDate = new DateTime(DateTime.UtcNow.Year, DateTime.UtcNow.Month, 1).AddMonths(1)
                        });
                        return;
                    }
                }
