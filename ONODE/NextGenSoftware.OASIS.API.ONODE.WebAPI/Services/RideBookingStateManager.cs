...
namespace NextGenSoftware.OASIS.API.ONODE.WebAPI.Services
{
    ...
    public class RideBookingStateManager
    {
        ...
        {
            var state = new RideBookingState { ConversationState = initialState ?? RideConversationStates.None };
        ...
        {
            var rideState = GetOrCreate(chatId);
            if (rideState.Pickup == null) rideState.Pickup = new RideLocation();
        ...
        {
            var rideState = GetOrCreate(chatId);
            if (rideState.Destination == null) rideState.Destination = new RideLocation();
        ...
        public string GetVehiclePreference(long chatId)
            => _states.TryGetValue(chatId, out var rideState) ? rideState.VehiclePreference : null;
        ...
        public DateTime? GetScheduledTime(long chatId)
            => _states.TryGetValue(chatId, out var rideState) ? rideState.ScheduledTime : null;
        ...
        public string GetLastBookingId(long chatId)
            => _states.TryGetValue(chatId, out var rideState) ? rideState.LastBookingId : null;
    }
