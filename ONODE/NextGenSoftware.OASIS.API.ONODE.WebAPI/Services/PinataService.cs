    ...
    public class PinataService
    {
        ...
        public async Task<OASISResult<string>> UploadImageAsync(byte[] imageBytes, string fileName)
        {
            try
            {
                ...
                var request = new HttpRequestMessage(HttpMethod.Post, "/pinning/pinFileToIPFS") { Content = content };
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    return new OASISResult<string> { IsError = true, Message = $"Pinata upload failed: {responseBody}" };
                }

                var jsonDoc = JsonDocument.Parse(responseBody);
                if (jsonDoc.RootElement.TryGetProperty("IpfsHash", out var hashElement))
                {
                    var ipfsUrl = $"https://gateway.pinata.cloud/ipfs/{hashElement.GetString()}";
                    _logger?.LogInformation($"[PinataService] Upload successful: {ipfsUrl}");
                    return new OASISResult<string> { Result = ipfsUrl, Message = "Image uploaded to IPFS successfully", IsSaved = true };
                }

                return new OASISResult<string> { IsError = true, Message = "No IPFS hash in response" };
            }
            catch (Exception ex)
            {
                _logger?.LogError(ex, "[PinataService] Error uploading to Pinata");
                return new OASISResult<string> { IsError = true, Message = $"Error uploading to Pinata: {ex.Message}" };
            }
        }
        ...
        public async Task<OASISResult<string>> UploadMetadataAsync(object metadata, string fileName = "metadata.json")
        {
            try
            {
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    return new OASISResult<string> { IsError = true, Message = $"Pinata metadata upload failed: {responseBody}" };
                }
                ...
                if (jsonDoc.RootElement.TryGetProperty("IpfsHash", out var hashElement))
                {
                    ...
                    return new OASISResult<string> { Result = ipfsUrl, Message = "Metadata uploaded to IPFS successfully", IsSaved = true };
                }
            ...
            catch (Exception ex)
            {
                ...
                return new OASISResult<string> { IsError = true, Message = $"Error uploading metadata to Pinata: {ex.Message}" };
            }
        }
            ...
            object attributes = null)
        {
            ...
            catch (Exception ex)
            {
                ...
                return new OASISResult<string> { IsError = true, Message = $"Error creating NFT metadata: {ex.Message}" };
            }
        }
    }
}
