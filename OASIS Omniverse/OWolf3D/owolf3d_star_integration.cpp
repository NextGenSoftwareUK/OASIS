...
static star_item_t g_inv_items[OW_INV_MAX];
...
void OWolf3D_STAR_Init(void)
{
    ...
    if (star_api_init(&cfg) != 0) {
        OW_LOG("star_api_init failed\n");
        return;
    }
...
void OWolf3D_STAR_Tick(void)
{
    ...
    g_ow_beamedin = star_api_is_beamedin();
    if (g_ow_beamedin) {
        const char *u = star_api_get_username();
        if (u) {
            strncpy(g_ow_username, u, sizeof(g_ow_username) - 1);
            g_ow_username[sizeof(g_ow_username) - 1] = '\0';
        }
        g_ow_xp = star_api_get_xp();
    }
...
int OWolf3D_STAR_CheckDoorAccess(int keynum)
{
    ...
    if (!cross) return 0;
    return star_api_has_item(cross);
}
...
void OWolf3D_STAR_ToggleInventoryPopup(void)
{
    ...
    if (g_inv_popup_open) {
        g_inv_count = star_api_get_inventory(g_inv_items, OW_INV_MAX);
        g_inv_sel   = 0;
    }
}
...
void OWolf3D_STAR_ToggleQuestPopup(void)
{
    ...
    if (g_quest_popup_open) {
        g_quest_count = star_api_get_quests(g_quests, OW_QUEST_MAX);
        g_quest_sel   = 0;
    }
}
...
void OWolf3D_STAR_HandleKeys(void)
{
    ...
    if (g_inv_popup_open) {
        ...
        if (key_u && !s_u_prev && g_inv_count > 0) {
            star_api_use_item(g_inv_items[g_inv_sel].id);
        ...
        if (key_a && !s_a_prev && g_inv_count > 0) {
            star_api_send_to_avatar(g_inv_items[g_inv_sel].id);
        ...
        if (key_c && !s_c_prev && g_inv_count > 0) {
            star_api_send_to_clan(g_inv_items[g_inv_sel].id);
