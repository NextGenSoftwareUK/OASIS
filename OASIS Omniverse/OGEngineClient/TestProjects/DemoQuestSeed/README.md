...
```bash
cd "OASIS Omniverse/OGEngineClient"
...
```bash
dotnet run --project "OASIS Omniverse/OGEngineClient/TestProjects/DemoQuestSeed/DemoQuestSeed.csproj"
```
...
**If you see “No active quests” or an error in the popup:** The games only load quests for the currently beamed-in avatar. Use the same WEB5 API URL and same credentials as the seed. If the quest API fails (e.g. not authenticated, wrong URL), the popup shows the error message; the same error is written to **ogengine.log** (next to the game exe or ogengine.dll) and to the game console (if it consumes `ogengine_consume_console_log`).
