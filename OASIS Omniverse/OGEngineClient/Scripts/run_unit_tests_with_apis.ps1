# Start WEB4 and WEB5 APIs (root Scripts), run OGEngineClient unit tests, then stop APIs.
# Same pattern as root Scripts: start APIs first, run tests, stop APIs.
...
try {
    ...
    try {
        Write-Host ""
        Write-Host "Running STAR API Client unit tests..." -ForegroundColor Cyan
    ...
    if ($exitCode -ne 0) { Write-Host "Unit tests failed with exit code $exitCode" -ForegroundColor Red }
    Write-Host ""
    exit $exitCode
}
