# Start WEB4 and WEB5 APIs (root Scripts), run OGEngineClient integration tests against real APIs, then stop APIs.
...
try {
    ...
    if ($exitCode -ne 0) { Write-Host "Integration tests failed with exit code $exitCode" -ForegroundColor Red }
    Write-Host ""
    exit $exitCode
}
