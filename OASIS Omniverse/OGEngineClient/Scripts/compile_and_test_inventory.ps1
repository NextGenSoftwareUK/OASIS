...
$nativeDir = Join-Path $scriptDir "../TestProjects/bin/Release/net8.0/$Runtime/native"
$publishDir = Join-Path $scriptDir "../TestProjects/bin/Release/net8.0/$Runtime/publish"
$testSource = Join-Path $scriptDir "../TestProjects/test_inventory.c"
$testExe = Join-Path $scriptDir "../TestProjects/test_inventory.exe"
...
if ($RebuildClient)
{
    ...
    $csproj = Join-Path $scriptDir "OGEngineClient.csproj"
...
if (!(Test-Path (Join-Path $nativeDir "ogengine.lib"))) { throw "Missing import lib: $nativeDir/ogengine.lib" }
if (!(Test-Path (Join-Path $publishDir "ogengine.dll"))) { throw "Missing DLL: $publishDir/ogengine.dll" }
...
$compileCmd = @(
    "call `"$vcvars`"",
    "cl.exe /nologo /W3 /I`"$scriptDir`" `"$testSource`" /link /LIBPATH:`"$nativeDir`" ogengine.lib /OUT:`"$testExe`""
) -join " && "
...
Copy-Item (Join-Path $publishDir "ogengine.dll") (Join-Path $scriptDir "ogengine.dll") -Force
