# Build and deploy OGEngineClient for Windows. For Linux/macOS use build-and-deploy-star-api-unix.sh.
param(
...
$projectPath = Join-Path $projectDir "OGEngineClient.csproj"
...
$headerPath = Join-Path $projectDir "ogengine.h"
$dllPathDefault = Join-Path $publishDirDefault "ogengine.dll"
$dllPathX64 = Join-Path $publishDirX64 "ogengine.dll"
$libPath = Join-Path $nativeDir "ogengine.lib"

# Build only when: forced, or dll missing, or any project source (.cs / .csproj) or ogengine.h is newer than ogengine.dll
...
if (!$needBuild -and (Test-Path $dllPath)) {
    ...
    $defPathPs = Join-Path $projectDir "ogengine.def"
    ...
    if (-not $needBuild) {
        ...
        if ($dumpbinEarly) {
            ...
            foreach ($reqSym in @("ogengine_start_quest_then_set_active_objective")) {
                if ($exportText -notlike "*${reqSym}*") {
                    Write-Host "[STARAPI] ogengine.dll missing export $reqSym; forcing rebuild (sync with build-and-deploy-star-api-unix.sh REQUIRED_STAR_EXPORTS)."
                    $needBuild = $true
                    break
                }
            }
...
if (!$needBuild -and (Test-Path $dllPath)) {
    Write-Host "OGEngineClient unchanged (ogengine.dll is up to date), skipping build."
} else {
    if (!(Test-Path $dllPath)) { Write-Host "OGEngineClient not built yet or output missing; building..." }
    # Build Contracts first so OGEngineClient can Reference the DLL when PublishAot=true (avoids NETSDK1207 from netstandard2.1 in graph).
...
$nativeDllDefault = Join-Path $projectDir "bin/Release/net10.0/$Runtime/native/ogengine.dll"
$nativeDllX64 = Join-Path $projectDir "bin/x64/Release/net10.0/$Runtime/native/ogengine.dll"
$dllPath = $null
...
if (!(Test-Path $dllPath)) { throw "Missing ogengine.dll (checked publish and native under bin/Release and bin/x64/Release)." }
if ($dllPath -eq $nativeDllX64 -or $dllPath -eq $nativeDllDefault) {
    Write-Host "Using ogengine.dll from native folder (publish copy may have been locked by another process)."
}
if (!(Test-Path $headerPath)) { throw "Missing header: $headerPath" }
$publishDir = Split-Path -Parent $dllPath

# Keep ogengine.def in sync with C# UnmanagedCallersOnly so new exports never cause LNK2001 when building DOOM/Quake.
$generateDefScript = Join-Path $projectDir "Scripts\generate_ogengine_def.ps1"
if (Test-Path $generateDefScript) {
    & $generateDefScript -ProjectDir $projectDir | Out-Null
}

# Generate ogengine.lib from ogengine.def (now auto-generated above) so .lib has all symbols regardless of trim. Else use dumpbin on the DLL.
...
$libPath = Join-Path $nativeDirForLib "ogengine.lib"
if (Test-Path $libPath) { Remove-Item $libPath -Force -ErrorAction SilentlyContinue }
$libGenerated = $false
$staticDefPath = Join-Path $projectDir "ogengine.def"
$dumpbinCmd = $null
$libCmd = $null
...
# Prefer static .def (source of truth from ogengine.h) so .lib always has all symbols; no dependency on what the trimmed DLL exports.
if ((Test-Path $staticDefPath) -and $libCmd) {
    ...
    if (Test-Path $libPath) {
        ...
        Write-Host "Generated ogengine.lib from ogengine.def ($exportCount exports)."
    }
}
if (-not $libGenerated -and $dumpbinCmd -and $libCmd) {
    ...
    if ($exportNames -and $exportNames.Count -gt 0) {
        $defPath = Join-Path $publishDir "ogengine.def"
        ...
        if (Test-Path $libPath) {
            $libGenerated = $true
            Write-Host "Generated ogengine.lib from DLL exports ($($exportNames.Count) symbols)."
        }
    }
}
if (!(Test-Path $libPath)) {
    ...
    throw "Missing ogengine.lib. To generate it, install Visual Studio or 'Build Tools for Visual Studio' with the 'Desktop development with C++' workload (includes dumpbin and lib). Then re-run this script, or run it from 'Developer Command Prompt for VS' so PATH is set. If you already have VS/Build Tools, close any process that might be locking ogengine.dll and re-run."
}
...
foreach ($target in $targets)
{
    ...
    if (Test-Path $resolvedTarget)
    {
        ...
        Copy-FileWithRetry $dllPath (Join-Path $resolvedTarget "ogengine.dll")
        Copy-FileWithRetry $libPath (Join-Path $resolvedTarget "ogengine.lib")
        Copy-FileWithRetry $headerPath (Join-Path $resolvedTarget "ogengine.h")
    }
}
...
if ($RunSmokeTest)
{
    ...
    try
    {
        ...
        {
            & cl.exe /nologo /W3 /I"$projectDir" "$smokeSource" /link /LIBPATH:"$nativeDir" ogengine.lib /OUT:"$smokeExe"
        }
