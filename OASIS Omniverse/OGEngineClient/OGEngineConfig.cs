...
public sealed class OGEngineConfig
{
    ...
    /// <summary>After successful progress POST: merge into local cache (default) or refetch all quests. Native: ogengine_set_quest_progress_cache_refresh; ODOOM: oasisstar.json quest_progress_refresh.</summary>
    ...
    /// <summary><see cref="OGEngineTransport.Remote"/> = HTTP client (default). <see cref="OGEngineTransport.Native"/> is only supported when ogengine is built/loaded with the full OASIS stack (not the shipped NativeAOT DLL).</summary>
    public OGEngineTransport Transport { get; init; } = OGEngineTransport.Remote;
