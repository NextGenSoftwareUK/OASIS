...
void OQuake_STAR_DrawBeamedInStatus(cb_context_t* cbx) {
    ...
    /* Poll for async beam-in completion every frame so face and login state update
     * even when the inventory overlay is never opened */
    OQ_CheckAuthenticationComplete();

    /* Re-apply face preference every frame so the engine always sees the current value
     * (it may read OQuake_STAR_ShouldUseAnorakFace / oasis_star_anorak_face at any time) */
    if (g_star_initialized)
        OQ_ApplyBeamFacePreference();
