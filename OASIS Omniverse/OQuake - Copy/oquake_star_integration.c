...
void OQuake_STAR_DrawBeamedInStatus(cb_context_t* cbx) {
    ...
    extern int glwidth;
    if (!cbx) {
        Con_Printf("[OQuake] DrawBeamedInStatus: cbx is NULL!\n");
        return;
    }
    ...
    // Use a fixed offset from bottom that should work regardless of status bar size
    int y_pos = glheight - 40; // Fixed position above status bar
    if (y_pos < 8) y_pos = 8; // Safety check
    
    // Diagnostic - always show first call, then every 60 calls
    static int call_count = 0;
    call_count++;
    if (call_count == 1 || (call_count % 60 == 0)) {
        Con_Printf("[OQuake] DrawBeamedInStatus call #%d: glheight=%d, sb_lines=%d, y_pos=%d, status='%s', username=%s\n", 
                   call_count, glheight, sb_lines, y_pos, status, username ? username : "NULL");
    }
    
    // Canvas should already be set to CANVAS_DEFAULT by caller
    // Draw the status text
    Draw_String(cbx, 8, y_pos, status);
    
    // For debugging: also draw at a fixed visible position to test
    if (call_count <= 3) {
        // Draw at top-left for first few frames to verify it's working
        Draw_String(cbx, 8, 20, status);
    }
}
