...
# --- Monster kill hook: call OQuake_STAR_OnEntityFreed(ent) before ED_Free; integration does classname/state checks and PR_GetString only when progs are valid (avoids program error on new game/map load). ---
...
# Replacement: single call into integration; no PR_GetString in engine (avoids host/program error during map load or new game).
$edFreeReplacement = @'

	{
		OQuake_STAR_OnEntityFreed($3);
	}
$1$2$3$4
'@
...
function Add-MonsterHookToFile {
    ...
    if ($content -match 'OQuake_STAR_OnEntityFreed') {
        return $true
    }
    # Migrate old hook (PR_GetString + OnMonsterKilled or sv.active block) to OnEntityFreed so progs are only touched when valid.
    if ($content -match 'OQuake_STAR_OnMonsterKilled' -and $content -match 'PR_GetString\s*\(\s*(\w+)->v\.classname\s*\)') {
        $varName = $Matches[1]
        $oldBlock = '(?ms)\t\{\r?\n\t\t(?:if \(sv\.active\) \{\r?\n\t\t)?(?:const char \*ed_classname|const char \*cls) = PR_GetString\(\w+->v\.classname\);\r?\n\t\tif \((?:ed_classname|cls) && strncmp\((?:ed_classname|cls), "monster_", 8\) == 0\)\r?\n\t\t\tOQuake_STAR_OnMonsterKilled\((?:ed_classname|cls)\);\r?\n\t\t(?:\}\r?\n)?\t\}'
        $newBlock = "`t{`r`n`t`tOQuake_STAR_OnEntityFreed($varName);`r`n`t}"
        $newContent = $content -replace $oldBlock, $newBlock
        if ($newContent -ne $content) {
            Set-Content $FilePath $newContent -NoNewline
            Write-Host "[OQuake] Fixed ${FileLabel}: migrated monster hook to OQuake_STAR_OnEntityFreed (avoids program error on new game)" -ForegroundColor Green
        }
        return $true
    }
