...
void OQuake_STAR_OnEntityFreed(void* ent) {
    (void)ent;
#if 0 /* hook disabled for demo video - re-enable by changing to #if 1 */
    const char* name;
    if (!ent)
        return;
    if (!g_star_beamed_in)
    ...
    /* Only touch progs when they are loaded; during new game / map load qcvm or pr_global_struct can be NULL. */
    if (!qcvm || !pr_global_struct)
        return;
    name = PR_GetString(((edict_t*)ent)->v.classname);
    if (!name || strncmp(name, "monster_", 8) != 0)
        return;
    OQuake_STAR_OnMonsterKilled(name);
#endif
}
