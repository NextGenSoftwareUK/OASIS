...
REM Test runs with: BaseUrl http://localhost:8888, Username dellams. Avatar ID = whatever the API returns for that login (see test output ">>> TEST IS USING AVATAR ID: ... <<<").
...
if /i "%REBUILD%"=="n" (
    ...
    powershell -ExecutionPolicy Bypass -Command "& '%~dp0compile_and_test_inventory.ps1' -BaseUrl 'http://localhost:8888' -Username 'dellams' -Password 'test!' -RebuildClient:$true"
)
