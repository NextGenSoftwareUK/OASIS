...
/** Set refresh token from oasisstar.json so client can auto-renew JWT. Call after star_api_set_saved_session when restoring. */
star_api_result_t star_api_set_refresh_token(const char* refresh_token);
