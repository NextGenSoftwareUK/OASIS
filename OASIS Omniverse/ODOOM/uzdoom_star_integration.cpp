...
/** Only show "Beam-in failed" once per failure session to avoid spam when server is down. */
static bool g_star_logged_beam_in_failure_this_session = false;
...
static void ODOOM_OnAuthDone(void* user_data) {
	...
	if (success) {
		...
		g_star_logged_beam_in_failure_this_session = false;  /* Next failure can show message again. */
		...
		g_star_init_failed_this_session = true;  /* Stop retrying on door/touch until user runs "star beamin" again. */
		if (!g_star_logged_beam_in_failure_this_session) {
			g_star_logged_beam_in_failure_this_session = true;
			/* Drain any STAR log lines queued during the failed auth so they appear above the failure message (failure stays at bottom so it's clear we're not hung). */
			{
				char log_buf[512] = {};
			...
			Printf(PRINT_HIGH, "Could not connect to STAR server. Beam-in failed: %s\n", err ? err : "Unknown error");
		}
	}
}
...
static bool StarTryInitializeAndAuthenticate(bool verbose) {
	...
	if (logVerbose) {
		...
		g_star_logged_beam_in_failure_this_session = false;  /* Allow one "Beam-in failed" message for this attempt. */
	}
...
CCMD(star)
{
	...
	if (strcmp(sub, "beamin") == 0) {
		...
		{
			const char* err = star_api_get_last_error();
			Printf("Could not connect to STAR server. Beam-in failed: %s\n", err ? err : "Unknown error");
		}
		Printf("\n");
		return;
	}
	if (strcmp(sub, "beamout") == 0) {
		...
		g_star_init_failed_this_session = false;
		g_star_logged_beam_in_failure_this_session = false;
		g_star_async_auth_pending = false;
		g_star_face_suppressed_for_session = false;
