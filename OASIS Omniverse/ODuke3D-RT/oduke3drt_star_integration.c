...
#include "oduke3drt_star_integration.h"
#include "star_api.h"
#include "star_sync.h"
...
static void refresh_inventory(void) { g_inv_count = 0; /* TODO: star_sync_get_inventory */ }
static void refresh_quests(void)    { g_quest_count = 0; /* TODO: star_sync_get_quests */ }
...
void ODuke3DRT_STAR_Init(void) {
    ...
    const char *env_key    = getenv("STAR_API_KEY");
    ...
    if (StarApi_Init(&cfg) == 0) {
        ...
        OSD_Printf(ODUKE3DRT_LOG_TAG "OASIS STAR API: Not authenticated. "
                   "Set STAR_USERNAME/STAR_PASSWORD or STAR_API_KEY/STAR_AVATAR_ID.\n");
    }
...
void ODuke3DRT_STAR_OnKeyPickup(const char *key_type) {
    ...
    star_sync_on_item_pickup(key_type, "Key", 1);
    char toast[128];
...
int ODuke3DRT_STAR_CheckDoorAccess(const char *key_type) {
    ...
    int result = star_sync_check_door_access(key_type);
    if (result) {
        char toast[128];
...
void ODuke3DRT_STAR_OnActorKilled(int picnum, int engine_is_boss) {
    ...
    g_xp += xp_award;
    star_sync_on_monster_killed(name, xp_award, is_boss, entry->do_mint);
    ...
    switch (sc) {
        ...
        case sc_U:
            if (g_inv_popup_open && g_inv_selected < g_inv_count) {
                star_sync_use_item(g_inv_names[g_inv_selected]);
                char toast[128];
        ...
        case sc_A:
            if (g_inv_popup_open && g_inv_selected < g_inv_count) {
                star_sync_send_to_avatar(g_inv_names[g_inv_selected]);
                char toast[128];
        ...
        case sc_C:
            if (g_inv_popup_open && g_inv_selected < g_inv_count) {
                star_sync_send_to_clan(g_inv_names[g_inv_selected]);
                char toast[128];
