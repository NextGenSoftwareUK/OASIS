...
static idCVar d3doom3_star_api_url(
    "d3doom3_star_api_url", "https://star-api.oasisplatform.world/api",
...
static void D3Doom3_LoadJson(const char* path) {
    ...
    if (oglib_json_extract(buf, "star_api_url", val, sizeof(val)) && val[0])
        d3doom3_star_api_url.SetString(val);
...
static void D3Doom3_SaveJson(const char* path) {
    ...
    fprintf(f, "{\n");
    fprintf(f, "  \"star_api_url\": \"%s\",\n",   d3doom3_star_api_url.GetString());
...
static void D3Doom3_OnOperationCallback(star_api_result_t result, int op_type, void* user_data) {
...
static void D3Doom3_STAR_CmdHandler(const idCmdArgs& args) {
    ...
    if (!strcasecmp(subcmd, "beamin")) {
        ...
        star_sync_auth_start(args.Argv(2), args.Argv(3), D3Doom3_OnAuthDone, nullptr);
        return;
    }
    ...
    if (!strcasecmp(subcmd, "add")) {
        ...
        if (!g_d3doom3_client_ready) { common->Printf(D3DOOM3_LOG_TAG "Not beamed in.\n"); return; }
        star_api_queue_add_item(args.Argv(2), "Added via console", D3DOOM3_GAME_SOURCE, "Item", nullptr, 1, 1);
...
void D3Doom3_STAR_Init(void) {
    ...
    g_d3doom3_debug = (d3doom3_star_debug.GetInteger() != 0);
    star_api_set_debug(g_d3doom3_debug ? 1 : 0);

    /* Initialise STAR API client */
    star_api_config_t cfg = {};
    cfg.base_url           = d3doom3_star_api_url.GetString();
    ...
    star_api_set_operation_callback(D3Doom3_OnOperationCallback, nullptr);
    star_api_set_oasis_base_url(d3doom3_star_oasis_url.GetString());
    g_d3doom3_initialized = true;

    /* Restore saved session if we have a JWT */
    if (g_d3doom3_saved_jwt[0]) {
        star_api_set_saved_session(g_d3doom3_saved_jwt);
        if (g_d3doom3_saved_refresh_token[0])
            star_api_set_refresh_token(g_d3doom3_saved_refresh_token);
...
void D3Doom3_STAR_OnItemPickup(const char* inv_name, const char* inv_classname, int is_carry_item) {
    ...
    if (is_carry_item)
        star_api_queue_quest_progress_from_pickup(D3DOOM3_GAME_SOURCE, item_type, inv_name);
...
void D3Doom3_STAR_ToggleInventoryPopup(void) {
    ...
    if (g_d3doom3_inv_popup_open) {
        ...
        g_d3doom3_inv_count = 0;   /* TODO: call star_api_get_inventory */
    }
}
...
void D3Doom3_STAR_ToggleQuestPopup(void) {
    ...
    if (g_d3doom3_quest_popup_open) {
        ...
        g_d3doom3_quest_count = 0; /* TODO: call star_api_get_quests */
    }
}
...
void D3Doom3_STAR_HandleKey(int key, int down) {
    ...
    if (key == 'u' && g_d3doom3_inv_popup_open && g_d3doom3_inv_selected < g_d3doom3_inv_count) {
        star_api_use_item(g_d3doom3_inv_names[g_d3doom3_inv_selected]);
    ...
    if (key == 'a' && g_d3doom3_inv_popup_open && g_d3doom3_inv_selected < g_d3doom3_inv_count) {
        star_api_send_item_to_avatar(g_d3doom3_inv_names[g_d3doom3_inv_selected]);
    ...
    if (key == 'c' && g_d3doom3_inv_popup_open && g_d3doom3_inv_selected < g_d3doom3_inv_count) {
        star_api_send_item_to_clan(g_d3doom3_inv_names[g_d3doom3_inv_selected]);
