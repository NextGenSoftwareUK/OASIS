...
static int OQ_SaveJsonConfig(const char *json_path) {
    ...
    if (g_star_initialized) {
        ...
        {
            char rt[2048] = {0};
            if (star_api_get_current_refresh_token((char*)rt, sizeof(rt)) > 0 && rt[0]) {
                q_strlcpy(g_oq_saved_refresh_token, rt, sizeof(g_oq_saved_refresh_token));
                fprintf(f, ",\n  \"refresh_token\": \"");
                { const char* p; for (p = rt; *p; p++) { if (*p == '"' || *p == '\\') fputc('\\', f); fputc((unsigned char)*p, f); } }
                fprintf(f, "\"");
            }
        }
