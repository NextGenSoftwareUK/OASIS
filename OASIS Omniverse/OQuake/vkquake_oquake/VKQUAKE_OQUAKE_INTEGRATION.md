...
**In `Host_Init()`** (e.g. after `PR_Init();` or near end before `host_initialized = true`):
...
- Add `oquake_star_integration.c` and `pr_ext_oquake.c` to the Quake project (e.g. the vkquake or game lib project that already has `pr_ext.c`).
...
2. Copy `star_api.dll` next to `vkquake.exe`.
