...
static int name_to_picnum(const char *entity_id) {
    ...
    if (strncmp(id, "oduke3d_", 8) == 0)   id += 8;
    else if (strncmp(id, "oduke3drt_", 10) == 0) id += 10;
    for (int i = 0; s_picnum_map[i].name; i++)
        if (strcmp(id, s_picnum_map[i].name) == 0) return s_picnum_map[i].picnum;
    return -1;
}

/* EDuke32 runtime spawn — inserts a new actor sprite into the world. */
...
static int OD3_ExtractJsonValue(const char *json, const char *key, char *out, int maxlen)
{
    char search[128];
    const char *p;
    int i;
...
void ODuke3D_STAR_Tick(void) {
        ...
        {
            {
                ...
                if (pn >= 0) {
                    ...
                    oglib_log(OGLIB_LOG_WARN, "OASIS SpawnEvent: unknown entity '%s' — no Duke3D picnum", entity_id);
                }
            }
    ...
    /* --- cross-game event poll --- */
    {
        ...
        {
            char evt_type[64] = "";
            OD3_ExtractJsonValue(evt_json, "EventType", evt_type, sizeof(evt_type));
            if (strcmp(evt_type, "ShowNarration") == 0) {
                char narration[256] = "";
                OD3_ExtractJsonValue(evt_json, "NarrationText", narration, sizeof(narration));
                if (narration[0]) set_toast(narration);
            } else if (strcmp(evt_type, "PlayAudio") == 0) {
                char audio_title[128] = "", audio_url[256] = "";
                OD3_ExtractJsonValue(evt_json, "AudioTitle", audio_title, sizeof(audio_title));
                OD3_ExtractJsonValue(evt_json, "AudioUrl",   audio_url,   sizeof(audio_url));
                oglib_log(OGLIB_LOG_INFO, "OASIS PlayAudio: %s (%s) — streaming not yet implemented", audio_title, audio_url);
                /* TODO: play audio via EDuke32 sound system */
            } else if (strcmp(evt_type, "PlayVideo") == 0) {
                char video_title[128] = "", video_url[256] = "";
                OD3_ExtractJsonValue(evt_json, "VideoTitle", video_title, sizeof(video_title));
                OD3_ExtractJsonValue(evt_json, "VideoUrl",   video_url,   sizeof(video_url));
                ...
                OD3_ExtractJsonValue(evt_json, "WebsiteUrl", website_url, sizeof(website_url));
