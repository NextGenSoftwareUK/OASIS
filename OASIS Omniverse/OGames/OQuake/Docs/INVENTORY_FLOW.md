...
- **On pickup:** OQuake calls `ogengine_queue_add_item(name, description, gameSource, itemType, NULL, quantity, 1)`. That enqueues to the C# client; no local array in the engine.
- **To show inventory:** OQuake calls `ogengine_get_inventory(&list)`. The C# client returns **API cache + local pending** merged (one row per type, qty = API qty + unsynced pickups). No append step in the engine.
...
- **OQuake:** `OQ_AddInventoryEvent` / `OQ_AddInventoryUnlockIfMissing` → `ogengine_queue_add_item` only. `OQ_RefreshOverlayFromClient` → `ogengine_get_inventory` only.
...
// OQ_AddInventoryEvent → ogengine_queue_add_item(item_prefix, description, "Quake", item_type, NULL, delta, 1);
```
...
```c
OQ_RefreshOverlayFromClient();  // ogengine_get_inventory(&list) → fill g_inventory_entries from list
```
...
| **OQuake**                 | `ogengine_queue_add_item` on pickup; `ogengine_get_inventory` to load; `star_sync_pump` for auth/send/use |
