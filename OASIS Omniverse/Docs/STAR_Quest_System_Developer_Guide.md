...
This document explains how **quests** flow from the **WEB5 STAR API** through **OGEngineClient** into **native games** (OQuake, ODOOM), so you can extend the backend, ship a new client, or integrate another engine without reverse-engineering the tree.
...
| STAR + WEB5 overview | `Docs/Devs/API Documentation/WEB5_OGENGINE_Documentation.md` (Quests section) |
| **OGEngine (WEB4 + WEB5 + OGEngineClient)** | `OASIS Omniverse/Docs/OGEngine_Overview.md` |
| OGEngineClient design | `OASIS Omniverse/OGEngineClient/README.md` |
...
1. **WEB5 STAR API** — Source of truth for quest definitions, status, objective completion, and **realtime progress** (`/progress`). Hosted STAR ODK / ONODE stack; base URL is `ogengine_url` in `oasisstar.json`.
2. **OGEngineClient** — NativeAOT (or managed) assembly implementing `ogengine_*`. Handles auth headers, **quest list cache**, **optimistic merge** after progress POSTs, background workers, and serialization for in-game UI.
...
Responses are **flat JSON** suited for clients (not full holon graphs). Implementations live in STAR ODK / ONODE; keep field names aligned with **`NextGenSoftware.OASIS.API.Contracts`** quest DTOs so OGEngineClient deserialization stays stable.
...
- **`StarApiConfig.ClientGameSource`** / native **`client_game_source`** in `ogengine_config_t` (e.g. ODOOM sets `"ODOOM"`), and
...
## 3. Client level (OGEngineClient)

### 3.1 Key types

- **`StarQuestInfo`**, **`StarQuestObjective`**, dictionary types — in **API.Contracts** (see README).
- **`QuestProgressCacheRefreshMode`**: **`ClientCacheMerge`** (default) vs **`FullServerRefresh`** after each successful progress POST (`ogengine_set_quest_progress_cache_refresh` / `oasisstar.json` `quest_progress_refresh`).
...
### 3.3 C ABI — quest-related (`ogengine.h`)

| Export | Role |
|--------|------|
| `ogengine_start_quest` | Start by quest GUID string |
| `ogengine_start_quest_then_set_active_objective` | Start then persist active objective (async ordering) |
| `ogengine_complete_quest_objective` | Manual objective complete |
| `ogengine_complete_quest` | Quest complete |
| `ogengine_get_quests_string` | Full serialized list (cache) |
| `ogengine_get_top_level_quests_string` | Parent quests only (left panel) |
| `ogengine_get_quest_sub_quests_string` | Sub-quests for parent id |
| `ogengine_get_quest_objectives_string` | Objectives for parent id |
| `ogengine_get_quest_prereqs_string` | Prerequisite quests |
| `ogengine_get_quest_objective_requirements_string` | Human-readable requirement lines |
| `ogengine_get_quest_tracker_objectives_string` | Tracker lines |
| `ogengine_get_quest_tracker_active_objective_index` | First incomplete index |
| `ogengine_get_tracker_quest_name` | HUD title |
| `ogengine_invalidate_quest_cache` | Force refetch path on next read |
| `ogengine_refresh_quest_cache_in_background` | Non-blocking refresh |
| `ogengine_set_quest_popup_open` | Suppress progress + full GET while UI open |
| `ogengine_get_active_quest_id` / `ogengine_get_active_objective_id` | From last profile |
| `ogengine_set_active_quest` | Persist tracker to WEB4 avatar detail |
| `ogengine_queue_quest_progress_from_pickup` | Progress only (no inventory add) |
| `ogengine_queue_monster_kill` | Kills + XP → progress pipeline |
| `ogengine_queue_quest_level_time` | Level elapsed seconds (OQuake ~10s throttle) |
...
2. **`ogengine_init`** with **`Web5StarApiBaseUrl`**, **`Web4OasisApiBaseUrl`**, and set **`client_game_source`** to a string your **quest objectives** use in DNA/API.
3. After beam-in, call **`ogengine_refresh_avatar_profile`** (or equivalent) so **active quest / objective** loads into the client cache.
...
|---------|--------|--------|
| Quest list UI | **`oquake_ogengine_integration.c`** — Q key, tabbed panels, Enter/K | **`uzdoom_ogengine_integration.cpp`** + **`odoom_inventory_popup.zs`** |
| Tracker / HUD | Same file + `ogengine_get_quest_tracker_*` | ZScript + CVars |
...
OQuake uses **`ogengine_set_quest_progress_cache_refresh`** from loaded config where wired.
