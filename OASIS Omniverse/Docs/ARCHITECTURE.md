...
- **Heavy lifting lives in the C# OGEngineClient:** HTTP, caching, queuing, background workers, mint + add_item batching, and all API logic.
- **Games are thin:** They call a small C API (`ogengine_*` and, where needed, `star_sync_*`). They do **not** implement threads for API work, sync logic, or inventory merging.
...
│  • Calls: ogengine_* (and star_sync_* for auth/inventory flow only)        │
...
│  ogengine.h + ogengine.dll exports                                         │
│  • ogengine_queue_pickup_with_mint, ogengine_queue_add_item                 │
│  • ogengine_has_item, ogengine_get_inventory, ogengine_use_item             │
│  • ogengine_authenticate, ogengine_init, ogengine_invalidate_inventory_cache │
│  • ogengine_complete_quest_objective, ogengine_send_item_to_avatar, etc.     │
└─────────────────────────────────────────────────────────────────────────────┘
                                      │
                                      ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  LAYER 3: C# OGEngineClient (all heavy lifting)                              │
...
| **Inventory cache** | C# client only | Calls `ogengine_has_item`, `ogengine_get_inventory`; client uses cache and hits API only when needed. |
| **Pickup (add item)** | C# client | Calls `ogengine_queue_add_item` (no mint) or `ogengine_queue_pickup_with_mint` (mint + add). Client queues and processes in background. |
| **Mint + add** | C# client | One call: `ogengine_queue_pickup_with_mint`. Client mints (if do_mint), then adds item with NFT id, in its existing add-item worker. |
| **Use item (e.g. door)** | C# client (cache + API) | Calls `ogengine_has_item` then `ogengine_use_item` (or queue/flush). No game-side use-item queue. |
| **Quest objectives** | C# client (optional queue) | Calls `ogengine_complete_quest_objective` (fire-and-forget or queued). |
| **Send item** | C# client | Calls `ogengine_send_item_to_avatar` (or async via star_sync). |
| **Config (URLs, mint flags, provider)** | Game reads oasisstar.json / CVars; client uses values passed per call | Game passes provider, send_to_address, etc. into `ogengine_queue_pickup_with_mint`; client does not read game config. |
...
- **Init:** Call `ogengine_init` (config: STAR API URL, WEB4 URL, etc.).
...
- **Game calls:**
  - If **mint enabled** for this category:  
    `ogengine_queue_pickup_with_mint(item_name, description, game_source, item_type, 1, provider, send_to_address, quantity)`  
  - Else:  
    `ogengine_queue_add_item(item_name, description, game_source, item_type, NULL, quantity, 1)`  
- **Optional:** `ogengine_complete_quest_objective(quest_id, objective_id, game_source)` for quest progress.
...
- **Game calls:**  
  `ogengine_has_item(key_name)` → if true, allow door and optionally  
  `ogengine_use_item(key_name, context)` (or queue + flush if using use_item queue).
- **No game logic:** No inventory merge; client’s cache is the source of truth.

### 5. Inventory UI

- **Trigger:** Player opens inventory overlay.
- **Game calls:** `ogengine_get_inventory(&list)`, then iterate list to fill UI; `ogengine_free_item_list(list)` when done.
- **Optional:** After send-item or after beam-in, call `ogengine_invalidate_inventory_cache()` so next get sees fresh data.
- **No game logic:** No game-side cache; client returns cached or fetches when needed.

### 6. Config

- **Game reads:** oasisstar.json (or equivalent) for: `ogengine_url`, `oasis_api_url`, `mint_weapons`, `mint_keys`, `nft_provider`, `send_to_address_after_minting`, etc.
- **Game passes:** Those values into the C API (e.g. into `ogengine_queue_pickup_with_mint`). The client does not read config files; it stays generic.

---

## C API surface (games only need this)

Games depend only on the C ABI in `ogengine.h` (and optionally `ogengine_sync.h` for auth/inventory flow). Summary:

| Purpose | C API | Notes |
|--------|--------|--------|
| Init | `ogengine_init`, `ogengine_set_oasis_base_url` | Once at startup. |
| Auth | `star_sync_auth_start`, `star_sync_auth_get_result`, … | Or equivalent that drives client auth. |
| Pickup (no mint) | `ogengine_queue_add_item` | Client queues and processes in background. |
| Pickup (with mint) | `ogengine_queue_pickup_with_mint` | Client mints then adds in background; same worker as add_item. |
| Flush (optional) | `ogengine_flush_add_item_jobs` | Only if game needs to block until queue is empty; usually not required. |
| Door / has item | `ogengine_has_item` | Cache-first in client. |
| Use item | `ogengine_use_item` or queue + `ogengine_flush_use_item_jobs` | Client updates cache/API. |
| Inventory UI | `ogengine_get_inventory`, `ogengine_free_item_list` | Client returns cached or fetches. |
...
- **ODOOM:** Pickup → `ogengine_queue_pickup_with_mint` or `ogengine_queue_add_item`; door → `ogengine_has_item` + `ogengine_use_item`; beam-in → star_sync auth; inventory UI → `ogengine_get_inventory`. No game-side mint or add_item threads.
- **OQuake:** Same pattern: pickup uses `ogengine_queue_pickup_with_mint` or `ogengine_queue_add_item`; door check uses `ogengine_has_item`; inventory and auth follow the same C API and star_sync flow.
...
1. **Link** the OGEngineClient native DLL and use `ogengine.h` (and optionally `ogengine_sync.h`).
2. **Init** with `ogengine_init` (and set WEB4 URL if needed).
...
4. **Pickup:** On collect, call `ogengine_queue_pickup_with_mint` or `ogengine_queue_add_item`; optionally `ogengine_complete_quest_objective`. No mint or add logic in the game.
5. **Door / use:** Call `ogengine_has_item`; if true, allow action and optionally call `ogengine_use_item`.
6. **Inventory UI:** Call `ogengine_get_inventory` and display; call `ogengine_free_item_list` when done.
...
**Status:** Done. star_sync is implemented in C# in **StarSyncExports.cs** and exported from ogengine.dll. BUILD ODOOM and BUILD_OQUAKE default to **OASIS_STAR_SYNC_IN_CLIENT=1**, so games use the client’s star_sync and do not compile `ogengine_sync.c`. The C file **ogengine_sync.c** remains in OGEngineClient so you can switch back (set `OASIS_STAR_SYNC_IN_CLIENT=0` or change one line in the build script). See **[STAR_INTEGRATION_AUDIT.md](./STAR_INTEGRATION_AUDIT.md)** (“Star_sync: in-client (C#) default vs C implementation”) and **ogengine_sync.h** for how to switch back.

**Goal:** Implement all of **star_sync** (async auth, async inventory refresh, async send item, async use item, init/cleanup/pump) inside the C# OGEngineClient and export the same `star_sync_*` C API from the client DLL. Then remove the C implementation (`ogengine_sync.c`) from ODOOM and OQuake builds; keep `ogengine_sync.h` for the API declarations; games link only the client and call the client’s exports.
...
- **[OGEngineClient/README.md](../OGEngineClient/README.md)** – C# client behaviour, cache, queues, build.
