# Start WEB4 and WEB5 APIs in serial (WEB4 first, then WEB5), run both API integration tests, then stop the APIs.
# Use run_web4_web5_integration_tests.ps1 if you do not need the APIs running.
...
try {
    ...
    Write-Host "  WEB4 (ONODE) integration tests..." -ForegroundColor Cyan
    $null = dotnet test $web4Project -m:1 -v n --logger "console;verbosity=minimal"
    $web4Exit = $LASTEXITCODE

    Write-Host ""
    Write-Host "  WEB5 (STAR) integration tests..." -ForegroundColor Cyan
