using System;
using NextGenSoftware.OASIS.API.Providers.LensV2OASIS;

namespace NextGenSoftware.OASIS.API.Providers.LensV2OASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named LENSV2OASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class LensV2OASISTestFactory
    {
        public static LensV2OASIS Create()
        {
            var type = typeof(LensV2OASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"LENSV2OASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (LensV2OASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(LensV2OASIS).Name} - no usable constructor.");
        }
