...
namespace NextGenSoftware.OASIS.API.Providers.LensV2OASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from LENSV2OASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class LensV2OASISIntegrationTests
    {
        private LensV2OASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = LensV2OASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live LensV2OASIS backend configured - set the LENSV2OASIS_* environment variables to run this.");
        }
