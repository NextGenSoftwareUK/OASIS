...
namespace NextGenSoftware.OASIS.API.Providers.DiscordOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named DISCORDOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class DiscordOASISTestFactory
    {
        public static DiscordOASIS Create()
        {
            var type = typeof(DiscordOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"DISCORDOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (DiscordOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(DiscordOASIS).Name} - no usable constructor.");
        }
