...
namespace NextGenSoftware.OASIS.API.Providers.FarcasterOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from FARCASTEROASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class FarcasterOASISIntegrationTests
    {
        private FarcasterOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = FarcasterOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live FarcasterOASIS backend configured - set the FARCASTEROASIS_* environment variables to run this.");
        }
