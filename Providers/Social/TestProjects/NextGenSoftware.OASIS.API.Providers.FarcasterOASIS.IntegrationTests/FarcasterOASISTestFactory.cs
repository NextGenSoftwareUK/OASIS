using System;
using NextGenSoftware.OASIS.API.Providers.FarcasterOASIS;

namespace NextGenSoftware.OASIS.API.Providers.FarcasterOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named FARCASTEROASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class FarcasterOASISTestFactory
    {
        public static FarcasterOASIS Create()
        {
            var type = typeof(FarcasterOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"FARCASTEROASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (FarcasterOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(FarcasterOASIS).Name} - no usable constructor.");
        }
