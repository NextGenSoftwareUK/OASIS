...
namespace NextGenSoftware.OASIS.API.Providers.LoomOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from LOOMOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class LoomOASISIntegrationTests
    {
        private LoomOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = LoomOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live LoomOASIS backend configured - set the LOOMOASIS_* environment variables to run this.");
        }
