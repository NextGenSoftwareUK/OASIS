using System;
using NextGenSoftware.OASIS.API.Providers.LoomOASIS;

namespace NextGenSoftware.OASIS.API.Providers.LoomOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named LOOMOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class LoomOASISTestFactory
    {
        public static LoomOASIS Create()
        {
            var type = typeof(LoomOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"LOOMOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (LoomOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(LoomOASIS).Name} - no usable constructor.");
        }
