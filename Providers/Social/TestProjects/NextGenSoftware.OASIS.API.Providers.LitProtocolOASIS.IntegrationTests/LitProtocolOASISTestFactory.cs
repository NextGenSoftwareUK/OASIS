using System;
using NextGenSoftware.OASIS.API.Providers.LitProtocolOASIS;

namespace NextGenSoftware.OASIS.API.Providers.LitProtocolOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named LITPROTOCOLOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class LitProtocolOASISTestFactory
    {
        public static LitProtocolOASIS Create()
        {
            var type = typeof(LitProtocolOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"LITPROTOCOLOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (LitProtocolOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(LitProtocolOASIS).Name} - no usable constructor.");
        }
