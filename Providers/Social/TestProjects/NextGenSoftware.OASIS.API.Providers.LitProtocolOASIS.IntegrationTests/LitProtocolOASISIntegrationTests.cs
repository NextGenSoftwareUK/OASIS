...
namespace NextGenSoftware.OASIS.API.Providers.LitProtocolOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from LITPROTOCOLOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class LitProtocolOASISIntegrationTests
    {
        private LitProtocolOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = LitProtocolOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live LitProtocolOASIS backend configured - set the LITPROTOCOLOASIS_* environment variables to run this.");
        }
