using System;
using NextGenSoftware.OASIS.API.Providers.TheGraphOASIS;

namespace NextGenSoftware.OASIS.API.Providers.TheGraphOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named THEGRAPHOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class TheGraphOASISTestFactory
    {
        public static TheGraphOASIS Create()
        {
            var type = typeof(TheGraphOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"THEGRAPHOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (TheGraphOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(TheGraphOASIS).Name} - no usable constructor.");
        }
