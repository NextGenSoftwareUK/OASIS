...
namespace NextGenSoftware.OASIS.API.Providers.TheGraphOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from THEGRAPHOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class TheGraphOASISIntegrationTests
    {
        private TheGraphOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = TheGraphOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live TheGraphOASIS backend configured - set the THEGRAPHOASIS_* environment variables to run this.");
        }
