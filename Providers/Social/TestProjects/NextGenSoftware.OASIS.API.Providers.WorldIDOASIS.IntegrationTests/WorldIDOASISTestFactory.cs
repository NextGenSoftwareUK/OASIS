using System;
using NextGenSoftware.OASIS.API.Providers.WorldIDOASIS;

namespace NextGenSoftware.OASIS.API.Providers.WorldIDOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named WORLDIDOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class WorldIDOASISTestFactory
    {
        public static WorldIDOASIS Create()
        {
            var type = typeof(WorldIDOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"WORLDIDOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (WorldIDOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(WorldIDOASIS).Name} - no usable constructor.");
        }
