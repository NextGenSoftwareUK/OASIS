...
namespace NextGenSoftware.OASIS.API.Providers.WorldIDOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from WORLDIDOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class WorldIDOASISIntegrationTests
    {
        private WorldIDOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = WorldIDOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live WorldIDOASIS backend configured - set the WORLDIDOASIS_* environment variables to run this.");
        }
