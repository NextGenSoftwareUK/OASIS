using System;
using NextGenSoftware.OASIS.API.Providers.NostrOASIS;

namespace NextGenSoftware.OASIS.API.Providers.NostrOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named NOSTROASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class NostrOASISTestFactory
    {
        public static NostrOASIS Create()
        {
            var type = typeof(NostrOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"NOSTROASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (NostrOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(NostrOASIS).Name} - no usable constructor.");
        }
