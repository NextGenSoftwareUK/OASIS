...
namespace NextGenSoftware.OASIS.API.Providers.NostrOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from NOSTROASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class NostrOASISIntegrationTests
    {
        private NostrOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = NostrOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live NostrOASIS backend configured - set the NOSTROASIS_* environment variables to run this.");
        }
