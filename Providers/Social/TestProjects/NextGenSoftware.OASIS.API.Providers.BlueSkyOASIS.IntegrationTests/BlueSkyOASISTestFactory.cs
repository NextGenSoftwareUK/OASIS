using System;
using NextGenSoftware.OASIS.API.Providers.BlueSkyOASIS;

namespace NextGenSoftware.OASIS.API.Providers.BlueSkyOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named BLUESKYOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class BlueSkyOASISTestFactory
    {
        public static BlueSkyOASIS Create()
        {
            var type = typeof(BlueSkyOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"BLUESKYOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (BlueSkyOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(BlueSkyOASIS).Name} - no usable constructor.");
        }
