...
namespace NextGenSoftware.OASIS.API.Providers.BlueSkyOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from BLUESKYOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class BlueSkyOASISIntegrationTests
    {
        private BlueSkyOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = BlueSkyOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live BlueSkyOASIS backend configured - set the BLUESKYOASIS_* environment variables to run this.");
        }
