using System;
using NextGenSoftware.OASIS.API.Providers.LensOASIS;

namespace NextGenSoftware.OASIS.API.Providers.LensOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named LENSOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class LensOASISTestFactory
    {
        public static LensOASIS Create()
        {
            var type = typeof(LensOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"LENSOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (LensOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(LensOASIS).Name} - no usable constructor.");
        }
