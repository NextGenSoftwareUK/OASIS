...
namespace NextGenSoftware.OASIS.API.Providers.LensOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from LENSOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class LensOASISIntegrationTests
    {
        private LensOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = LensOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live LensOASIS backend configured - set the LENSOASIS_* environment variables to run this.");
        }
