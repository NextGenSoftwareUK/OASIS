...
namespace NextGenSoftware.OASIS.API.Providers.StoryProtocolOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from STORYPROTOCOLOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class StoryProtocolOASISIntegrationTests
    {
        private StoryProtocolOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = StoryProtocolOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live StoryProtocolOASIS backend configured - set the STORYPROTOCOLOASIS_* environment variables to run this.");
        }
