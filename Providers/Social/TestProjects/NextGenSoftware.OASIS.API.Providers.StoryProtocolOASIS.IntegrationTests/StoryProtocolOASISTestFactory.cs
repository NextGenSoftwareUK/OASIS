using System;
using NextGenSoftware.OASIS.API.Providers.StoryProtocolOASIS;

namespace NextGenSoftware.OASIS.API.Providers.StoryProtocolOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named STORYPROTOCOLOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class StoryProtocolOASISTestFactory
    {
        public static StoryProtocolOASIS Create()
        {
            var type = typeof(StoryProtocolOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"STORYPROTOCOLOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (StoryProtocolOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(StoryProtocolOASIS).Name} - no usable constructor.");
        }
