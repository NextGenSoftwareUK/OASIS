using System;
using NextGenSoftware.OASIS.API.Providers.MatrixOASIS;

namespace NextGenSoftware.OASIS.API.Providers.MatrixOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named MATRIXOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class MatrixOASISTestFactory
    {
        public static MatrixOASIS Create()
        {
            var type = typeof(MatrixOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"MATRIXOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (MatrixOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(MatrixOASIS).Name} - no usable constructor.");
        }
