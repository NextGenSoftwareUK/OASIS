...
namespace NextGenSoftware.OASIS.API.Providers.MatrixOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from MATRIXOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class MatrixOASISIntegrationTests
    {
        private MatrixOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = MatrixOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live MatrixOASIS backend configured - set the MATRIXOASIS_* environment variables to run this.");
        }
