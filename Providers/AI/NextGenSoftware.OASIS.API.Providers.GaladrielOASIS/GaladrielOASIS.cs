...
namespace NextGenSoftware.OASIS.API.Providers.GaladrielOASIS
{
    public class GaladrielOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISNETProvider, IOASISBlockchainStorageProvider
    {
        ...
        {
            var body = JsonConvert.SerializeObject(new { jsonrpc = "2.0", id = _rpcId++, method, @params = parms ?? new object[0] });
        ...
        public override async Task<OASISResult<bool>> ActivateProviderAsync()
        {
            ...
            try
            {
                ...
                if (json["result"] != null) { _isActivated = true; r.Result = true; r.Message = $"GaladrielOASIS activated — block {json["result"]}"; }
                else OASISErrorHandling.HandleError(ref r, $"Galadriel RPC error: {json["error"]}");
            }
        ...
        public override async Task<OASISResult<IAvatar>> LoadAvatarByProviderKeyAsync(string address, int v = 0)
        {
            ...
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"Galadriel LoadAvatar failed: {ex.Message}", ex); }
            return r;
        }
        public override async Task<OASISResult<IHolon>> LoadHolonAsync(string key, bool lc = true, bool rec = true, int md = 0, bool coe = true, bool lcfp = false, int v = 0)
        {
            ...
            try
            {
                // key = tx hash or block number/hash
                JObject json;
                ...
                var holon = new Holon { Name = $"Galadriel {key}" };
                holon.ProviderUniqueStorageKey[Core.Enums.ProviderType.GaladrielOASIS] = key;
                holon.MetaData["key"] = key;
                holon.MetaData["hash"] = result?["hash"]?.ToString() ?? key;
                holon.MetaData["from"] = result?["from"]?.ToString() ?? "";
                holon.MetaData["to"] = result?["to"]?.ToString() ?? result?["miner"]?.ToString() ?? "";
                r.Result = holon;
            }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"Galadriel LoadHolon failed: {ex.Message}", ex); }
            return r;
        }
        public override async Task<OASISResult<IHolon>> SaveHolonAsync(IHolon h, bool sc = true, bool rec = true, int md = 0, bool coe = true, bool scop = false) { if (h.Id == Guid.Empty) h.Id = Guid.NewGuid(); return new OASISResult<IHolon> { Result = h, Message = "Galadriel writes require eth_sendRawTransaction with a signed transaction; data not persisted externally." }; }
