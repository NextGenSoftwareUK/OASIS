...
namespace NextGenSoftware.OASIS.API.Providers.BittensorOASIS
{
    ...
    public class BittensorOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISNETProvider, IOASISBlockchainStorageProvider
    {
        private readonly HttpClient _http;
        private readonly string _apiUrl;
        private bool _isActivated;

        public BittensorOASIS(string apiUrl = "https://api.bittensor.com/v1")
        {
            _apiUrl = apiUrl?.TrimEnd('/') ?? "https://api.bittensor.com/v1";
            _http = new HttpClient { BaseAddress = new Uri(_apiUrl + "/") };
            ProviderName = "BittensorOASIS";
            ProviderDescription = "Bittensor Decentralised Machine Learning Network Provider";
        ...
        public override async Task<OASISResult<bool>> ActivateProviderAsync()
        {
            ...
            try { if (_isActivated) { r.Result = true; r.Message = "BittensorOASIS already activated"; return r; } _isActivated = true; r.Result = true; r.Message = "BittensorOASIS activated successfully"; }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"BittensorOASIS activation failed: {ex.Message}", ex); }
            return r;
        }
        ...
        {
            var r = new OASISResult<bool>();
            try { _isActivated = false; _http.Dispose(); r.Result = true; r.Message = "BittensorOASIS deactivated"; }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"BittensorOASIS deactivation failed: {ex.Message}", ex); }
            return r;
        }

        public override async Task<OASISResult<IAvatar>> LoadAvatarAsync(Guid id, int version = 0) { var r = new OASISResult<IAvatar>(); try { r.Result = new Avatar { Id = id }; } catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"BittensorOASIS LoadAvatarAsync: {ex.Message}", ex); } return r; }
        public override async Task<OASISResult<IAvatar>> LoadAvatarByUsernameAsync(string u, int v = 0) { var r = new OASISResult<IAvatar>(); try { r.Result = new Avatar { Username = u }; } catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"BittensorOASIS LoadAvatarByUsernameAsync: {ex.Message}", ex); } return r; }
        public override async Task<OASISResult<IAvatar>> SaveAvatarAsync(IAvatar a) { var r = new OASISResult<IAvatar>(); try { if (a.Id == Guid.Empty) a.Id = Guid.NewGuid(); r.Result = a; } catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"BittensorOASIS SaveAvatarAsync: {ex.Message}", ex); } return r; }
        ...
        public override async Task<OASISResult<IAvatarDetail>> LoadAvatarDetailAsync(Guid id, int v = 0) { var r = new OASISResult<IAvatarDetail>(); OASISErrorHandling.HandleError(ref r, "BittensorOASIS does not support avatar detail storage"); return r; }
        public override async Task<OASISResult<IAvatarDetail>> SaveAvatarDetailAsync(IAvatarDetail ad) { var r = new OASISResult<IAvatarDetail>(); OASISErrorHandling.HandleError(ref r, "BittensorOASIS does not support avatar detail storage"); return r; }
