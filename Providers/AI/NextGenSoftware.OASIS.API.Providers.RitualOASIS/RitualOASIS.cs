...
namespace NextGenSoftware.OASIS.API.Providers.RitualOASIS
{
    public class RitualOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISNETProvider, IOASISBlockchainStorageProvider
    {
        ...
        public RitualOASIS(string baseUrl = "http://localhost:4000")
        {
            ...
            ProviderName = "RitualOASIS"; ProviderDescription = "Ritual Infernet node REST API for decentralised AI compute jobs.";
        ...
        public override async Task<OASISResult<bool>> ActivateProviderAsync()
        {
            ...
            try { await GetAsync("api/status"); _isActivated = true; r.Result = true; r.Message = "RitualOASIS activated"; }
        ...
        public override async Task<OASISResult<IAvatar>> LoadAvatarByProviderKeyAsync(string containerId, int v = 0)
        {
            var r = new OASISResult<IAvatar>();
            try
            {
                // containerId = Infernet container ID
                var json = await GetAsync($"api/containers");
                var containers = json["containers"] as JArray ?? new JArray(json);
                var match = containers.First as JObject;
                foreach (var c in containers) { if (c["id"]?.ToString() == containerId) { match = c as JObject; break; } }
                ...
                avatar.MetaData["image"] = match?["image"]?.ToString() ?? "";
                avatar.MetaData["description"] = match?["description"]?.ToString() ?? "";
                r.Result = avatar;
            }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"Ritual LoadAvatar failed: {ex.Message}", ex); }
            return r;
        }
        public override async Task<OASISResult<IHolon>> LoadHolonAsync(string key, bool lc = true, bool rec = true, int md = 0, bool coe = true, bool lcfp = false, int v = 0)
        {
            ...
            {
                // key = job ID
                var json = await GetAsync($"api/jobs/{Uri.EscapeDataString(key)}");
                var job = json["job"] ?? json;
                var holon = new Holon { Name = $"Ritual Job {key}" };
                holon.ProviderUniqueStorageKey[Core.Enums.ProviderType.RitualOASIS] = key;
                holon.MetaData["job_id"] = key;
            ...
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"Ritual LoadHolon failed: {ex.Message}", ex); }
            return r;
        }
        public override async Task<OASISResult<IHolon>> SaveHolonAsync(IHolon h, bool sc = true, bool rec = true, int md = 0, bool coe = true, bool scop = false)
        {
            ...
            try
            {
                ...
                var body = new
                {
                    containers = new[] { h.MetaData.ContainsKey("container") ? h.MetaData["container"] : "default" },
                    data = h.MetaData.ContainsKey("input") ? JsonConvert.DeserializeObject(h.MetaData["input"]?.ToString() ?? "{}") : new { }
                };
                var json = await PostAsync("api/jobs", body);
                h.ProviderUniqueStorageKey[Core.Enums.ProviderType.RitualOASIS] = json["id"]?.ToString() ?? h.Id.ToString();
                r.Result = h;
            }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"Ritual SaveHolon failed: {ex.Message}", ex); }
            return r;
        }
