...
namespace NextGenSoftware.OASIS.API.Providers.ReadyPlayerMeOASIS
{
    public class ReadyPlayerMeOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISNETProvider, IOASISBlockchainStorageProvider
    {
        ...
        public ReadyPlayerMeOASIS(string apiKey = "", string appId = "demo")
        {
            ...
            ProviderName = "ReadyPlayerMeOASIS"; ProviderDescription = "Ready Player Me cross-app 3D avatar creation and management provider.";
        ...
        public override async Task<OASISResult<bool>> ActivateProviderAsync()
        {
            ...
            try { await GetAsync("avatars?limit=1"); _isActivated = true; r.Result = true; r.Message = "ReadyPlayerMeOASIS activated"; }
        ...
        public override async Task<OASISResult<IHolon>> LoadHolonAsync(string key, bool lc = true, bool rec = true, int md = 0, bool coe = true, bool lcfp = false, int v = 0)
        {
            ...
            try
            {
                ...
                var data = json["data"] ?? json;
                var holon = new Holon { Name = $"ReadyPlayerMe Avatar {key}" };
                holon.ProviderUniqueStorageKey[Core.Enums.ProviderType.ReadyPlayerMeOASIS] = key;
                holon.MetaData["avatar_id"] = key;
                holon.MetaData["model_url"] = data["modelUrl"]?.ToString() ?? $"https://models.readyplayer.me/{key}.glb";
        ...
        public override async Task<OASISResult<IHolon>> SaveHolonAsync(IHolon h, bool sc = true, bool rec = true, int md = 0, bool coe = true, bool scop = false)
        {
            ...
            try
            {
                ...
                var body = new { partner = _appId, gender = h.MetaData.ContainsKey("gender") ? h.MetaData["gender"] : "neutral" };
                var json = await PostAsync("users", body);
                var userId = json["data"]?["id"]?.ToString() ?? h.Id.ToString();
                h.ProviderUniqueStorageKey[Core.Enums.ProviderType.ReadyPlayerMeOASIS] = userId;
                r.Result = h;
            }
