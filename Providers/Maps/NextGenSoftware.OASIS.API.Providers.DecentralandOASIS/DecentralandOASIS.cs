...
namespace NextGenSoftware.OASIS.API.Providers.DecentralandOASIS
{
    public class DecentralandOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISNETProvider, IOASISBlockchainStorageProvider
    {
        ...
        private async Task<JObject> GetCatalystAsync(string path) { var r = await _catalyst.GetAsync(path); r.EnsureSuccessStatusCode(); return JObject.Parse(await r.Content.ReadAsStringAsync()); }
        private async Task<JObject> GetContentAsync(string path) { var r = await _content.GetAsync(path); r.EnsureSuccessStatusCode(); return JObject.Parse(await r.Content.ReadAsStringAsync()); }
