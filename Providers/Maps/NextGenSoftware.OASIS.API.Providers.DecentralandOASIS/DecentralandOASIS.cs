...
namespace NextGenSoftware.OASIS.API.Providers.DecentralandOASIS
{
    ...
    public class DecentralandOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISNETProvider, IOASISBlockchainStorageProvider
    {
        ...
        public DecentralandOASIS(string apiUrl = "https://api.decentraland.org/v1")
        {
            _apiUrl = apiUrl?.TrimEnd('/') ?? "https://api.decentraland.org/v1";
            _http = new HttpClient { BaseAddress = new Uri(_apiUrl + "/") };
            ProviderName = "DecentralandOASIS";
            ProviderDescription = "Decentraland Metaverse World Integration Provider";
        ...
        public override async Task<OASISResult<bool>> ActivateProviderAsync()
        {
            ...
            try { if (_isActivated) { r.Result = true; r.Message = "DecentralandOASIS already activated"; return r; } _isActivated = true; r.Result = true; r.Message = "DecentralandOASIS activated successfully"; }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"DecentralandOASIS activation failed: {ex.Message}", ex); }
            return r;
        }
        ...
        {
            var r = new OASISResult<bool>();
            try { _isActivated = false; _http.Dispose(); r.Result = true; r.Message = "DecentralandOASIS deactivated"; }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"DecentralandOASIS deactivation failed: {ex.Message}", ex); }
            return r;
        }

        public override async Task<OASISResult<IAvatar>> LoadAvatarAsync(Guid id, int version = 0) { var r = new OASISResult<IAvatar>(); try { r.Result = new Avatar { Id = id }; } catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"DecentralandOASIS LoadAvatarAsync: {ex.Message}", ex); } return r; }
        public override async Task<OASISResult<IAvatar>> LoadAvatarByUsernameAsync(string u, int v = 0) { var r = new OASISResult<IAvatar>(); try { r.Result = new Avatar { Username = u }; } catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"DecentralandOASIS LoadAvatarByUsernameAsync: {ex.Message}", ex); } return r; }
        public override async Task<OASISResult<IAvatar>> SaveAvatarAsync(IAvatar a) { var r = new OASISResult<IAvatar>(); try { if (a.Id == Guid.Empty) a.Id = Guid.NewGuid(); r.Result = a; } catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"DecentralandOASIS SaveAvatarAsync: {ex.Message}", ex); } return r; }
        ...
        public override async Task<OASISResult<IAvatarDetail>> LoadAvatarDetailAsync(Guid id, int v = 0) { var r = new OASISResult<IAvatarDetail>(); OASISErrorHandling.HandleError(ref r, "DecentralandOASIS does not support avatar detail storage"); return r; }
        public override async Task<OASISResult<IAvatarDetail>> SaveAvatarDetailAsync(IAvatarDetail ad) { var r = new OASISResult<IAvatarDetail>(); OASISErrorHandling.HandleError(ref r, "DecentralandOASIS does not support avatar detail storage"); return r; }
