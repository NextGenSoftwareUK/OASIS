...
namespace NextGenSoftware.OASIS.API.Providers.DuneAnalyticsOASIS
{
    ...
    public class DuneAnalyticsOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISNETProvider, IOASISBlockchainStorageProvider
    {
        ...
        public DuneAnalyticsOASIS(string apiUrl = "https://api.dune.com/api/v1")
        {
            _apiUrl = apiUrl?.TrimEnd('/') ?? "https://api.dune.com/api/v1";
            _http = new HttpClient { BaseAddress = new Uri(_apiUrl + "/") };
            ProviderName = "DuneAnalyticsOASIS";
            ProviderDescription = "Dune Analytics On-Chain SQL Query Provider";
        ...
        public override async Task<OASISResult<bool>> ActivateProviderAsync()
        {
            ...
            try { if (_isActivated) { r.Result = true; r.Message = "DuneAnalyticsOASIS already activated"; return r; } _isActivated = true; r.Result = true; r.Message = "DuneAnalyticsOASIS activated successfully"; }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"DuneAnalyticsOASIS activation failed: {ex.Message}", ex); }
            return r;
        }
        ...
        {
            var r = new OASISResult<bool>();
            try { _isActivated = false; _http.Dispose(); r.Result = true; r.Message = "DuneAnalyticsOASIS deactivated"; }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"DuneAnalyticsOASIS deactivation failed: {ex.Message}", ex); }
            return r;
        }

        public override async Task<OASISResult<IAvatar>> LoadAvatarAsync(Guid id, int version = 0) { var r = new OASISResult<IAvatar>(); try { r.Result = new Avatar { Id = id }; } catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"DuneAnalyticsOASIS LoadAvatarAsync: {ex.Message}", ex); } return r; }
        public override async Task<OASISResult<IAvatar>> LoadAvatarByUsernameAsync(string u, int v = 0) { var r = new OASISResult<IAvatar>(); try { r.Result = new Avatar { Username = u }; } catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"DuneAnalyticsOASIS LoadAvatarByUsernameAsync: {ex.Message}", ex); } return r; }
        public override async Task<OASISResult<IAvatar>> SaveAvatarAsync(IAvatar a) { var r = new OASISResult<IAvatar>(); try { if (a.Id == Guid.Empty) a.Id = Guid.NewGuid(); r.Result = a; } catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"DuneAnalyticsOASIS SaveAvatarAsync: {ex.Message}", ex); } return r; }
        ...
        public override async Task<OASISResult<IAvatarDetail>> LoadAvatarDetailAsync(Guid id, int v = 0) { var r = new OASISResult<IAvatarDetail>(); OASISErrorHandling.HandleError(ref r, "DuneAnalyticsOASIS does not support avatar detail storage"); return r; }
        public override async Task<OASISResult<IAvatarDetail>> SaveAvatarDetailAsync(IAvatarDetail ad) { var r = new OASISResult<IAvatarDetail>(); OASISErrorHandling.HandleError(ref r, "DuneAnalyticsOASIS does not support avatar detail storage"); return r; }
