...
namespace NextGenSoftware.OASIS.API.Providers.MoralisStreamsOASIS
{
    public class MoralisStreamsOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISNETProvider, IOASISBlockchainStorageProvider
    {
        ...
        public MoralisStreamsOASIS(string apiKey = "")
        {
            ...
            ProviderName = "MoralisStreamsOASIS"; ProviderDescription = "Moralis Streams real-time blockchain event webhooks provider.";
        ...
        private async Task<JObject> PutAsync(string path, object body) { var resp = await _http.PutAsync(path, new StringContent(JsonConvert.SerializeObject(body), Encoding.UTF8, "application/json")); resp.EnsureSuccessStatusCode(); return JObject.Parse(await resp.Content.ReadAsStringAsync()); }
        public override async Task<OASISResult<bool>> ActivateProviderAsync()
        {
            var r = new OASISResult<bool>();
            try { var res = await GetAsync("beta/streams?limit=5"); _isActivated = res["result"] != null; r.Result = _isActivated; r.Message = "MoralisStreamsOASIS activated"; }
        ...
        public override async Task<OASISResult<IAvatar>> LoadAvatarByProviderKeyAsync(string address, int v = 0)
        {
            ...
            try
            {
                // List streams and find ones watching this address
                var json = await GetAsync("beta/streams?limit=20");
                ...
                avatar.MetaData["streams"] = json["result"]?.ToString(Formatting.None) ?? "[]";
                r.Result = avatar;
            }
        ...
        public override async Task<OASISResult<IHolon>> LoadHolonAsync(string key, bool lc = true, bool rec = true, int md = 0, bool coe = true, bool lcfp = false, int v = 0)
        {
            ...
            try
            {
                var json = await GetAsync($"beta/streams/{key}");
                var holon = new Holon { Name = json["tag"]?.ToString() ?? key };
                holon.ProviderUniqueStorageKey[Core.Enums.ProviderType.MoralisStreamsOASIS] = key;
                holon.MetaData["stream_id"] = json["id"]?.ToString() ?? key;
                holon.MetaData["status"] = json["status"]?.ToString() ?? "";
                holon.MetaData["webhook_url"] = json["webhookUrl"]?.ToString() ?? "";
                holon.MetaData["chains"] = json["chainIds"]?.ToString(Formatting.None) ?? "[]";
                r.Result = holon;
            }
        ...
        public override async Task<OASISResult<IHolon>> SaveHolonAsync(IHolon h, bool sc = true, bool rec = true, int md = 0, bool coe = true, bool scop = false)
        {
            ...
            try
            {
                ...
                var existingId = h.ProviderUniqueStorageKey.ContainsKey(Core.Enums.ProviderType.MoralisStreamsOASIS) ? h.ProviderUniqueStorageKey[Core.Enums.ProviderType.MoralisStreamsOASIS] : null;
                var body = new { tag = h.Name, webhookUrl = h.MetaData.ContainsKey("webhook_url") ? h.MetaData["webhook_url"] : "https://example.com/webhook", description = h.Description ?? h.Name, chainIds = new[] { "0x1" }, includeNativeTxs = true };
                JObject json;
                if (!string.IsNullOrEmpty(existingId))
                    json = await PutAsync($"beta/streams/{existingId}", body);
                else
                    json = await PostAsync("beta/streams", body);
                h.ProviderUniqueStorageKey[Core.Enums.ProviderType.MoralisStreamsOASIS] = json["id"]?.ToString() ?? existingId ?? "";
                r.Result = h;
            }
