...
namespace NextGenSoftware.OASIS.API.Providers.ZapperOASIS
{
    ...
    public class ZapperOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISNETProvider, IOASISBlockchainStorageProvider
    {
        private readonly HttpClient _http;
        private readonly string _apiUrl;
        private bool _isActivated;

        public ZapperOASIS(string apiUrl = "https://api.zapper.xyz/v2")
        {
            _apiUrl = apiUrl?.TrimEnd('/') ?? "https://api.zapper.xyz/v2";
            _http = new HttpClient { BaseAddress = new Uri(_apiUrl + "/") };
            ProviderName = "ZapperOASIS";
            ProviderDescription = "Zapper DeFi Portfolio and Position Data Provider";
        ...
        public override async Task<OASISResult<bool>> ActivateProviderAsync()
        {
            ...
            try { if (_isActivated) { r.Result = true; r.Message = "ZapperOASIS already activated"; return r; } _isActivated = true; r.Result = true; r.Message = "ZapperOASIS activated successfully"; }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"ZapperOASIS activation failed: {ex.Message}", ex); }
            return r;
        }
        ...
        {
            var r = new OASISResult<bool>();
            try { _isActivated = false; _http.Dispose(); r.Result = true; r.Message = "ZapperOASIS deactivated"; }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"ZapperOASIS deactivation failed: {ex.Message}", ex); }
            return r;
        }

        public override async Task<OASISResult<IAvatar>> LoadAvatarAsync(Guid id, int version = 0) { var r = new OASISResult<IAvatar>(); try { r.Result = new Avatar { Id = id }; } catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"ZapperOASIS LoadAvatarAsync: {ex.Message}", ex); } return r; }
        public override async Task<OASISResult<IAvatar>> LoadAvatarByUsernameAsync(string u, int v = 0) { var r = new OASISResult<IAvatar>(); try { r.Result = new Avatar { Username = u }; } catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"ZapperOASIS LoadAvatarByUsernameAsync: {ex.Message}", ex); } return r; }
        public override async Task<OASISResult<IAvatar>> SaveAvatarAsync(IAvatar a) { var r = new OASISResult<IAvatar>(); try { if (a.Id == Guid.Empty) a.Id = Guid.NewGuid(); r.Result = a; } catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"ZapperOASIS SaveAvatarAsync: {ex.Message}", ex); } return r; }
        ...
        public override async Task<OASISResult<IAvatarDetail>> LoadAvatarDetailAsync(Guid id, int v = 0) { var r = new OASISResult<IAvatarDetail>(); OASISErrorHandling.HandleError(ref r, "ZapperOASIS does not support avatar detail storage"); return r; }
        public override async Task<OASISResult<IAvatarDetail>> SaveAvatarDetailAsync(IAvatarDetail ad) { var r = new OASISResult<IAvatarDetail>(); OASISErrorHandling.HandleError(ref r, "ZapperOASIS does not support avatar detail storage"); return r; }
