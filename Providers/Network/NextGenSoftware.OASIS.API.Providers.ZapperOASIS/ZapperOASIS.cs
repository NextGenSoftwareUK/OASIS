...
namespace NextGenSoftware.OASIS.API.Providers.ZapperOASIS
{
    public class ZapperOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISNETProvider, IOASISBlockchainStorageProvider
    {
        ...
        public override async Task<OASISResult<bool>> ActivateProviderAsync()
        {
            ...
            try { await GetAsync("balances?addresses[]=0x0000000000000000000000000000000000000001"); _isActivated = true; r.Result = true; r.Message = "ZapperOASIS activated"; }
        ...
        public override async Task<OASISResult<IAvatar>> LoadAvatarByProviderKeyAsync(string address, int v = 0)
        {
            ...
            try
            {
                var json = await GetAsync($"balances?addresses[]={address}");
                var avatar = new Avatar { Username = address };
                avatar.ProviderUniqueStorageKey[Core.Enums.ProviderType.ZapperOASIS] = address;
                // Parse portfolio totals from balances response
                var appBalances = json["balances"]?[address];
                double totalUsd = 0;
                if (appBalances is JObject apps)
                    foreach (var app in apps) { var appTotal = app.Value?["meta"]?.First?["value"]?.Value<double>() ?? 0; totalUsd += appTotal; }
                avatar.MetaData["karma_score"] = ((int)(totalUsd / 100)).ToString();
                avatar.MetaData["total_usd"] = totalUsd.ToString("F2");
        ...
        public override async Task<OASISResult<IHolon>> LoadHolonAsync(string key, bool lc = true, bool rec = true, int md = 0, bool coe = true, bool lcfp = false, int v = 0)
        {
            ...
            try
            {
                // key = address → fetch NFT holdings
                var json = await GetAsync($"nft/user/tokens?userAddresses={key}&limit=50");
                var holon = new Holon { Name = $"Zapper NFTs for {key[..8]}..." };
                holon.ProviderUniqueStorageKey[Core.Enums.ProviderType.ZapperOASIS] = key;
                holon.MetaData["address"] = key;
                holon.MetaData["nfts"] = json["items"]?.ToString(Formatting.None) ?? "[]";
                r.Result = holon;
            }
        ...
        { if (h.Id == Guid.Empty) h.Id = Guid.NewGuid(); return new OASISResult<IHolon> { Result = h, Message = "Zapper is a read-only data API; holon not persisted externally." }; }
