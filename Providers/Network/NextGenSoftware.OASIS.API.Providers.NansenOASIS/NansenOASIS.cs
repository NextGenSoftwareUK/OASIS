...
namespace NextGenSoftware.OASIS.API.Providers.NansenOASIS
{
    public class NansenOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISNETProvider, IOASISBlockchainStorageProvider
    {
        ...
        public NansenOASIS(string apiKey = "")
        {
            ...
            if (!string.IsNullOrEmpty(apiKey)) _http.DefaultRequestHeaders.Add("apiKey", apiKey);
            ProviderName = "NansenOASIS"; ProviderDescription = "Nansen on-chain analytics and wallet intelligence provider.";
        ...
        public override async Task<OASISResult<bool>> ActivateProviderAsync()
        {
            ...
            try { await GetAsync("wallet/0x0000000000000000000000000000000000000001"); _isActivated = true; r.Result = true; r.Message = "NansenOASIS activated"; }
        ...
        public override async Task<OASISResult<IAvatar>> LoadAvatarByProviderKeyAsync(string address, int v = 0)
        {
            ...
            try
            {
                var json = await GetAsync($"wallet/{address}");
                ...
                avatar.MetaData["labels"] = json["labels"]?.ToString(Formatting.None) ?? "[]";
                avatar.MetaData["entity"] = json["entity"]?.ToString() ?? "";
                avatar.MetaData["categories"] = json["categories"]?.ToString(Formatting.None) ?? "[]";
                var portfolio = await GetAsync($"wallet/{address}/portfolio");
                avatar.MetaData["portfolio_usd"] = portfolio["total_usd"]?.ToString() ?? "0";
                r.Result = avatar;
            }
        ...
        public override async Task<OASISResult<IHolon>> LoadHolonAsync(string key, bool lc = true, bool rec = true, int md = 0, bool coe = true, bool lcfp = false, int v = 0)
        {
            ...
            try
            {
                var json = await GetAsync($"wallet/{key}/transactions");
                var holon = new Holon { Name = $"Nansen Wallet {key[..8]}..." };
                holon.ProviderUniqueStorageKey[Core.Enums.ProviderType.NansenOASIS] = key;
                holon.MetaData["transactions"] = json["transactions"]?.ToString(Formatting.None) ?? "[]";
                r.Result = holon;
            }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"Nansen LoadHolon failed: {ex.Message}", ex); }
            return r;
        }
        public override async Task<OASISResult<IHolon>> SaveHolonAsync(IHolon h, bool sc = true, bool rec = true, int md = 0, bool coe = true, bool scop = false) { if (h.Id == Guid.Empty) h.Id = Guid.NewGuid(); return new OASISResult<IHolon> { Result = h, Message = "Nansen is a read-only analytics API." }; }
