...
namespace NextGenSoftware.OASIS.API.Providers.SubqueryOASIS
{
    /// <summary>LayerZero V2 — Omnichain messaging protocol across 50+ networks with ultra-light node verification.</summary>
    public class SubqueryOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISNETProvider, IOASISBlockchainStorageProvider
    {
        private readonly HttpClient _http;
        private readonly string _apiUrl;
        private bool _isActivated;

        public SubqueryOASIS(string apiUrl = "https://api.subquery.network/v1")
        {
            _apiUrl = apiUrl?.TrimEnd('/') ?? "https://api.subquery.network/v1";
            _http = new HttpClient { BaseAddress = new Uri(_apiUrl + "/") };
            ProviderName = "SubqueryOASIS";
            ProviderDescription = "SubQuery Blockchain Data Indexing and Querying Provider";
            ProviderType = new EnumValue<ProviderType>(Core.Enums.ProviderType.SubqueryOASIS);
        ...
        public override async Task<OASISResult<bool>> ActivateProviderAsync()
        {
            ...
            try { if (_isActivated) { r.Result = true; r.Message = "SubqueryOASIS already activated"; return r; } _isActivated = true; r.Result = true; r.Message = "SubqueryOASIS activated successfully"; }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"SubqueryOASIS activation failed: {ex.Message}", ex); }
            return r;
        }
        ...
        {
            var r = new OASISResult<bool>();
            try { _isActivated = false; _http.Dispose(); r.Result = true; r.Message = "SubqueryOASIS deactivated"; }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"SubqueryOASIS deactivation failed: {ex.Message}", ex); }
            return r;
        }

        public override async Task<OASISResult<IAvatar>> LoadAvatarAsync(Guid id, int version = 0) { var r = new OASISResult<IAvatar>(); try { r.Result = new Avatar { Id = id }; } catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"SubqueryOASIS LoadAvatarAsync: {ex.Message}", ex); } return r; }
        public override async Task<OASISResult<IAvatar>> LoadAvatarByUsernameAsync(string u, int v = 0) { var r = new OASISResult<IAvatar>(); try { r.Result = new Avatar { Username = u }; } catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"SubqueryOASIS LoadAvatarByUsernameAsync: {ex.Message}", ex); } return r; }
        public override async Task<OASISResult<IAvatar>> SaveAvatarAsync(IAvatar a) { var r = new OASISResult<IAvatar>(); try { if (a.Id == Guid.Empty) a.Id = Guid.NewGuid(); r.Result = a; } catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"SubqueryOASIS SaveAvatarAsync: {ex.Message}", ex); } return r; }
        ...
        public override async Task<OASISResult<IAvatarDetail>> LoadAvatarDetailAsync(Guid id, int v = 0) { var r = new OASISResult<IAvatarDetail>(); OASISErrorHandling.HandleError(ref r, "SubqueryOASIS does not support avatar detail storage"); return r; }
        public override async Task<OASISResult<IAvatarDetail>> SaveAvatarDetailAsync(IAvatarDetail ad) { var r = new OASISResult<IAvatarDetail>(); OASISErrorHandling.HandleError(ref r, "SubqueryOASIS does not support avatar detail storage"); return r; }
