using System;
using NextGenSoftware.OASIS.API.Providers.TablelandOASIS;

namespace NextGenSoftware.OASIS.API.Providers.TablelandOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named TABLELANDOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class TablelandOASISTestFactory
    {
        public static TablelandOASIS Create()
        {
            var type = typeof(TablelandOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"TABLELANDOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (TablelandOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(TablelandOASIS).Name} - no usable constructor.");
        }
