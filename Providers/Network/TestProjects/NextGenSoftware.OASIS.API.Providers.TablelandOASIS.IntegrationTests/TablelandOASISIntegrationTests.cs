...
namespace NextGenSoftware.OASIS.API.Providers.TablelandOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from TABLELANDOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class TablelandOASISIntegrationTests
    {
        private TablelandOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = TablelandOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live TablelandOASIS backend configured - set the TABLELANDOASIS_* environment variables to run this.");
        }
