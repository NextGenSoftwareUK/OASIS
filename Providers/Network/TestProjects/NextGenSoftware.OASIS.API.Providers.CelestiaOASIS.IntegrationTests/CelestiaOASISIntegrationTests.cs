...
namespace NextGenSoftware.OASIS.API.Providers.CelestiaOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from CELESTIAOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class CelestiaOASISIntegrationTests
    {
        private CelestiaOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = CelestiaOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live CelestiaOASIS backend configured - set the CELESTIAOASIS_* environment variables to run this.");
        }
