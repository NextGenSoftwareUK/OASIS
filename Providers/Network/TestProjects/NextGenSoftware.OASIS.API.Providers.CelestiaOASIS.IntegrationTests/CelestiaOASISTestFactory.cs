using System;
using NextGenSoftware.OASIS.API.Providers.CelestiaOASIS;

namespace NextGenSoftware.OASIS.API.Providers.CelestiaOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named CELESTIAOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class CelestiaOASISTestFactory
    {
        public static CelestiaOASIS Create()
        {
            var type = typeof(CelestiaOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"CELESTIAOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (CelestiaOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(CelestiaOASIS).Name} - no usable constructor.");
        }
