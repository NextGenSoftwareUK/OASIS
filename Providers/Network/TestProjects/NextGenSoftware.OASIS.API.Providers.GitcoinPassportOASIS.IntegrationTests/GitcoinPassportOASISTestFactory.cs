using System;
using NextGenSoftware.OASIS.API.Providers.GitcoinPassportOASIS;

namespace NextGenSoftware.OASIS.API.Providers.GitcoinPassportOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named GITCOINPASSPORTOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class GitcoinPassportOASISTestFactory
    {
        public static GitcoinPassportOASIS Create()
        {
            var type = typeof(GitcoinPassportOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"GITCOINPASSPORTOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (GitcoinPassportOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(GitcoinPassportOASIS).Name} - no usable constructor.");
        }
