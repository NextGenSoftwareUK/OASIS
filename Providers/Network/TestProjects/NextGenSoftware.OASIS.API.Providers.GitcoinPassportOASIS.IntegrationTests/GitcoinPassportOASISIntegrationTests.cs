...
namespace NextGenSoftware.OASIS.API.Providers.GitcoinPassportOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from GITCOINPASSPORTOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class GitcoinPassportOASISIntegrationTests
    {
        private GitcoinPassportOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = GitcoinPassportOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live GitcoinPassportOASIS backend configured - set the GITCOINPASSPORTOASIS_* environment variables to run this.");
        }
