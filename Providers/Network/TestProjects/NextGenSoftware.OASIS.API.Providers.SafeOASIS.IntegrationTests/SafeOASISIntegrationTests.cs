...
namespace NextGenSoftware.OASIS.API.Providers.SafeOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from SAFEOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class SafeOASISIntegrationTests
    {
        private SafeOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = SafeOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live SafeOASIS backend configured - set the SAFEOASIS_* environment variables to run this.");
        }
