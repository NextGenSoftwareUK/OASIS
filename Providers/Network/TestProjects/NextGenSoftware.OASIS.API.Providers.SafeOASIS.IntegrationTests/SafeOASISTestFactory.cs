using System;
using NextGenSoftware.OASIS.API.Providers.SafeOASIS;

namespace NextGenSoftware.OASIS.API.Providers.SafeOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named SAFEOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class SafeOASISTestFactory
    {
        public static SafeOASIS Create()
        {
            var type = typeof(SafeOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"SAFEOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (SafeOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(SafeOASIS).Name} - no usable constructor.");
        }
