...
namespace NextGenSoftware.OASIS.API.Providers.PrivyOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from PRIVYOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class PrivyOASISIntegrationTests
    {
        private PrivyOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = PrivyOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live PrivyOASIS backend configured - set the PRIVYOASIS_* environment variables to run this.");
        }
