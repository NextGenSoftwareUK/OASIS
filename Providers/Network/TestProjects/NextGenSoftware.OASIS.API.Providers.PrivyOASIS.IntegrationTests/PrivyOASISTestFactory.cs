using System;
using NextGenSoftware.OASIS.API.Providers.PrivyOASIS;

namespace NextGenSoftware.OASIS.API.Providers.PrivyOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named PRIVYOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class PrivyOASISTestFactory
    {
        public static PrivyOASIS Create()
        {
            var type = typeof(PrivyOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"PRIVYOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (PrivyOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(PrivyOASIS).Name} - no usable constructor.");
        }
