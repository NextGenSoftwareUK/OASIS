using System;
using NextGenSoftware.OASIS.API.Providers.LivepeerOASIS;

namespace NextGenSoftware.OASIS.API.Providers.LivepeerOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named LIVEPEEROASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class LivepeerOASISTestFactory
    {
        public static LivepeerOASIS Create()
        {
            var type = typeof(LivepeerOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"LIVEPEEROASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (LivepeerOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(LivepeerOASIS).Name} - no usable constructor.");
        }
