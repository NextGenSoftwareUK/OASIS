...
namespace NextGenSoftware.OASIS.API.Providers.LivepeerOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from LIVEPEEROASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class LivepeerOASISIntegrationTests
    {
        private LivepeerOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = LivepeerOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live LivepeerOASIS backend configured - set the LIVEPEEROASIS_* environment variables to run this.");
        }
