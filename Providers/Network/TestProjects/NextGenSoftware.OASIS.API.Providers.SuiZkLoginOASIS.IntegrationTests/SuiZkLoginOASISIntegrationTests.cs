...
namespace NextGenSoftware.OASIS.API.Providers.SuiZkLoginOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from SUIZKLOGINOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class SuiZkLoginOASISIntegrationTests
    {
        private SuiZkLoginOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = SuiZkLoginOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live SuiZkLoginOASIS backend configured - set the SUIZKLOGINOASIS_* environment variables to run this.");
        }
