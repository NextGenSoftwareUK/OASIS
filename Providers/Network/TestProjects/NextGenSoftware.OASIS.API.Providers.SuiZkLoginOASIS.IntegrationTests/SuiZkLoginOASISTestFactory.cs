using System;
using NextGenSoftware.OASIS.API.Providers.SuiZkLoginOASIS;

namespace NextGenSoftware.OASIS.API.Providers.SuiZkLoginOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named SUIZKLOGINOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class SuiZkLoginOASISTestFactory
    {
        public static SuiZkLoginOASIS Create()
        {
            var type = typeof(SuiZkLoginOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"SUIZKLOGINOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (SuiZkLoginOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(SuiZkLoginOASIS).Name} - no usable constructor.");
        }
