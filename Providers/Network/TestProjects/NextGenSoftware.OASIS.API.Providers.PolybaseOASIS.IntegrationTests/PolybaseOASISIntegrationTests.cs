...
namespace NextGenSoftware.OASIS.API.Providers.PolybaseOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from POLYBASEOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class PolybaseOASISIntegrationTests
    {
        private PolybaseOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = PolybaseOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live PolybaseOASIS backend configured - set the POLYBASEOASIS_* environment variables to run this.");
        }
