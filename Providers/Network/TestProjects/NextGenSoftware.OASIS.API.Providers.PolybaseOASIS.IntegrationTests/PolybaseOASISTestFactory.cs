using System;
using NextGenSoftware.OASIS.API.Providers.PolybaseOASIS;

namespace NextGenSoftware.OASIS.API.Providers.PolybaseOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named POLYBASEOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class PolybaseOASISTestFactory
    {
        public static PolybaseOASIS Create()
        {
            var type = typeof(PolybaseOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"POLYBASEOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (PolybaseOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(PolybaseOASIS).Name} - no usable constructor.");
        }
