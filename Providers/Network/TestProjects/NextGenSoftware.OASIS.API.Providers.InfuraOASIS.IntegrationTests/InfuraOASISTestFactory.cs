...
namespace NextGenSoftware.OASIS.API.Providers.InfuraOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named INFURAOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class InfuraOASISTestFactory
    {
        public static InfuraOASIS Create()
        {
            var type = typeof(InfuraOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"INFURAOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (InfuraOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(InfuraOASIS).Name} - no usable constructor.");
        }
