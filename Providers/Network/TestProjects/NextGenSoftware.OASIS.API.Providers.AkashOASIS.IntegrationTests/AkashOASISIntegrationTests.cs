...
namespace NextGenSoftware.OASIS.API.Providers.AkashOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from AKASHOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class AkashOASISIntegrationTests
    {
        private AkashOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = AkashOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live AkashOASIS backend configured - set the AKASHOASIS_* environment variables to run this.");
        }
