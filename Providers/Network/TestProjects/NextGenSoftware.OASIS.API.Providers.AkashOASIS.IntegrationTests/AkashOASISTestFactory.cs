using System;
using NextGenSoftware.OASIS.API.Providers.AkashOASIS;

namespace NextGenSoftware.OASIS.API.Providers.AkashOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named AKASHOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class AkashOASISTestFactory
    {
        public static AkashOASIS Create()
        {
            var type = typeof(AkashOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"AKASHOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (AkashOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(AkashOASIS).Name} - no usable constructor.");
        }
