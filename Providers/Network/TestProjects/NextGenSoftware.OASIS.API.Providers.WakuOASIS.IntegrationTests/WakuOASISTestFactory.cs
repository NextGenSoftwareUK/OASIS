using System;
using NextGenSoftware.OASIS.API.Providers.WakuOASIS;

namespace NextGenSoftware.OASIS.API.Providers.WakuOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named WAKUOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class WakuOASISTestFactory
    {
        public static WakuOASIS Create()
        {
            var type = typeof(WakuOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"WAKUOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (WakuOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(WakuOASIS).Name} - no usable constructor.");
        }
