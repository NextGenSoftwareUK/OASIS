...
namespace NextGenSoftware.OASIS.API.Providers.WakuOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from WAKUOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class WakuOASISIntegrationTests
    {
        private WakuOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = WakuOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live WakuOASIS backend configured - set the WAKUOASIS_* environment variables to run this.");
        }
