...
namespace NextGenSoftware.OASIS.API.Providers.MoralisOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from MORALISOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class MoralisOASISIntegrationTests
    {
        private MoralisOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = MoralisOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live MoralisOASIS backend configured - set the MORALISOASIS_* environment variables to run this.");
        }
