...
namespace NextGenSoftware.OASIS.API.Providers.LayerZeroOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from LAYERZEROOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class LayerZeroOASISIntegrationTests
    {
        private LayerZeroOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = LayerZeroOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live LayerZeroOASIS backend configured - set the LAYERZEROOASIS_* environment variables to run this.");
        }
