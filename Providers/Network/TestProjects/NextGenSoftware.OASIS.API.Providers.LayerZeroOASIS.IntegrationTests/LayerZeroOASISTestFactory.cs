using System;
using NextGenSoftware.OASIS.API.Providers.LayerZeroOASIS;

namespace NextGenSoftware.OASIS.API.Providers.LayerZeroOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named LAYERZEROOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class LayerZeroOASISTestFactory
    {
        public static LayerZeroOASIS Create()
        {
            var type = typeof(LayerZeroOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"LAYERZEROOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (LayerZeroOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(LayerZeroOASIS).Name} - no usable constructor.");
        }
