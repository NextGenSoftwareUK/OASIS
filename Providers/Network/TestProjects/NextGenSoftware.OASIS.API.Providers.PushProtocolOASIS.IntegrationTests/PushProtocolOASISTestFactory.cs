using System;
using NextGenSoftware.OASIS.API.Providers.PushProtocolOASIS;

namespace NextGenSoftware.OASIS.API.Providers.PushProtocolOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named PUSHPROTOCOLOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class PushProtocolOASISTestFactory
    {
        public static PushProtocolOASIS Create()
        {
            var type = typeof(PushProtocolOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"PUSHPROTOCOLOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (PushProtocolOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(PushProtocolOASIS).Name} - no usable constructor.");
        }
