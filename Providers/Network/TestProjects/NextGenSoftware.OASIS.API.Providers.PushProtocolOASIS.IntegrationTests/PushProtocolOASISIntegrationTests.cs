...
namespace NextGenSoftware.OASIS.API.Providers.PushProtocolOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from PUSHPROTOCOLOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class PushProtocolOASISIntegrationTests
    {
        private PushProtocolOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = PushProtocolOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live PushProtocolOASIS backend configured - set the PUSHPROTOCOLOASIS_* environment variables to run this.");
        }
