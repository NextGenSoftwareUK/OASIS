...
namespace NextGenSoftware.OASIS.API.Providers.MoralisOASIS.TestHarness
{
    /// <summary>
    /// Console harness that walks the full CRUD surface against a live backend.
    /// Configure with MORALISOASIS_&lt;PARAM&gt; environment variables.
    /// </summary>
    public static class MoralisOASISTestHarness
    {
        public static async Task Main()
        {
            Console.WriteLine("MoralisOASIS Test Harness");
            Console.WriteLine(new string('-', 60));

            var provider = MoralisOASISTestFactory.Create();
            ...
            {
                Console.WriteLine($"\nSet the MORALISOASIS_* environment variables and try again.");
                return;
            }
