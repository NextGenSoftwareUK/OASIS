...
namespace NextGenSoftware.OASIS.API.Providers.EclipseOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from ECLIPSEOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class EclipseOASISIntegrationTests
    {
        private EclipseOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = EclipseOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live EclipseOASIS backend configured - set the ECLIPSEOASIS_* environment variables to run this.");
        }
