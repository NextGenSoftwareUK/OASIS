using System;
using NextGenSoftware.OASIS.API.Providers.EclipseOASIS;

namespace NextGenSoftware.OASIS.API.Providers.EclipseOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named ECLIPSEOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class EclipseOASISTestFactory
    {
        public static EclipseOASIS Create()
        {
            var type = typeof(EclipseOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"ECLIPSEOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (EclipseOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(EclipseOASIS).Name} - no usable constructor.");
        }
