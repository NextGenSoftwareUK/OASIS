...
namespace NextGenSoftware.OASIS.API.Providers.TelegramOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from TELEGRAMOASISPROVIDER_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class TelegramOASISProviderIntegrationTests
    {
        ...
        public void Setup()
        {
            _provider = TelegramOASISProviderTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live TelegramOASISProvider backend configured - set the TELEGRAMOASISPROVIDER_* environment variables to run this.");
        }
