...
namespace NextGenSoftware.OASIS.API.Providers.TelegramOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named TELEGRAMOASISPROVIDER_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class TelegramOASISProviderTestFactory
    {
        public static TelegramOASISProvider Create()
        {
            var type = typeof(TelegramOASISProvider);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"TELEGRAMOASISPROVIDER_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (TelegramOASISProvider)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(TelegramOASISProvider).Name} - no usable constructor.");
        }
