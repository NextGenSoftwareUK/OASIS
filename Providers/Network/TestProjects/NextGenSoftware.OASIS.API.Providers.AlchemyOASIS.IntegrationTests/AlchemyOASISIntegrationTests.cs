...
namespace NextGenSoftware.OASIS.API.Providers.AlchemyOASIS.IntegrationTests
{
    /// <summary>
    /// Exercises the real CRUD surface against a live backend.
    ///
    /// Connection details come from ALCHEMYOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class AlchemyOASISIntegrationTests
    {
        private AlchemyOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = AlchemyOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live AlchemyOASIS backend configured - set the ALCHEMYOASIS_* environment variables to run this.");
        }
