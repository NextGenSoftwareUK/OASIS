using System;
using NextGenSoftware.OASIS.API.Providers.ZkSyncSSOOASIS;

namespace NextGenSoftware.OASIS.API.Providers.ZkSyncSSOOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named ZKSYNCSSOOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class ZkSyncSSOOASISTestFactory
    {
        public static ZkSyncSSOOASIS Create()
        {
            var type = typeof(ZkSyncSSOOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"ZKSYNCSSOOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (ZkSyncSSOOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(ZkSyncSSOOASIS).Name} - no usable constructor.");
        }
