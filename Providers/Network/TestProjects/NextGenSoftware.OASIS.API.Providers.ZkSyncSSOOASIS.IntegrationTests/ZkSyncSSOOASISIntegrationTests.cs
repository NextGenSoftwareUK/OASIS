...
namespace NextGenSoftware.OASIS.API.Providers.ZkSyncSSOOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from ZKSYNCSSOOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class ZkSyncSSOOASISIntegrationTests
    {
        private ZkSyncSSOOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = ZkSyncSSOOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live ZkSyncSSOOASIS backend configured - set the ZKSYNCSSOOASIS_* environment variables to run this.");
        }
