using System;
using NextGenSoftware.OASIS.API.Providers.ENSOASIS;

namespace NextGenSoftware.OASIS.API.Providers.ENSOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named ENSOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class ENSOASISTestFactory
    {
        public static ENSOASIS Create()
        {
            var type = typeof(ENSOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"ENSOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (ENSOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(ENSOASIS).Name} - no usable constructor.");
        }
