...
namespace NextGenSoftware.OASIS.API.Providers.ENSOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from ENSOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class ENSOASISIntegrationTests
    {
        private ENSOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = ENSOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live ENSOASIS backend configured - set the ENSOASIS_* environment variables to run this.");
        }
