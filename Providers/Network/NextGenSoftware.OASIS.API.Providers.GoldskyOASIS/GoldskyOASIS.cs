...
namespace NextGenSoftware.OASIS.API.Providers.GoldskyOASIS
{
    public class GoldskyOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISNETProvider, IOASISBlockchainStorageProvider
    {
        ...
        public GoldskyOASIS(string projectId = "", string subgraphName = "oasis", string version = "latest")
        {
            var url = string.IsNullOrEmpty(projectId)
                ? "https://api.goldsky.com/api/public/project_default/subgraphs/oasis/latest/gn"
                : $"https://api.goldsky.com/api/public/{projectId}/subgraphs/{subgraphName}/{version}/gn";
            _http = new HttpClient { BaseAddress = new Uri(url) };
            ProviderName = "GoldskyOASIS"; ProviderDescription = "Goldsky real-time subgraph indexing provider.";
        ...
        {
            var body = variables == null ? new { query } : (object)new { query, variables };
        ...
        public override async Task<OASISResult<bool>> ActivateProviderAsync()
        {
            ...
            try { var res = await GraphQLAsync("{ __schema { queryType { name } } }"); _isActivated = res["data"] != null; r.Result = _isActivated; r.Message = "GoldskyOASIS activated"; }
        ...
        public override async Task<OASISResult<IAvatar>> LoadAvatarByProviderKeyAsync(string address, int v = 0)
        {
            ...
            try
            {
                var query = $"{{ account(id: \"{address.ToLower()}\") {{ id totalValueLockedUSD txCount }} }}";
                var res = await GraphQLAsync(query);
                var acct = res["data"]?["account"];
                var avatar = new Avatar { Username = address };
                avatar.ProviderUniqueStorageKey[Core.Enums.ProviderType.GoldskyOASIS] = address;
                avatar.MetaData["id"] = acct?["id"]?.ToString() ?? address;
                avatar.MetaData["txCount"] = acct?["txCount"]?.ToString() ?? "0";
                avatar.MetaData["tvl_usd"] = acct?["totalValueLockedUSD"]?.ToString() ?? "0";
                r.Result = avatar;
            }
        ...
        public override async Task<OASISResult<IHolon>> LoadHolonAsync(string key, bool lc = true, bool rec = true, int md = 0, bool coe = true, bool lcfp = false, int v = 0)
        {
            ...
            try
            {
                var query = $"{{ token(id: \"{key.ToLower()}\") {{ id symbol name decimals totalSupply }} }}";
                var res = await GraphQLAsync(query);
                var token = res["data"]?["token"];
                var holon = new Holon { Name = token?["name"]?.ToString() ?? key };
        ...
        public override async Task<OASISResult<IHolon>> SaveHolonAsync(IHolon h, bool sc = true, bool rec = true, int md = 0, bool coe = true, bool scop = false) { if (h.Id == Guid.Empty) h.Id = Guid.NewGuid(); return new OASISResult<IHolon> { Result = h, Message = "Goldsky is a read-only indexer." }; }
        ...
        public override async Task<OASISResult<ISearchResults>> SearchAsync(ISearchParams sp, bool lc = true, bool rec = true, int md = 0, bool coe = true, int v = 0)
        {
            ...
            try
            {
                ...
                var query = $"{{ tokens(where: {{ name_contains_nocase: \"{q}\" }}, first: 20) {{ id name symbol }} }}";
                var res = await GraphQLAsync(query);
                var holons = new List<IHolon>();
                foreach (var t in res["data"]?["tokens"] as JArray ?? new JArray())
                { var h = new Holon { Name = t["name"]?.ToString() ?? "" }; h.ProviderUniqueStorageKey[Core.Enums.ProviderType.GoldskyOASIS] = t["id"]?.ToString() ?? ""; holons.Add(h); }
