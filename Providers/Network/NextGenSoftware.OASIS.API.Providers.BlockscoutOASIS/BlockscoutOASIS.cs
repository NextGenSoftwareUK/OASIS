...
namespace NextGenSoftware.OASIS.API.Providers.BlockscoutOASIS
{
    public class BlockscoutOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISNETProvider, IOASISBlockchainStorageProvider
    {
        ...
        public BlockscoutOASIS(string baseUrl = "https://eth.blockscout.com/api/v2")
        {
            _http = new HttpClient { BaseAddress = new Uri(baseUrl.TrimEnd('/') + "/") };
            ProviderName = "BlockscoutOASIS";
            ProviderDescription = "Blockscout open-source block explorer REST API provider.";
        ...
        public override async Task<OASISResult<bool>> ActivateProviderAsync()
        {
            ...
            try { await GetAsync("stats"); _isActivated = true; r.Result = true; r.Message = "BlockscoutOASIS activated"; }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"Blockscout activation failed: {ex.Message}", ex); }
            return r;
        }
        ...
        public override async Task<OASISResult<IAvatar>> LoadAvatarByProviderKeyAsync(string address, int v = 0)
        {
            ...
            try
            {
                var json = await GetAsync($"addresses/{address}");
                var avatar = new Avatar { Username = address };
                avatar.ProviderUniqueStorageKey[Core.Enums.ProviderType.BlockscoutOASIS] = address;
                avatar.MetaData["balance"] = json["coin_balance"]?.ToString() ?? "0";
                avatar.MetaData["tx_count"] = json["transactions_count"]?.ToString() ?? "0";
                avatar.MetaData["is_contract"] = json["is_contract"]?.ToString() ?? "false";
                avatar.MetaData["name"] = json["name"]?.ToString() ?? "";
                r.Result = avatar;
            }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"Blockscout LoadAvatar failed: {ex.Message}", ex); }
            return r;
        }

        // Holon = tx hash or block number
        public override async Task<OASISResult<IHolon>> LoadHolonAsync(string key, bool lc = true, bool rec = true, int md = 0, bool coe = true, bool lcfp = false, int v = 0)
        {
            ...
            {
                var holon = new Holon();
                holon.ProviderUniqueStorageKey[Core.Enums.ProviderType.BlockscoutOASIS] = key;
                JObject json;
                if (key.StartsWith("0x") && key.Length == 66)
                {
                    json = await GetAsync($"transactions/{key}");
                    holon.Name = $"Tx {key[..10]}...";
                    holon.MetaData["hash"] = key;
                    holon.MetaData["from"] = json["from"]?["hash"]?.ToString() ?? "";
                    holon.MetaData["to"] = json["to"]?["hash"]?.ToString() ?? "";
                    holon.MetaData["value"] = json["value"]?.ToString() ?? "0";
                    holon.MetaData["gas_used"] = json["gas_used"]?.ToString() ?? "";
                    holon.MetaData["status"] = json["status"]?.ToString() ?? "";
                }
                else
                {
                    json = await GetAsync($"blocks/{key}");
                    holon.Name = $"Block {key}";
                    holon.MetaData["block_number"] = json["height"]?.ToString() ?? key;
                    holon.MetaData["hash"] = json["hash"]?.ToString() ?? "";
                    holon.MetaData["tx_count"] = json["tx_count"]?.ToString() ?? "0";
                    holon.MetaData["miner"] = json["miner"]?["hash"]?.ToString() ?? "";
                    holon.MetaData["timestamp"] = json["timestamp"]?.ToString() ?? "";
                }
                r.Result = holon;
            }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"Blockscout LoadHolon failed: {ex.Message}", ex); }
            return r;
        }
        ...
        { if (h.Id == Guid.Empty) h.Id = Guid.NewGuid(); return new OASISResult<IHolon> { Result = h, Message = "Blockscout is a read-only explorer; data not persisted externally." }; }
        ...
        public override async Task<OASISResult<ISearchResults>> SearchAsync(ISearchParams sp, bool lc = true, bool rec = true, int md = 0, bool coe = true, int v = 0) { var r = new OASISResult<ISearchResults>(); try { var json = await GetAsync($"search?q={Uri.EscapeDataString((sp?.FilterByMetaData?.ContainsKey("query") == true ? sp.FilterByMetaData["query"] : "") ?? "")}"); r.Result = new SearchResults(); } catch (Exception ex) { OASISErrorHandling.HandleError(ref r, ex.Message, ex); } return r; }
