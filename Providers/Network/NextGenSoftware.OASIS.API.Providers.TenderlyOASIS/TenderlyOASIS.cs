...
namespace NextGenSoftware.OASIS.API.Providers.TenderlyOASIS
{
    public class TenderlyOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISNETProvider, IOASISBlockchainStorageProvider
    {
        ...
        public TenderlyOASIS(string accessKey = "", string accountSlug = "")
        {
            _accountSlug = accountSlug;
            _http = new HttpClient { BaseAddress = new Uri("https://api.tenderly.co/api/v1/") };
            if (!string.IsNullOrEmpty(accessKey)) _http.DefaultRequestHeaders.Add("X-Access-Key", accessKey);
            ProviderName = "TenderlyOASIS"; ProviderDescription = "Tenderly smart contract simulation and monitoring provider.";
        ...
        public override async Task<OASISResult<bool>> ActivateProviderAsync()
        {
            ...
            try { var res = await GetAsync("user"); _isActivated = res["user"] != null; r.Result = _isActivated; r.Message = "TenderlyOASIS activated"; }
        ...
        public override async Task<OASISResult<IAvatar>> LoadAvatarByProviderKeyAsync(string address, int v = 0)
        {
            ...
            try
            {
                var prefix = string.IsNullOrEmpty(_accountSlug) ? "" : $"account/{_accountSlug}/";
                var json = await GetAsync($"{prefix}contracts?address={address}");
                var avatar = new Avatar { Username = address };
                avatar.ProviderUniqueStorageKey[Core.Enums.ProviderType.TenderlyOASIS] = address;
                avatar.MetaData["contracts"] = json["contracts"]?.ToString(Formatting.None) ?? "[]";
                r.Result = avatar;
            }
        ...
        public override async Task<OASISResult<IHolon>> LoadHolonAsync(string key, bool lc = true, bool rec = true, int md = 0, bool coe = true, bool lcfp = false, int v = 0)
        {
            ...
            try
            {
                // Simulate a transaction to get call trace
                var simBody = new { network_id = "1", from = "0x0000000000000000000000000000000000000000", to = key, gas = 21000, gas_price = "0", value = "0", input = "0x" };
                var json = await PostAsync("simulate", simBody);
                var tx = json["transaction"];
                var holon = new Holon { Name = $"Tenderly Sim {key[..Math.Min(8, key.Length)]}..." };
                holon.ProviderUniqueStorageKey[Core.Enums.ProviderType.TenderlyOASIS] = key;
                holon.MetaData["status"] = tx?["status"]?.ToString() ?? "";
                holon.MetaData["gas_used"] = tx?["gas_used"]?.ToString() ?? "0";
                r.Result = holon;
            }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"Tenderly LoadHolon failed: {ex.Message}", ex); }
            return r;
        }
        public override async Task<OASISResult<IHolon>> SaveHolonAsync(IHolon h, bool sc = true, bool rec = true, int md = 0, bool coe = true, bool scop = false) { if (h.Id == Guid.Empty) h.Id = Guid.NewGuid(); return new OASISResult<IHolon> { Result = h, Message = "TenderlyOASIS is a simulation/monitoring provider; data not persisted externally." }; }
