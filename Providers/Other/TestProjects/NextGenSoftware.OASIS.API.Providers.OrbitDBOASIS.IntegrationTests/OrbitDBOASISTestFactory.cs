...
namespace NextGenSoftware.OASIS.API.Providers.OrbitDBOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named ORBITDBOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class OrbitDBOASISTestFactory
    {
        public static OrbitDBOASIS Create()
        {
            var type = typeof(OrbitDBOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"ORBITDBOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (OrbitDBOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(OrbitDBOASIS).Name} - no usable constructor.");
        }
