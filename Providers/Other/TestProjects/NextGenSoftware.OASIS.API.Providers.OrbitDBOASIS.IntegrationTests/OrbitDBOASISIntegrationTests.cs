...
namespace NextGenSoftware.OASIS.API.Providers.OrbitDBOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from ORBITDBOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class OrbitDBOASISIntegrationTests
    {
        private OrbitDBOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = OrbitDBOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live OrbitDBOASIS backend configured - set the ORBITDBOASIS_* environment variables to run this.");
        }
