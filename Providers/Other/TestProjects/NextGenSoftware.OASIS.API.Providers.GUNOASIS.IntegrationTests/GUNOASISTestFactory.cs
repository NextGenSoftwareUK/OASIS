using System;
using NextGenSoftware.OASIS.API.Providers.GUNOASIS;

namespace NextGenSoftware.OASIS.API.Providers.GUNOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named GUNOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class GUNOASISTestFactory
    {
        public static GUNOASIS Create()
        {
            var type = typeof(GUNOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"GUNOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (GUNOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(GUNOASIS).Name} - no usable constructor.");
        }
