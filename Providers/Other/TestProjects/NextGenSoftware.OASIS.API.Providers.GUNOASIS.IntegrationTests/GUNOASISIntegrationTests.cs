...
namespace NextGenSoftware.OASIS.API.Providers.GUNOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from GUNOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class GUNOASISIntegrationTests
    {
        private GUNOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = GUNOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live GUNOASIS backend configured - set the GUNOASIS_* environment variables to run this.");
        }
