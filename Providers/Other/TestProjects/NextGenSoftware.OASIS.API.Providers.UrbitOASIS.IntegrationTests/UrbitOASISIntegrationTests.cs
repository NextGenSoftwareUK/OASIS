...
namespace NextGenSoftware.OASIS.API.Providers.UrbitOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from URBITOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class UrbitOASISIntegrationTests
    {
        private UrbitOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = UrbitOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live UrbitOASIS backend configured - set the URBITOASIS_* environment variables to run this.");
        }
