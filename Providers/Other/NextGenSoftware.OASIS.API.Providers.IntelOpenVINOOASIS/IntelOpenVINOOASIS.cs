...
namespace NextGenSoftware.OASIS.API.Providers.IntelOpenVINOOASIS
{
    ...
    public class IntelOpenVINOOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISNETProvider
    {
        private readonly HttpClient _http;
        private readonly string _temporalAddress;
        private bool _isActivated;

        public IntelOpenVINOOASIS(string temporalAddress = "http://localhost:9000")
        {
            _temporalAddress = temporalAddress?.TrimEnd('/') ?? "http://localhost:9000";
            _http = new HttpClient { BaseAddress = new Uri(_temporalAddress + "/") };
            ProviderName = "IntelOpenVINOOASIS";
            ProviderDescription = "Intel OpenVINO On-Premise Inference Provider";
