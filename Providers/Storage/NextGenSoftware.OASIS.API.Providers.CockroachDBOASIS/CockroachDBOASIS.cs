...
namespace NextGenSoftware.OASIS.API.Providers.CockroachDBOASIS
{
    /// <summary>
    /// OASIS provider for CockroachDB — distributed, cloud-native SQL via Npgsql ADO.NET.
    ///
    /// CockroachDB is PostgreSQL-wire-compatible, so this provider is essentially
    /// identical to PostgreSQLOASIS but named for discoverability and uses the
    /// CockroachDB-specific connection string format.
    ///
    /// Also compatible with: CockroachDB Serverless, CockroachDB Dedicated.
    ///
    /// Tables are auto-created on ActivateProvider.
    /// Schema:
    ///   oasis_avatars       — id UUID PK, username TEXT, email TEXT, is_deleted BOOL, data_json TEXT
    ///   oasis_avatar_details — id UUID PK, username TEXT, email TEXT, data_json TEXT
    ///   oasis_holons        — id UUID PK, parent_holon_id UUID, holon_type INT, is_deleted BOOL, data_json TEXT
    ///
    /// Constructor parameters:
    ///   connectionString — Npgsql connection string pointing to CockroachDB, e.g.:
    ///     "Host=free-tier.gcp-us-central1.cockroachlabs.cloud;Port=26257;Database=defaultdb;Username=user;Password=pass;SslMode=Require"
    /// </summary>
    public class CockroachDBOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISDBStorageProvider
    {
        ...
        public CockroachDBOASIS(string connectionString)
        {
        ...
        private async Task EnsureTablesAsync()
        {
            ...
            await Exec("CREATE TABLE IF NOT EXISTS oasis_avatars (id UUID PRIMARY KEY, username TEXT, email TEXT, is_deleted BOOL NOT NULL DEFAULT FALSE, data_json TEXT, INDEX (username), INDEX (email))");
            await Exec("CREATE TABLE IF NOT EXISTS oasis_avatar_details (id UUID PRIMARY KEY, username TEXT, email TEXT, data_json TEXT, INDEX (username), INDEX (email))");
            await Exec("CREATE TABLE IF NOT EXISTS oasis_holons (id UUID PRIMARY KEY, parent_holon_id UUID, holon_type INT DEFAULT 0, is_deleted BOOL NOT NULL DEFAULT FALSE, data_json TEXT, INDEX (parent_holon_id), INDEX (holon_type))");
        }

        public override async Task<OASISResult<bool>> ActivateProviderAsync()
        {
            var result = new OASISResult<bool>();
            try { await EnsureTablesAsync(); result.Result = true; result.IsError = false; result.Message = "CockroachDBOASIS activated."; }
            catch (Exception ex) { result.Exception = ex; OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: Error activating — {ex.Message}"); }
            return result;
        }

        public override OASISResult<bool> ActivateProvider() => ActivateProviderAsync().Result;
        public override async Task<OASISResult<bool>> DeActivateProviderAsync() => await Task.FromResult(new OASISResult<bool> { Result = true, IsError = false, Message = "CockroachDBOASIS deactivated." });
        ...
        public override async Task<OASISResult<IAvatar>> SaveAvatarAsync(IAvatar avatar)
        {
            ...
            try
            {
                ...
                avatar.ProviderUniqueStorageKey[Core.Enums.ProviderType.CockroachDBOASIS] = avatar.Id.ToString();
                await using var conn = await OpenAsync();
                await using var cmd = new NpgsqlCommand("INSERT INTO oasis_avatars(id,username,email,is_deleted,data_json) VALUES(@id,@username,@email,@del,@data) ON CONFLICT(id) DO UPDATE SET username=EXCLUDED.username,email=EXCLUDED.email,is_deleted=EXCLUDED.is_deleted,data_json=EXCLUDED.data_json", conn);
                ...
                cmd.Parameters.AddWithValue("del", avatar.IsDeleted);
                cmd.Parameters.AddWithValue("data", Ser(avatar));
                await cmd.ExecuteNonQueryAsync();
                result.Result = avatar; result.IsError = false; result.Message = $"CockroachDBOASIS: Avatar '{avatar.Username}' saved.";
            }
            catch (Exception ex) { result.Exception = ex; OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: Error saving avatar: {ex.Message}"); }
            return result;
        }
        ...
        private async Task<Avatar?> QueryAvatarAsync(string sql, Action<NpgsqlCommand> bind)
        {
            ...
            return Des<Avatar>(r.GetString(0));
        }
        ...
        public override async Task<OASISResult<IAvatar>> LoadAvatarAsync(Guid id, int version = 0)
        {
            ...
            try
            {
                var a = await QueryAvatarAsync("SELECT data_json FROM oasis_avatars WHERE id=@id AND is_deleted=FALSE LIMIT 1", c => c.Parameters.AddWithValue("id", id));
                if (a == null) { OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: No avatar for ID '{id}'."); return result; }
                result.Result = a; result.IsError = false; result.Message = "CockroachDBOASIS: Avatar loaded.";
            }
            catch (Exception ex) { result.Exception = ex; OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: {ex.Message}"); }
            return result;
        }
        ...
        public override async Task<OASISResult<IAvatar>> LoadAvatarByUsernameAsync(string username, int version = 0)
        {
            ...
            try
            {
                var a = await QueryAvatarAsync("SELECT data_json FROM oasis_avatars WHERE username=@u AND is_deleted=FALSE LIMIT 1", c => c.Parameters.AddWithValue("u", username));
                if (a == null) { OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: No avatar for username '{username}'."); return result; }
                result.Result = a; result.IsError = false; result.Message = "CockroachDBOASIS: Avatar loaded.";
            }
            catch (Exception ex) { result.Exception = ex; OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: {ex.Message}"); }
            return result;
        ...
        public override async Task<OASISResult<IAvatar>> LoadAvatarByEmailAsync(string avatarEmail, int version = 0)
        {
            ...
            try
            {
                var a = await QueryAvatarAsync("SELECT data_json FROM oasis_avatars WHERE email=@e AND is_deleted=FALSE LIMIT 1", c => c.Parameters.AddWithValue("e", avatarEmail));
                if (a == null) { OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: No avatar for email '{avatarEmail}'."); return result; }
                result.Result = a; result.IsError = false; result.Message = "CockroachDBOASIS: Avatar loaded.";
            }
            catch (Exception ex) { result.Exception = ex; OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: {ex.Message}"); }
            return result;
        ...
        public override async Task<OASISResult<IAvatar>> LoadAvatarByProviderKeyAsync(string providerKey, int version = 0)
        {
            ...
            var r = new OASISResult<IAvatar>(); OASISErrorHandling.HandleError(ref r, $"CockroachDBOASIS: Invalid GUID '{providerKey}'."); return r;
        }
        ...
        public override async Task<OASISResult<IEnumerable<IAvatar>>> LoadAllAvatarsAsync(int version = 0)
        {
            ...
            try
            {
                ...
                result.Result = list; result.IsError = false; result.Message = $"CockroachDBOASIS: Loaded {list.Count} avatar(s).";
            }
            catch (Exception ex) { result.Exception = ex; OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: {ex.Message}"); }
            return result;
        }
        ...
        public override async Task<OASISResult<bool>> DeleteAvatarAsync(Guid id, bool softDelete = true)
        {
            ...
            {
                await using var conn = await OpenAsync();
                string sql = softDelete ? "UPDATE oasis_avatars SET is_deleted=TRUE WHERE id=@id" : "DELETE FROM oasis_avatars WHERE id=@id";
                await using var cmd = new NpgsqlCommand(sql, conn);
                cmd.Parameters.AddWithValue("id", id);
                await cmd.ExecuteNonQueryAsync();
                result.Result = true; result.IsError = false; result.Message = $"CockroachDBOASIS: Avatar '{id}' deleted.";
            }
            catch (Exception ex) { result.Exception = ex; OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: {ex.Message}"); }
            return result;
        }
        ...
        public override async Task<OASISResult<bool>> DeleteAvatarByUsernameAsync(string u, bool softDelete = true) { var a = await LoadAvatarByUsernameAsync(u); if (a.IsError || a.Result == null) { var r = new OASISResult<bool>(); OASISErrorHandling.HandleError(ref r, $"CockroachDBOASIS: Avatar '{u}' not found."); return r; } return await DeleteAvatarAsync(a.Result.Id, softDelete); }
        ...
        public override async Task<OASISResult<bool>> DeleteAvatarByEmailAsync(string e, bool softDelete = true) { var a = await LoadAvatarByEmailAsync(e); if (a.IsError || a.Result == null) { var r = new OASISResult<bool>(); OASISErrorHandling.HandleError(ref r, $"CockroachDBOASIS: Avatar not found."); return r; } return await DeleteAvatarAsync(a.Result.Id, softDelete); }
        ...
        public override async Task<OASISResult<IAvatarDetail>> SaveAvatarDetailAsync(IAvatarDetail d)
        {
            ...
            try
            {
                ...
                await cmd.ExecuteNonQueryAsync();
                result.Result = d; result.IsError = false; result.Message = "CockroachDBOASIS: AvatarDetail saved.";
            }
            catch (Exception ex) { result.Exception = ex; OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: {ex.Message}"); }
            return result;
        }
        ...
        {
            var result = new OASISResult<IAvatarDetail>();
            try { var d = await QueryDetailAsync("SELECT data_json FROM oasis_avatar_details WHERE id=@id LIMIT 1", c => c.Parameters.AddWithValue("id", id)); if (d == null) { OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: No detail for ID '{id}'."); return result; } result.Result = d; result.IsError = false; result.Message = "CockroachDBOASIS: AvatarDetail loaded."; }
            catch (Exception ex) { result.Exception = ex; OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: {ex.Message}"); }
            return result;
        }
        ...
        {
            var result = new OASISResult<IAvatarDetail>();
            try { var d = await QueryDetailAsync("SELECT data_json FROM oasis_avatar_details WHERE username=@u LIMIT 1", c => c.Parameters.AddWithValue("u", u)); if (d == null) { OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: No detail for username '{u}'."); return result; } result.Result = d; result.IsError = false; result.Message = "CockroachDBOASIS: AvatarDetail loaded."; }
            catch (Exception ex) { result.Exception = ex; OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: {ex.Message}"); }
            return result;
        }
        ...
        {
            var result = new OASISResult<IAvatarDetail>();
            try { var d = await QueryDetailAsync("SELECT data_json FROM oasis_avatar_details WHERE email=@e LIMIT 1", c => c.Parameters.AddWithValue("e", e)); if (d == null) { OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: No detail for email '{e}'."); return result; } result.Result = d; result.IsError = false; result.Message = "CockroachDBOASIS: AvatarDetail loaded."; }
            catch (Exception ex) { result.Exception = ex; OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: {ex.Message}"); }
            return result;
        }
        ...
        public override async Task<OASISResult<IEnumerable<IAvatarDetail>>> LoadAllAvatarDetailsAsync(int version = 0)
        {
            ...
            try
            {
                ...
                result.Result = list; result.IsError = false; result.Message = $"CockroachDBOASIS: Loaded {list.Count} detail(s).";
            }
            catch (Exception ex) { result.Exception = ex; OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: {ex.Message}"); }
            return result;
        }
        ...
        public override async Task<OASISResult<IHolon>> SaveHolonAsync(IHolon holon, bool saveChildren = true, bool recursive = true, int maxChildDepth = 0, bool continueOnError = true, bool saveChildrenOnProvider = false)
        {
            ...
            try
            {
                ...
                holon.ProviderUniqueStorageKey[Core.Enums.ProviderType.CockroachDBOASIS] = holon.Id.ToString();
                await using var conn = await OpenAsync();
                ...
                cmd.Parameters.AddWithValue("parent", holon.ParentHolonId == Guid.Empty ? (object)DBNull.Value : holon.ParentHolonId);
                cmd.Parameters.AddWithValue("type", (int)holon.HolonType);
                cmd.Parameters.AddWithValue("del", holon.IsDeleted);
                cmd.Parameters.AddWithValue("data", Ser(holon));
                await cmd.ExecuteNonQueryAsync();
                result.Result = holon; result.IsError = false; result.Message = $"CockroachDBOASIS: Holon '{holon.Name}' saved.";
            }
            catch (Exception ex) { result.Exception = ex; OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: {ex.Message}"); }
            return result;
        }
        ...
        public override async Task<OASISResult<IEnumerable<IHolon>>> SaveHolonsAsync(IEnumerable<IHolon> holons, bool saveChildren = true, bool recursive = true, int maxChildDepth = 0, int curentChildDepth = 0, bool continueOnError = true, bool saveChildrenOnProvider = false)
        {
            ...
            result.Result = saved; result.IsError = errors.Count > 0; result.Message = errors.Count > 0 ? string.Join("; ", errors) : $"CockroachDBOASIS: {saved.Count} holon(s) saved.";
            return result;
        }
        ...
        {
            var result = new OASISResult<IHolon>();
            try { var list = await QueryHolonsAsync("SELECT data_json FROM oasis_holons WHERE id=@id AND is_deleted=FALSE LIMIT 1", c => c.Parameters.AddWithValue("id", id)); if (list.Count == 0) { OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: No holon for ID '{id}'."); return result; } result.Result = list[0]; result.IsError = false; result.Message = "CockroachDBOASIS: Holon loaded."; }
            catch (Exception ex) { result.Exception = ex; OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: {ex.Message}"); }
            return result;
        }
        ...
        { if (Guid.TryParse(pk, out Guid id)) return await LoadHolonAsync(id, loadChildren, recursive, maxChildDepth, continueOnError, loadChildrenFromProvider, version); var r = new OASISResult<IHolon>(); OASISErrorHandling.HandleError(ref r, $"CockroachDBOASIS: Invalid GUID '{pk}'."); return r; }
        ...
        public override async Task<OASISResult<IEnumerable<IHolon>>> LoadAllHolonsAsync(HolonType type = HolonType.All, bool loadChildren = true, bool recursive = true, int maxChildDepth = 0, int curentChildDepth = 0, bool continueOnError = true, bool loadChildrenFromProvider = false, int version = 0)
        {
            ...
            try
            {
                ...
                result.Result = list; result.IsError = false; result.Message = $"CockroachDBOASIS: Loaded {list.Count} holon(s).";
            }
            catch (Exception ex) { result.Exception = ex; OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: {ex.Message}"); }
            return result;
        }
        ...
        public override async Task<OASISResult<IEnumerable<IHolon>>> LoadHolonsForParentAsync(Guid id, HolonType type = HolonType.All, bool loadChildren = true, bool recursive = true, int maxChildDepth = 0, int curentChildDepth = 0, bool continueOnError = true, bool loadChildrenFromProvider = false, int version = 0)
        {
            ...
            try
            {
                ...
                result.Result = list; result.IsError = false; result.Message = $"CockroachDBOASIS: Loaded {list.Count} holon(s) for parent '{id}'.";
            }
            catch (Exception ex) { result.Exception = ex; OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: {ex.Message}"); }
            return result;
        }
        ...
        { if (Guid.TryParse(pk, out Guid id)) return await LoadHolonsForParentAsync(id, type, loadChildren, recursive, maxChildDepth, curentChildDepth, continueOnError, loadChildrenFromProvider, version); var r = new OASISResult<IEnumerable<IHolon>>(); OASISErrorHandling.HandleError(ref r, $"CockroachDBOASIS: Invalid GUID '{pk}'."); return r; }
        ...
        public override async Task<OASISResult<IHolon>> DeleteHolonAsync(Guid id)
        {
            ...
            try
            {
                var loaded = await LoadHolonAsync(id); if (loaded.IsError || loaded.Result == null) { OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: Holon '{id}' not found."); return result; }
                ...
                result.Result = loaded.Result; result.IsError = false; result.Message = $"CockroachDBOASIS: Holon '{id}' soft-deleted.";
            }
            catch (Exception ex) { result.Exception = ex; OASISErrorHandling.HandleError(ref result, $"CockroachDBOASIS: {ex.Message}"); }
            return result;
        }
        ...
        public override async Task<OASISResult<IHolon>> DeleteHolonAsync(string pk) { if (Guid.TryParse(pk, out Guid id)) return await DeleteHolonAsync(id); var r = new OASISResult<IHolon>(); OASISErrorHandling.HandleError(ref r, $"CockroachDBOASIS: Invalid GUID '{pk}'."); return r; }
        ...
        { var all = await LoadAllHolonsAsync(type); var holons = all.Result?.Where(h => h.MetaData != null && h.MetaData.TryGetValue(metaKey, out var v) && v?.ToString() == metaValue).ToList() ?? new List<IHolon>(); return new OASISResult<IEnumerable<IHolon>> { Result = holons, IsError = false, Message = $"CockroachDBOASIS: {holons.Count} holon(s)." }; }
