...
namespace NextGenSoftware.OASIS.API.Providers.RedisOASIS.TestHarness
{
    public class RedisOASISTestHarness
    {
        private static RedisOASIS _provider = null!;

        public static async Task Main(string[] args)
        {
            var connStr = Environment.GetEnvironmentVariable("REDIS_CONN") ?? "localhost:6379";
            Console.WriteLine($"RedisOASIS Test Harness — {connStr}");
            _provider = new RedisOASIS(connStr);
        ...
        private static async Task TestAvatarOperations()
        {
            ...
            var avatar = new Avatar { Id = Guid.NewGuid(), Username = "redis-harness-user", Email = "redis-harness@test.com" };
        ...
        private static async Task TestHolonOperations()
        {
            ...
            var holon = new Holon { Id = Guid.NewGuid(), Name = "Redis Harness Holon" };
