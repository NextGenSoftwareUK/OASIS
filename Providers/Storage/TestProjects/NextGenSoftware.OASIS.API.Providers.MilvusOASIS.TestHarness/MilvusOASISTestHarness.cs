...
namespace NextGenSoftware.OASIS.API.Providers.MilvusOASIS.TestHarness
{
    public class MilvusOASISTestHarness
    {
        private static MilvusOASIS _provider = null!;

        public static async Task Main(string[] args)
        {
            var connStr = Environment.GetEnvironmentVariable("MILVUS_URL") ?? "http://localhost:19530";
            Console.WriteLine($"MilvusOASIS Test Harness - {connStr}");
        ...
        private static async Task TestAvatarOperations()
        {
            ...
            var avatar = new Avatar { Id = Guid.NewGuid(), Username = "milvus-harness-user", Email = "milvus-harness@test.com" };
        ...
        private static async Task TestHolonOperations()
        {
            ...
            var holon = new Holon { Id = Guid.NewGuid(), Name = "Milvus Harness Holon" };
