using System;
using NextGenSoftware.OASIS.API.Providers.SurrealDBOASIS;

namespace NextGenSoftware.OASIS.API.Providers.SurrealDBOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named SURREALDBOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class SurrealDBOASISTestFactory
    {
        public static SurrealDBOASIS Create()
        {
            var type = typeof(SurrealDBOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"SURREALDBOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (SurrealDBOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(SurrealDBOASIS).Name} - no usable constructor.");
        }
