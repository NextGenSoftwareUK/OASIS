...
namespace NextGenSoftware.OASIS.API.Providers.RavenDBOASIS.TestHarness
{
    /// <summary>
    ...
    /// variable named RAVENDBOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class RavenDBOASISTestFactory
    {
        public static RavenDBOASIS Create()
        {
            var type = typeof(RavenDBOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"RAVENDBOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (RavenDBOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(RavenDBOASIS).Name} - no usable constructor.");
        }
