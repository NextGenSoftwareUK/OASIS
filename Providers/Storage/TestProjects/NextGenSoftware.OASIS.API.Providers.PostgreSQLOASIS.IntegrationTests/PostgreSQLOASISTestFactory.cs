...
namespace NextGenSoftware.OASIS.API.Providers.PostgreSQLOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named POSTGRESQLOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class PostgreSQLOASISTestFactory
    {
        public static PostgreSQLOASIS Create()
        {
            var type = typeof(PostgreSQLOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"POSTGRESQLOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (PostgreSQLOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(PostgreSQLOASIS).Name} - no usable constructor.");
        }
