...
namespace NextGenSoftware.OASIS.API.Providers.PostgreSQLOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from POSTGRESQLOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class PostgreSQLOASISIntegrationTests
    {
        private PostgreSQLOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = PostgreSQLOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live PostgreSQLOASIS backend configured - set the POSTGRESQLOASIS_* environment variables to run this.");
        }
