...
namespace NextGenSoftware.OASIS.API.Providers.CassandraOASIS.IntegrationTests
{
    /// <summary>
    /// Exercises the real CRUD surface against a live backend.
    ///
    /// Connection details come from CASSANDRAOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class CassandraOASISIntegrationTests
    {
        private CassandraOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = CassandraOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live CassandraOASIS backend configured - set the CASSANDRAOASIS_* environment variables to run this.");
        }
