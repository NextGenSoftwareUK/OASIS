...
namespace NextGenSoftware.OASIS.API.Providers.OpenSearchOASIS.TestHarness
{
    public class OpenSearchOASISTestHarness
    {
        private static OpenSearchOASIS _provider = null!;

        public static async Task Main(string[] args)
        {
            var connStr = Environment.GetEnvironmentVariable("OPENSEARCH_URL") ?? "http://localhost:9200";
            Console.WriteLine($"OpenSearchOASIS Test Harness — {connStr}");
            _provider = new OpenSearchOASIS(connStr);
        ...
        private static async Task TestAvatarOperations()
        {
            ...
            var avatar = new Avatar { Id = Guid.NewGuid(), Username = "opensearch-harness-user", Email = "opensearch-harness@test.com" };
        ...
        private static async Task TestHolonOperations()
        {
            ...
            var holon = new Holon { Id = Guid.NewGuid(), Name = "OpenSearch Harness Holon" };
