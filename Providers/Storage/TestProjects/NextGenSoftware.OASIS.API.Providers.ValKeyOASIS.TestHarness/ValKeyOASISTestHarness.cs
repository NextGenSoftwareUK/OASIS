...
namespace NextGenSoftware.OASIS.API.Providers.ValKeyOASIS.TestHarness
{
    public class ValKeyOASISTestHarness
    {
        private static ValKeyOASIS _provider = null!;

        public static async Task Main(string[] args)
        {
            var connStr = Environment.GetEnvironmentVariable("VALKEY_CONN") ?? "localhost:6379";
            Console.WriteLine($"ValKeyOASIS Test Harness - {connStr}");
            _provider = new ValKeyOASIS(connStr);
        ...
        {
            Console.WriteLine("\n--- Avatar Operations ---");
            var avatar = new Avatar { Id = Guid.NewGuid(), Username = "valkey-harness", Email = "valkey-harness@test.com" };
        ...
        {
            Console.WriteLine("\n--- Holon Operations ---");
            var holon = new Holon { Id = Guid.NewGuid(), Name = "ValKey Harness Holon" };
