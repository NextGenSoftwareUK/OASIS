using System;
using NextGenSoftware.OASIS.API.Providers.PlanetScaleOASIS;

namespace NextGenSoftware.OASIS.API.Providers.PlanetScaleOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named PLANETSCALEOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class PlanetScaleOASISTestFactory
    {
        public static PlanetScaleOASIS Create()
        {
            var type = typeof(PlanetScaleOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"PLANETSCALEOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (PlanetScaleOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(PlanetScaleOASIS).Name} - no usable constructor.");
        }
