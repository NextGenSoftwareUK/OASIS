...
namespace NextGenSoftware.OASIS.API.Providers.PlanetScaleOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from PLANETSCALEOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class PlanetScaleOASISIntegrationTests
    {
        private PlanetScaleOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = PlanetScaleOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live PlanetScaleOASIS backend configured - set the PLANETSCALEOASIS_* environment variables to run this.");
        }
