...
namespace NextGenSoftware.OASIS.API.Providers.CockroachDBOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from COCKROACHDBOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class CockroachDBOASISIntegrationTests
    {
        private CockroachDBOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = CockroachDBOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live CockroachDBOASIS backend configured - set the COCKROACHDBOASIS_* environment variables to run this.");
        }
