using System;
using NextGenSoftware.OASIS.API.Providers.CockroachDBOASIS;

namespace NextGenSoftware.OASIS.API.Providers.CockroachDBOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named COCKROACHDBOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class CockroachDBOASISTestFactory
    {
        public static CockroachDBOASIS Create()
        {
            var type = typeof(CockroachDBOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"COCKROACHDBOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (CockroachDBOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(CockroachDBOASIS).Name} - no usable constructor.");
        }
