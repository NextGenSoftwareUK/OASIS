...
namespace NextGenSoftware.OASIS.API.Providers.RedshiftOASIS.TestHarness
{
    public class RedshiftOASISTestHarness
    {
        private static RedshiftOASIS _provider = null!;

        public static async Task Main(string[] args)
        {
            var connStr = Environment.GetEnvironmentVariable("REDSHIFT_CONN") ?? "Host=localhost;Port=5439;Database=oasis;Username=oasis;Password=pw";
            Console.WriteLine($"RedshiftOASIS Test Harness - {connStr}");
        ...
        {
            Console.WriteLine("\n--- Avatar Operations ---");
            var avatar = new Avatar { Id = Guid.NewGuid(), Username = "redshift-harness", Email = "redshift-harness@test.com" };
        ...
        {
            Console.WriteLine("\n--- Holon Operations ---");
            var holon = new Holon { Id = Guid.NewGuid(), Name = "Redshift Harness Holon" };
