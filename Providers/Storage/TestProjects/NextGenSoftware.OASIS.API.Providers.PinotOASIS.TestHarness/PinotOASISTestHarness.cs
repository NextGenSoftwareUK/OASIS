...
namespace NextGenSoftware.OASIS.API.Providers.PinotOASIS.TestHarness
{
    public class PinotOASISTestHarness
    {
        private static PinotOASIS _provider = null!;

        public static async Task Main(string[] args)
        {
            var connStr = Environment.GetEnvironmentVariable("PINOT_BROKER") ?? "http://localhost:8000";
            Console.WriteLine($"PinotOASIS Test Harness - {connStr}");
            _provider = new PinotOASIS(connStr, Environment.GetEnvironmentVariable("PINOT_CONTROLLER") ?? "http://localhost:9000");
        ...
        {
            Console.WriteLine("\n--- Avatar Operations ---");
            var avatar = new Avatar { Id = Guid.NewGuid(), Username = "pinot-harness", Email = "pinot-harness@test.com" };
        ...
        {
            Console.WriteLine("\n--- Holon Operations ---");
            var holon = new Holon { Id = Guid.NewGuid(), Name = "Pinot Harness Holon" };
