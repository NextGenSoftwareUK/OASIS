...
namespace NextGenSoftware.OASIS.API.Providers.QuestDBOASIS.TestHarness
{
    public class QuestDBOASISTestHarness
    {
        private static QuestDBOASIS _provider = null!;

        public static async Task Main(string[] args)
        {
            var connStr = Environment.GetEnvironmentVariable("QUESTDB_CONN") ?? "Host=localhost;Port=8812;Username=admin;Password=quest;Database=qdb";
            Console.WriteLine($"QuestDBOASIS Test Harness — {connStr}");
            _provider = new QuestDBOASIS(connStr);
        ...
        private static async Task TestAvatarOperations()
        {
            ...
            var avatar = new Avatar { Id = Guid.NewGuid(), Username = "questdb-harness-user", Email = "questdb-harness@test.com" };
        ...
        private static async Task TestHolonOperations()
        {
            ...
            var holon = new Holon { Id = Guid.NewGuid(), Name = "QuestDB Harness Holon" };
