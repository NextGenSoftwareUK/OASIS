...
namespace NextGenSoftware.OASIS.API.Providers.SnowflakeOASIS.TestHarness
{
    public class SnowflakeOASISTestHarness
    {
        private static SnowflakeOASIS _provider = null!;

        public static async Task Main(string[] args)
        {
            var connStr = Environment.GetEnvironmentVariable("SNOWFLAKE_CONN") ?? "account=xy12345;user=oasis;password=pw;db=OASIS;schema=PUBLIC";
            Console.WriteLine($"SnowflakeOASIS Test Harness - {connStr}");
        ...
        {
            Console.WriteLine("\n--- Avatar Operations ---");
            var avatar = new Avatar { Id = Guid.NewGuid(), Username = "snowflake-harness", Email = "snowflake-harness@test.com" };
        ...
        {
            Console.WriteLine("\n--- Holon Operations ---");
            var holon = new Holon { Id = Guid.NewGuid(), Name = "Snowflake Harness Holon" };
