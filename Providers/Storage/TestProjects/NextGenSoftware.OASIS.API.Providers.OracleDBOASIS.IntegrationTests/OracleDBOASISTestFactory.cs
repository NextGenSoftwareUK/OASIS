using System;
using NextGenSoftware.OASIS.API.Providers.OracleDBOASIS;

namespace NextGenSoftware.OASIS.API.Providers.OracleDBOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named ORACLEDBOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class OracleDBOASISTestFactory
    {
        public static OracleDBOASIS Create()
        {
            var type = typeof(OracleDBOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"ORACLEDBOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (OracleDBOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(OracleDBOASIS).Name} - no usable constructor.");
        }
