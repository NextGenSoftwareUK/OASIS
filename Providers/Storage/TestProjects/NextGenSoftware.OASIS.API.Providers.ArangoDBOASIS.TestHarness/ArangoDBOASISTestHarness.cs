...
namespace NextGenSoftware.OASIS.API.Providers.ArangoDBOASIS.TestHarness
{
    public class ArangoDBOASISTestHarness
    {
        private static ArangoDBOASIS _provider = null!;

        public static async Task Main(string[] args)
        {
            var connStr = Environment.GetEnvironmentVariable("ARANGODB_URL") ?? "http://localhost:8529";
            Console.WriteLine($"ArangoDBOASIS Test Harness — {connStr}");
            _provider = new ArangoDBOASIS(connStr);
        ...
        private static async Task TestAvatarOperations()
        {
            ...
            var avatar = new Avatar { Id = Guid.NewGuid(), Username = "arangodb-harness-user", Email = "arangodb-harness@test.com" };
        ...
        private static async Task TestHolonOperations()
        {
            ...
            var holon = new Holon { Id = Guid.NewGuid(), Name = "ArangoDB Harness Holon" };
