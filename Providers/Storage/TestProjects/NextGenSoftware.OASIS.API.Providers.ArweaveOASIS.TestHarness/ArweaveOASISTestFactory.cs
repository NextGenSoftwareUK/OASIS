...
namespace NextGenSoftware.OASIS.API.Providers.ArweaveOASIS.TestHarness
{
    /// <summary>
    ...
    /// variable named ARWEAVEOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class ArweaveOASISTestFactory
    {
        public static ArweaveOASIS Create()
        {
            var type = typeof(ArweaveOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"ARWEAVEOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (ArweaveOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(ArweaveOASIS).Name} - no usable constructor.");
        }
