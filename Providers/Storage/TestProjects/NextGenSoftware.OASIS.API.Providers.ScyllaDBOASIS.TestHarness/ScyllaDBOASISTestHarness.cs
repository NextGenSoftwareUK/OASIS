...
namespace NextGenSoftware.OASIS.API.Providers.ScyllaDBOASIS.TestHarness
{
    public class ScyllaDBOASISTestHarness
    {
        private static ScyllaDBOASIS _provider = null!;

        public static async Task Main(string[] args)
        {
            var contactPoints = Environment.GetEnvironmentVariable("SCYLLADB_CONTACT_POINTS") ?? (args.Length > 0 ? args[0] : "127.0.0.1");
            Console.WriteLine($"ScyllaDBOASIS Test Harness — connecting to {contactPoints}");
        ...
        private static async Task TestAvatarOperations()
        {
            ...
            var avatar = new Avatar { Id = Guid.NewGuid(), Username = "scylla-harness-user", Email = "scylla-harness@test.com" };
        ...
        private static async Task TestHolonOperations()
        {
            ...
            var holon = new Holon { Id = Guid.NewGuid(), Name = "ScyllaDB Harness Holon" };
