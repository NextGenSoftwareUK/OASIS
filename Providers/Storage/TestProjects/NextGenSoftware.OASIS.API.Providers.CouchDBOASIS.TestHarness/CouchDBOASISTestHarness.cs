...
namespace NextGenSoftware.OASIS.API.Providers.CouchDBOASIS.TestHarness
{
    public class CouchDBOASISTestHarness
    {
        private static CouchDBOASIS _provider = null!;

        public static async Task Main(string[] args)
        {
            var connStr = Environment.GetEnvironmentVariable("COUCHDB_URL") ?? "http://localhost:5984";
            Console.WriteLine($"CouchDBOASIS Test Harness — {connStr}");
            _provider = new CouchDBOASIS(connStr, Environment.GetEnvironmentVariable("COUCHDB_USER") ?? "admin", Environment.GetEnvironmentVariable("COUCHDB_PASS") ?? "password");
        ...
        private static async Task TestAvatarOperations()
        {
            ...
            var avatar = new Avatar { Id = Guid.NewGuid(), Username = "couchdb-harness-user", Email = "couchdb-harness@test.com" };
        ...
        private static async Task TestHolonOperations()
        {
            ...
            var holon = new Holon { Id = Guid.NewGuid(), Name = "CouchDB Harness Holon" };
