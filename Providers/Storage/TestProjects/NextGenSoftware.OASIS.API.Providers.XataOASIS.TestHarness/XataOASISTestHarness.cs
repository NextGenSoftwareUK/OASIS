...
namespace NextGenSoftware.OASIS.API.Providers.XataOASIS.TestHarness
{
    public class XataOASISTestHarness
    {
        private static XataOASIS _provider = null!;

        public static async Task Main(string[] args)
        {
            var connStr = Environment.GetEnvironmentVariable("XATA_URL") ?? "https://workspace.region.xata.sh/db/oasis:main";
            Console.WriteLine($"XataOASIS Test Harness — {connStr}");
            _provider = new XataOASIS(connStr, Environment.GetEnvironmentVariable("XATA_API_KEY") ?? "APIKEY");
        ...
        private static async Task TestAvatarOperations()
        {
            ...
            var avatar = new Avatar { Id = Guid.NewGuid(), Username = "xata-harness-user", Email = "xata-harness@test.com" };
        ...
        private static async Task TestHolonOperations()
        {
            ...
            var holon = new Holon { Id = Guid.NewGuid(), Name = "Xata Harness Holon" };
