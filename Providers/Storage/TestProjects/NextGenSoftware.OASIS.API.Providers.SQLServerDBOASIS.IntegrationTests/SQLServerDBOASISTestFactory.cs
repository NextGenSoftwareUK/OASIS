using System;
using NextGenSoftware.OASIS.API.Providers.SQLServerDBOASIS;

namespace NextGenSoftware.OASIS.API.Providers.SQLServerDBOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named SQLSERVERDBOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class SQLServerDBOASISTestFactory
    {
        public static SQLServerDBOASIS Create()
        {
            var type = typeof(SQLServerDBOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"SQLSERVERDBOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (SQLServerDBOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(SQLServerDBOASIS).Name} - no usable constructor.");
        }
