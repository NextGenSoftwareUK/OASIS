...
namespace NextGenSoftware.OASIS.API.Providers.SQLServerDBOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from SQLSERVERDBOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class SQLServerDBOASISIntegrationTests
    {
        private SQLServerDBOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = SQLServerDBOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live SQLServerDBOASIS backend configured - set the SQLSERVERDBOASIS_* environment variables to run this.");
        }
