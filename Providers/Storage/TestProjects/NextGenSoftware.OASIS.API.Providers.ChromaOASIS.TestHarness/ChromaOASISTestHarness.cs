...
namespace NextGenSoftware.OASIS.API.Providers.ChromaOASIS.TestHarness
{
    public class ChromaOASISTestHarness
    {
        private static ChromaOASIS _provider = null!;

        public static async Task Main(string[] args)
        {
            var connStr = Environment.GetEnvironmentVariable("CHROMA_URL") ?? "http://localhost:8000";
            Console.WriteLine($"ChromaOASIS Test Harness - {connStr}");
        ...
        private static async Task TestAvatarOperations()
        {
            ...
            var avatar = new Avatar { Id = Guid.NewGuid(), Username = "chroma-harness-user", Email = "chroma-harness@test.com" };
        ...
        private static async Task TestHolonOperations()
        {
            ...
            var holon = new Holon { Id = Guid.NewGuid(), Name = "Chroma Harness Holon" };
