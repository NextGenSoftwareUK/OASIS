...
namespace NextGenSoftware.OASIS.API.Providers.MemcachedOASIS.TestHarness
{
    public class MemcachedOASISTestHarness
    {
        private static MemcachedOASIS _provider = null!;

        public static async Task Main(string[] args)
        {
            var connStr = Environment.GetEnvironmentVariable("MEMCACHED_HOST") ?? "localhost";
            Console.WriteLine($"MemcachedOASIS Test Harness — {connStr}");
            _provider = new MemcachedOASIS(connStr, int.Parse(Environment.GetEnvironmentVariable("MEMCACHED_PORT") ?? "11211"));
        ...
        private static async Task TestAvatarOperations()
        {
            ...
            var avatar = new Avatar { Id = Guid.NewGuid(), Username = "memcached-harness-user", Email = "memcached-harness@test.com" };
        ...
        private static async Task TestHolonOperations()
        {
            ...
            var holon = new Holon { Id = Guid.NewGuid(), Name = "Memcached Harness Holon" };
