...
namespace NextGenSoftware.OASIS.API.Providers.GarnetOASIS.TestHarness
{
    public class GarnetOASISTestHarness
    {
        private static GarnetOASIS _provider = null!;

        public static async Task Main(string[] args)
        {
            var connStr = Environment.GetEnvironmentVariable("GARNET_CONN") ?? "localhost:3278";
            Console.WriteLine($"GarnetOASIS Test Harness - {connStr}");
        ...
        {
            Console.WriteLine("\n--- Avatar Operations ---");
            var avatar = new Avatar { Id = Guid.NewGuid(), Username = "garnet-harness", Email = "garnet-harness@test.com" };
        ...
        {
            Console.WriteLine("\n--- Holon Operations ---");
            var holon = new Holon { Id = Guid.NewGuid(), Name = "Garnet Harness Holon" };
