...
namespace NextGenSoftware.OASIS.API.Providers.PgVectorOASIS.TestHarness
{
    public class PgVectorOASISTestHarness
    {
        private static PgVectorOASIS _provider = null!;

        public static async Task Main(string[] args)
        {
            var connStr = Environment.GetEnvironmentVariable("PGVECTOR_CONN") ?? "Host=localhost;Port=5432;Database=oasis;Username=postgres;Password=postgres";
            Console.WriteLine($"PgVectorOASIS Test Harness - {connStr}");
            _provider = new PgVectorOASIS(connStr);
        ...
        private static async Task TestAvatarOperations()
        {
            ...
            var avatar = new Avatar { Id = Guid.NewGuid(), Username = "pgvector-harness-user", Email = "pgvector-harness@test.com" };
        ...
        private static async Task TestHolonOperations()
        {
            ...
            var holon = new Holon { Id = Guid.NewGuid(), Name = "PgVector Harness Holon" };
