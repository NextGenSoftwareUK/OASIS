...
namespace NextGenSoftware.OASIS.API.Providers.LanceDBOASIS.TestHarness
{
    public class LanceDBOASISTestHarness
    {
        private static LanceDBOASIS _provider = null!;

        public static async Task Main(string[] args)
        {
            var connStr = Environment.GetEnvironmentVariable("LANCEDB_URL") ?? "https://db.lancedb.com";
            Console.WriteLine($"LanceDBOASIS Test Harness - {connStr}");
            _provider = new LanceDBOASIS(connStr, Environment.GetEnvironmentVariable("LANCEDB_API_KEY") ?? "APIKEY", Environment.GetEnvironmentVariable("LANCEDB_PROJECT") ?? "oasis");
        ...
        private static async Task TestAvatarOperations()
        {
            ...
            var avatar = new Avatar { Id = Guid.NewGuid(), Username = "lancedb-harness-user", Email = "lancedb-harness@test.com" };
        ...
        private static async Task TestHolonOperations()
        {
            ...
            var holon = new Holon { Id = Guid.NewGuid(), Name = "LanceDB Harness Holon" };
