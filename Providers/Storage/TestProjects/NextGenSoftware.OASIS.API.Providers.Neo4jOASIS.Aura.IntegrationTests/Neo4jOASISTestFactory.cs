...
namespace NextGenSoftware.OASIS.API.Providers.Neo4jOASIS.Aura.IntegrationTests
{
    /// <summary>
    ...
    /// variable named NEO4JOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class Neo4jOASISTestFactory
    {
        public static Neo4jOASIS Create()
        {
            var type = typeof(Neo4jOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"NEO4JOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (Neo4jOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(Neo4jOASIS).Name} - no usable constructor.");
        }
