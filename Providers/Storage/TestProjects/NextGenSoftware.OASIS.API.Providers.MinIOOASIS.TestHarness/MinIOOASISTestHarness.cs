...
namespace NextGenSoftware.OASIS.API.Providers.MinIOOASIS.TestHarness
{
    public class MinIOOASISTestHarness
    {
        private static MinIOOASIS _provider = null!;

        public static async Task Main(string[] args)
        {
            var endpoint = Environment.GetEnvironmentVariable("MINIO_ENDPOINT") ?? (args.Length > 0 ? args[0] : "http://localhost:9000");
            var accessKey = Environment.GetEnvironmentVariable("MINIO_ACCESS_KEY") ?? (args.Length > 1 ? args[1] : "minioadmin");
            var secretKey = Environment.GetEnvironmentVariable("MINIO_SECRET_KEY") ?? (args.Length > 2 ? args[2] : "minioadmin");
            Console.WriteLine($"MinIOOASIS Test Harness — endpoint: {endpoint}");
        ...
        private static async Task TestAvatarOperations()
        {
            ...
            var avatar = new Avatar { Id = Guid.NewGuid(), Username = "minio-harness-user", Email = "minio-harness@test.com" };
        ...
        private static async Task TestHolonOperations()
        {
            ...
            var holon = new Holon { Id = Guid.NewGuid(), Name = "MinIO Harness Holon" };
