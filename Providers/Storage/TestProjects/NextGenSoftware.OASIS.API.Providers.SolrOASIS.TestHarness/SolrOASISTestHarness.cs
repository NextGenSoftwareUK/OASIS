...
namespace NextGenSoftware.OASIS.API.Providers.SolrOASIS.TestHarness
{
    public class SolrOASISTestHarness
    {
        private static SolrOASIS _provider = null!;

        public static async Task Main(string[] args)
        {
            var connStr = Environment.GetEnvironmentVariable("SOLR_URL") ?? "http://localhost:8983";
            Console.WriteLine($"SolrOASIS Test Harness — {connStr}");
            _provider = new SolrOASIS(connStr);
        ...
        private static async Task TestAvatarOperations()
        {
            ...
            var avatar = new Avatar { Id = Guid.NewGuid(), Username = "solr-harness-user", Email = "solr-harness@test.com" };
        ...
        private static async Task TestHolonOperations()
        {
            ...
            var holon = new Holon { Id = Guid.NewGuid(), Name = "Solr Harness Holon" };
