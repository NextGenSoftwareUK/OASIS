...
namespace NextGenSoftware.OASIS.API.Providers.RedisOASIS.IntegrationTests
{
    [TestClass]
    public class RedisOASISIntegrationTests
    {
        private RedisOASIS _provider = null!;
        private static readonly string connStr = Environment.GetEnvironmentVariable("REDIS_CONN") ?? "localhost:6379";
        ...
        {
            var avatar = new Avatar { Id = Guid.NewGuid(), Username = "redis-test-user", Email = "redis@test.com" };
        ...
        {
            var holon = new Holon { Id = Guid.NewGuid(), Name = "Redis Test Holon" };
