    ...
    public class KeyDBOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISDBStorageProvider
    {
        ...
        public override async Task<OASISResult<IAvatar>> LoadAvatarByUsernameAsync(string username, int version = 0)
        {
            ...
            try
            {
                ...
                if (!idStr.HasValue || !Guid.TryParse((string?)idStr, out Guid id)) { OASISErrorHandling.HandleError(ref result, $"KeyDBOASIS: No avatar for username '{username}'."); return result; }
        ...
        public override async Task<OASISResult<IAvatar>> LoadAvatarByEmailAsync(string avatarEmail, int version = 0)
        {
            ...
            try
            {
                ...
                if (!idStr.HasValue || !Guid.TryParse((string?)idStr, out Guid id)) { OASISErrorHandling.HandleError(ref result, $"KeyDBOASIS: No avatar for email '{avatarEmail}'."); return result; }
                return await LoadAvatarAsync(id, version);
            }
            catch (Exception ex) { result.Exception = ex; OASISErrorHandling.HandleError(ref result, $"KeyDBOASIS: {ex.Message}"); }
            return result;
        ...
        public override async Task<OASISResult<IAvatar>> LoadAvatarByProviderKeyAsync(string providerKey, int version = 0) { if (Guid.TryParse((string?)providerKey, out Guid id)) return await LoadAvatarAsync(id, version); var r = new OASISResult<IAvatar>(); OASISErrorHandling.HandleError(ref r, $"KeyDBOASIS: Invalid GUID '{providerKey}'."); return r; }
        ...
        public override async Task<OASISResult<IEnumerable<IAvatar>>> LoadAllAvatarsAsync(int version = 0)
        {
            ...
            try
            {
                ...
                foreach (var idVal in ids) { if (!Guid.TryParse((string?)idVal, out Guid id)) continue; var r = await LoadAvatarAsync(id, version); if (!r.IsError && r.Result != null) list.Add(r.Result); }
        ...
        public override async Task<OASISResult<bool>> DeleteAvatarAsync(string pk, bool softDelete = true) { if (Guid.TryParse((string?)pk, out Guid id)) return await DeleteAvatarAsync(id, softDelete); return await DeleteAvatarByUsernameAsync(pk, softDelete); }
        ...
        public override async Task<OASISResult<IEnumerable<IHolon>>> LoadAllHolonsAsync(HolonType holonType = HolonType.All, bool loadChildren = true, bool recursive = true, int maxChildDepth = 0, int curentChildDepth = 0, bool continueOnError = true, bool loadChildrenFromProvider = false, int version = 0)
        {
            ...
            try
            {
                ...
                foreach (var idVal in ids) { if (!Guid.TryParse((string?)idVal, out Guid id)) continue; var r = await LoadHolonAsync(id, loadChildren, recursive, maxChildDepth, continueOnError, loadChildrenFromProvider, version); if (r.IsError || r.Result == null || r.Result.IsDeleted) continue; if (holonType != HolonType.All && r.Result.HolonType != holonType) continue; list.Add(r.Result); }
        ...
        { var result = new OASISResult<IEnumerable<IHolon>>(); try { var all = await LoadAllHolonsAsync(holonType, loadChildren, recursive, maxChildDepth, curentChildDepth, continueOnError, loadChildrenFromProvider, version); var list = new List<IHolon>(); if (!all.IsError && all.Result != null) foreach (var h in all.Result) { if (h.ParentHolonId == id) list.Add(h); } result.Result = list; result.IsError = false; result.Message = $"KeyDBOASIS: Loaded {list.Count} child holon(s)."; } catch (Exception ex) { result.Exception = ex; OASISErrorHandling.HandleError(ref result, $"KeyDBOASIS: {ex.Message}"); } return result; }
        ...
        public async Task<OASISResult<bool>> DeleteHolonSoftAsync(string pk, bool softDelete = true) { if (Guid.TryParse((string?)pk, out Guid id)) return await DeleteHolonSoftAsync(id, softDelete); var r = new OASISResult<bool>(); OASISErrorHandling.HandleError(ref r, $"KeyDBOASIS: Invalid GUID '{pk}'."); return r; }
        ...
        {
            var result = new OASISResult<IHolon>();
            if (Guid.TryParse((string?)providerKey, out var id))
        ...
        public override async Task<OASISResult<ISearchResults>> SearchAsync(ISearchParams searchParams, bool loadChildren = true, bool recursive = true, int maxChildDepth = 0, bool continueOnError = true, int version = 0)
        {
            ...
            {
                OASISErrorHandling.HandleError(ref result, $"KeyDBOASIS: SearchAsync failed: {ex.Message}");
            }
