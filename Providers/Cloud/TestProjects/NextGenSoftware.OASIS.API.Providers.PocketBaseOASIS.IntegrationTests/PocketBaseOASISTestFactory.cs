using System;
using NextGenSoftware.OASIS.API.Providers.PocketBaseOASIS;

namespace NextGenSoftware.OASIS.API.Providers.PocketBaseOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named POCKETBASEOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class PocketBaseOASISTestFactory
    {
        public static PocketBaseOASIS Create()
        {
            var type = typeof(PocketBaseOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"POCKETBASEOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (PocketBaseOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(PocketBaseOASIS).Name} - no usable constructor.");
        }
