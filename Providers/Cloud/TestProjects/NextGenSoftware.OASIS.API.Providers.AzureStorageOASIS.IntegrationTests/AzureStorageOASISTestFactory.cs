using System;
using NextGenSoftware.OASIS.API.Providers.AzureStorageOASIS;

namespace NextGenSoftware.OASIS.API.Providers.AzureStorageOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named AZURESTORAGEOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class AzureStorageOASISTestFactory
    {
        public static AzureStorageOASIS Create()
        {
            var type = typeof(AzureStorageOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"AZURESTORAGEOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (AzureStorageOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(AzureStorageOASIS).Name} - no usable constructor.");
        }
