using System;
using NextGenSoftware.OASIS.API.Providers.FirebaseOASIS;

namespace NextGenSoftware.OASIS.API.Providers.FirebaseOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named FIREBASEOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class FirebaseOASISTestFactory
    {
        public static FirebaseOASIS Create()
        {
            var type = typeof(FirebaseOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"FIREBASEOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (FirebaseOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(FirebaseOASIS).Name} - no usable constructor.");
        }
