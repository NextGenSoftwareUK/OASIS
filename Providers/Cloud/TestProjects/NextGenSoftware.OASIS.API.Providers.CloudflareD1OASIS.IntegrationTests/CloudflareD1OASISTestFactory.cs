...
namespace NextGenSoftware.OASIS.API.Providers.CloudflareD1OASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named CLOUDFLARED1OASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class CloudflareD1OASISTestFactory
    {
        public static CloudflareD1OASIS Create()
        {
            var type = typeof(CloudflareD1OASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"CLOUDFLARED1OASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (CloudflareD1OASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(CloudflareD1OASIS).Name} - no usable constructor.");
        }
