using System;
using NextGenSoftware.OASIS.API.Providers.FaunaOASIS;

namespace NextGenSoftware.OASIS.API.Providers.FaunaOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named FAUNAOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class FaunaOASISTestFactory
    {
        public static FaunaOASIS Create()
        {
            var type = typeof(FaunaOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"FAUNAOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (FaunaOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(FaunaOASIS).Name} - no usable constructor.");
        }
