...
namespace NextGenSoftware.OASIS.API.Providers.FaunaOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from FAUNAOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class FaunaOASISIntegrationTests
    {
        private FaunaOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = FaunaOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live FaunaOASIS backend configured - set the FAUNAOASIS_* environment variables to run this.");
        }
