...
namespace NextGenSoftware.OASIS.API.Providers.CloudflareOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from CLOUDFLAREOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class CloudflareOASISIntegrationTests
    {
        private CloudflareOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = CloudflareOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live CloudflareOASIS backend configured - set the CLOUDFLAREOASIS_* environment variables to run this.");
        }
