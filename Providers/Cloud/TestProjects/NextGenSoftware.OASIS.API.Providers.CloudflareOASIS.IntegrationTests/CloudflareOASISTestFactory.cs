using System;
using NextGenSoftware.OASIS.API.Providers.CloudflareOASIS;

namespace NextGenSoftware.OASIS.API.Providers.CloudflareOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named CLOUDFLAREOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class CloudflareOASISTestFactory
    {
        public static CloudflareOASIS Create()
        {
            var type = typeof(CloudflareOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"CLOUDFLAREOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (CloudflareOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(CloudflareOASIS).Name} - no usable constructor.");
        }
