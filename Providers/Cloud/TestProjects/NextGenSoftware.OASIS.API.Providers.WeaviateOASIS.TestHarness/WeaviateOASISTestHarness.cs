...
namespace NextGenSoftware.OASIS.API.Providers.WeaviateOASIS.TestHarness
{
    public class WeaviateOASISTestHarness
    {
        private static WeaviateOASIS _provider;

        public static async Task Main(string[] args)
        {
            Console.WriteLine("=== WeaviateOASIS Test Harness ===");
            string host = args.Length > 0 ? args[0] : (Environment.GetEnvironmentVariable("WEAVIATE_HOST") ?? "http://localhost:8080");
        ...
        private static async Task TestAvatarOperations()
        {
            ...
            {
                Id = Guid.NewGuid(),
                Username = "WeaviateOASISTestUser",
                Email = "weaviateoasistest@example.com",
                FirstName = "Weaviate",
                LastName = "Tester",
        ...
        private static async Task TestHolonOperations()
        {
            ...
            {
                Id = Guid.NewGuid(),
                Name = "WeaviateOASISTestHolon",
                Description = "Test Holon for WeaviateOASIS",
                CreatedDate = DateTime.UtcNow
            };
