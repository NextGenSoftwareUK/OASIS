...
namespace NextGenSoftware.OASIS.API.Providers.GoogleCloudOASIS.TestHarness
{
    /// <summary>
    ...
    /// variable named GOOGLECLOUDOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class GoogleCloudOASISTestFactory
    {
        public static GoogleCloudOASIS Create()
        {
            var type = typeof(GoogleCloudOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"GOOGLECLOUDOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (GoogleCloudOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(GoogleCloudOASIS).Name} - no usable constructor.");
        }
