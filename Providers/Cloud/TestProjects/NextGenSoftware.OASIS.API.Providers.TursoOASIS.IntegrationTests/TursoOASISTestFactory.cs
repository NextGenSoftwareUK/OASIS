using System;
using NextGenSoftware.OASIS.API.Providers.TursoOASIS;

namespace NextGenSoftware.OASIS.API.Providers.TursoOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named TURSOOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class TursoOASISTestFactory
    {
        public static TursoOASIS Create()
        {
            var type = typeof(TursoOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"TURSOOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (TursoOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(TursoOASIS).Name} - no usable constructor.");
        }
