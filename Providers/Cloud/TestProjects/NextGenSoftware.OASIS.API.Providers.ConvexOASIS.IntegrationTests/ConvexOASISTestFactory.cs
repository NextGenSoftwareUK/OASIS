using System;
using NextGenSoftware.OASIS.API.Providers.ConvexOASIS;

namespace NextGenSoftware.OASIS.API.Providers.ConvexOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named CONVEXOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class ConvexOASISTestFactory
    {
        public static ConvexOASIS Create()
        {
            var type = typeof(ConvexOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"CONVEXOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (ConvexOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(ConvexOASIS).Name} - no usable constructor.");
        }
