using System;
using NextGenSoftware.OASIS.API.Providers.AppwriteOASIS;

namespace NextGenSoftware.OASIS.API.Providers.AppwriteOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named APPWRITEOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class AppwriteOASISTestFactory
    {
        public static AppwriteOASIS Create()
        {
            var type = typeof(AppwriteOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"APPWRITEOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (AppwriteOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(AppwriteOASIS).Name} - no usable constructor.");
        }
