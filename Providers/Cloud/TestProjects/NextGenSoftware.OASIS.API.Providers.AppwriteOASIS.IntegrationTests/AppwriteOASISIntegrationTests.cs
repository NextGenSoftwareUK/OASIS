...
namespace NextGenSoftware.OASIS.API.Providers.AppwriteOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from APPWRITEOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class AppwriteOASISIntegrationTests
    {
        private AppwriteOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = AppwriteOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live AppwriteOASIS backend configured - set the APPWRITEOASIS_* environment variables to run this.");
        }
