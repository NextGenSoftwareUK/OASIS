using System;
using NextGenSoftware.OASIS.API.Providers.NeonOASIS;

namespace NextGenSoftware.OASIS.API.Providers.NeonOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named NEONOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class NeonOASISTestFactory
    {
        public static NeonOASIS Create()
        {
            var type = typeof(NeonOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"NEONOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (NeonOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(NeonOASIS).Name} - no usable constructor.");
        }
