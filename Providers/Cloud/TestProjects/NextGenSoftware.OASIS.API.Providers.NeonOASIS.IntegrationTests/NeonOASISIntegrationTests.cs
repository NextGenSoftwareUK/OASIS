...
namespace NextGenSoftware.OASIS.API.Providers.NeonOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from NEONOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class NeonOASISIntegrationTests
    {
        private NeonOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = NeonOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live NeonOASIS backend configured - set the NEONOASIS_* environment variables to run this.");
        }
