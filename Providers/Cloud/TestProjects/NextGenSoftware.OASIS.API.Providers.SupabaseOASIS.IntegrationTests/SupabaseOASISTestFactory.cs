using System;
using NextGenSoftware.OASIS.API.Providers.SupabaseOASIS;

namespace NextGenSoftware.OASIS.API.Providers.SupabaseOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named SUPABASEOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class SupabaseOASISTestFactory
    {
        public static SupabaseOASIS Create()
        {
            var type = typeof(SupabaseOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"SUPABASEOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (SupabaseOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(SupabaseOASIS).Name} - no usable constructor.");
        }
