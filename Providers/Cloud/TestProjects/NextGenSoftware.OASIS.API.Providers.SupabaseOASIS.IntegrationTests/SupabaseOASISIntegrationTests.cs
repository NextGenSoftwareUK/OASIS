...
namespace NextGenSoftware.OASIS.API.Providers.SupabaseOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from SUPABASEOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class SupabaseOASISIntegrationTests
    {
        private SupabaseOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = SupabaseOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live SupabaseOASIS backend configured - set the SUPABASEOASIS_* environment variables to run this.");
        }
