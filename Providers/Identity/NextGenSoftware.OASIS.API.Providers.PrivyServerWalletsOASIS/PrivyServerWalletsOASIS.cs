...
namespace NextGenSoftware.OASIS.API.Providers.PrivyServerWalletsOASIS
{
    public class PrivyServerWalletsOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISNETProvider, IOASISBlockchainStorageProvider
    {
        ...
        public PrivyServerWalletsOASIS(string appId = "", string appSecret = "")
        {
            ...
            ProviderName = "PrivyServerWalletsOASIS"; ProviderDescription = "Privy server-side embedded wallet and identity provider.";
        ...
        public override async Task<OASISResult<bool>> ActivateProviderAsync()
        {
            ...
            try { await GetAsync("apps/wallets?limit=1"); _isActivated = true; r.Result = true; r.Message = "PrivyServerWalletsOASIS activated"; }
        ...
        public override async Task<OASISResult<IAvatar>> LoadAvatarByProviderKeyAsync(string userId, int v = 0)
        {
            ...
            try
            {
                ...
                avatar.ProviderUniqueStorageKey[Core.Enums.ProviderType.PrivyServerWalletsOASIS] = json["id"]?.ToString() ?? userId;
                avatar.Email = json["email"]?["address"]?.ToString() ?? "";
                avatar.MetaData["did"] = json["id"]?.ToString() ?? "";
        ...
        public override async Task<OASISResult<IHolon>> LoadHolonAsync(string key, bool lc = true, bool rec = true, int md = 0, bool coe = true, bool lcfp = false, int v = 0)
        {
            ...
            try
            {
                ...
                var holon = new Holon { Name = json["address"]?.ToString() ?? key };
                holon.ProviderUniqueStorageKey[Core.Enums.ProviderType.PrivyServerWalletsOASIS] = key;
                holon.MetaData["wallet_id"] = key;
        ...
        public override async Task<OASISResult<IHolon>> SaveHolonAsync(IHolon h, bool sc = true, bool rec = true, int md = 0, bool coe = true, bool scop = false)
        {
            ...
            try
            {
                ...
                var body = new { chain_type = h.MetaData.ContainsKey("chain_type") ? h.MetaData["chain_type"] : "ethereum" };
                var json = await PostAsync("apps/wallets", body);
