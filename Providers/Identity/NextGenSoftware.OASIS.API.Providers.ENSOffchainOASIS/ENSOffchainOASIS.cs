...
namespace NextGenSoftware.OASIS.API.Providers.ENSOffchainOASIS
{
    public class ENSOffchainOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISNETProvider, IOASISBlockchainStorageProvider
    {
        ...
        public ENSOffchainOASIS()
        {
            ...
            ProviderName = "ENSOffchainOASIS"; ProviderDescription = "ENS offchain name resolution and Web3 identity profile provider.";
        ...
        public override async Task<OASISResult<bool>> ActivateProviderAsync()
        {
            ...
            try { await GetAsync("ns/ens/vitalik.eth"); _isActivated = true; r.Result = true; r.Message = "ENSOffchainOASIS activated"; }
        ...
        public override async Task<OASISResult<IAvatar>> LoadAvatarByProviderKeyAsync(string ensName, int v = 0)
        {
            ...
            try
            {
                ...
                var avatar = new Avatar { Username = ensName };
                avatar.ProviderUniqueStorageKey[Core.Enums.ProviderType.ENSOffchainOASIS] = ensName;
                avatar.FirstName = json["displayName"]?.ToString() ?? ensName;
        ...
        public override async Task<OASISResult<IHolon>> LoadHolonAsync(string key, bool lc = true, bool rec = true, int md = 0, bool coe = true, bool lcfp = false, int v = 0)
        {
            ...
            try
            {
                // key = address: resolve reverse lookup
                var json = await GetAsync($"profile/{Uri.EscapeDataString(key)}");
                ...
                holon.MetaData["ens"] = json["identity"]?.ToString() ?? "";
        ...
        public override async Task<OASISResult<IHolon>> SaveHolonAsync(IHolon h, bool sc = true, bool rec = true, int md = 0, bool coe = true, bool scop = false) { if (h.Id == Guid.Empty) h.Id = Guid.NewGuid(); return new OASISResult<IHolon> { Result = h, Message = "ENSOffchain is a read-only resolver; data not persisted externally." }; }
