...
namespace NextGenSoftware.OASIS.API.Providers.BNBChainOASIS.Infrastructure.Services.BNBChain;

public sealed class BNBChainBridgeService : IBNBChainBridgeService
{
    private const decimal WeiPerBNB = 1_000_000_000_000_000_000m;
        ...
        string hostUri = null)
    {
        ...
        if (useTestnet)
        {
            _hostUri = hostUri ?? "https://data-seed-prebsc-1-s1.binance.org:8545";
            _chainId = 97; // BSC Testnet
        }
        else
        {
            _hostUri = hostUri ?? "https://bsc-dataseed.binance.org";
            _chainId = 56; // BSC Mainnet
        }
    }

    private decimal ConvertWeiToBNB(System.Numerics.BigInteger weiAmount) => (decimal)weiAmount / WeiPerBNB;
    ...
    public async Task<OASISResult<decimal>> GetAccountBalanceAsync(string accountAddress, CancellationToken token = default)
    {
        ...
        try
        {
            ...
            result.Result = ConvertWeiToBNB(balance.Value);
            result.IsError = false;
            result.Message = $"Balance: {result.Result} BNB";
            return result;
        }
        catch (Exception ex)
        {
    ...
    public async Task<OASISResult<(string PublicKey, string PrivateKey, string SeedPhrase)>> CreateAccountAsync(CancellationToken token = default)
    {
        ...
        try
        {
            ...
            result.Result = (ecKey.GetPublicAddress(), ecKey.GetPrivateKey(), $"bnb seed {Guid.NewGuid().ToString("N")} keys");
            result.IsError = false;
