...
namespace NextGenSoftware.OASIS.API.Providers.EthereumOASIS.Infrastructure.Services.Ethereum;

/// <summary>
/// Ethereum bridge service for cross-chain operations
/// Implements all IOASISBridge methods for Ethereum blockchain
/// </summary>
public sealed class EthereumBridgeService : IEthereumBridgeService
{
        ...
        string hostUri = null)
    {
        ...
        if (useTestnet)
        {
            _hostUri = hostUri ?? "https://sepolia.infura.io/v3/";
            _chainId = 11155111; // Sepolia testnet
        }
        else
        {
            _hostUri = hostUri ?? "https://mainnet.infura.io/v3/";
        ...
        CreateAccountAsync(CancellationToken token = default)
    {
        ...
        try
        {
            ...
            var seedPhrase = $"ethereum seed {Guid.NewGuid().ToString("N")} wallet keys secure private";
            ...
            result.IsError = false;
            result.Message = "Ethereum account created successfully";
