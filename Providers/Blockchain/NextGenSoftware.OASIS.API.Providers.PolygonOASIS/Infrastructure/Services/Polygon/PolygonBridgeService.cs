...
namespace NextGenSoftware.OASIS.API.Providers.PolygonOASIS.Infrastructure.Services.Polygon;

/// <summary>
/// Polygon bridge service for cross-chain operations
/// Implements all IOASISBridge methods for Polygon blockchain
/// </summary>
public sealed class PolygonBridgeService : IPolygonBridgeService
{
    private const decimal WeiPerMatic = 1_000_000_000_000_000_000m; // 10^18
        ...
        string hostUri = null)
    {
        ...
        if (useTestnet)
        {
            _hostUri = hostUri ?? "https://rpc-mumbai.maticvigil.com";
            _chainId = 80001; // Mumbai testnet
        }
        else
        {
            _hostUri = hostUri ?? "https://polygon-rpc.com";
            _chainId = 137; // Polygon mainnet
        }
    }

    private decimal ConvertWeiToMatic(System.Numerics.BigInteger weiAmount) 
        => (decimal)weiAmount / WeiPerMatic;
        ...
        CancellationToken token = default)
    {
        ...
        try
        {
            ...
            result.Result = ConvertWeiToMatic(balance.Value);
            result.IsError = false;
            result.Message = $"Balance retrieved successfully: {result.Result} MATIC";
        ...
        CreateAccountAsync(CancellationToken token = default)
    {
        ...
        try
        {
            ...
            var seedPhrase = $"polygon seed {Guid.NewGuid().ToString("N")} wallet keys secure private";
            ...
            result.IsError = false;
            result.Message = "Polygon account created successfully";
        ...
        string senderPrivateKey)
    {
        ...
        try
        {
            ...
            result.Message = isSuccessful 
                ? $"Withdrawal successful: {amount} MATIC transferred" 
        ...
        string receiverAccountAddress)
    {
        ...
        try
        {
            ...
            result.IsError = !isSuccessful;
            result.Message = isSuccessful 
                ? $"Deposit successful: {amount} MATIC transferred" 
