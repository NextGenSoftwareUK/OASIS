...
namespace NextGenSoftware.OASIS.API.Providers.SynapseOASIS
{
    /// <summary>
    ...
    /// </summary>
    public class SynapseOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISNETProvider, IOASISBlockchainStorageProvider
    {
        ...
        public SynapseOASIS(string apiUrl = "")
        {
            _apiUrl = apiUrl?.TrimEnd('/') ?? "";
            _http = new HttpClient { BaseAddress = new Uri(_apiUrl + "/") };

            ProviderName = "SynapseOASIS";
        ...
        public override async Task<OASISResult<bool>> ActivateProviderAsync()
        {
            ...
            try
            {
                if (_isActivated) { result.Result = true; result.Message = "SynapseOASIS already activated"; return result; }
                _isActivated = true;
                result.Result = true;
                result.Message = "SynapseOASIS activated successfully";
            }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref result, $"SynapseOASIS activation failed: {ex.Message}", ex); }
            return result;
        }
        ...
        {
            var result = new OASISResult<bool>();
            try { _isActivated = false; _http.Dispose(); result.Result = true; result.Message = "SynapseOASIS deactivated"; }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref result, $"SynapseOASIS deactivation failed: {ex.Message}", ex); }
            return result;
        }

        public override async Task<OASISResult<IAvatar>> LoadAvatarAsync(Guid id, int version = 0) { var r = new OASISResult<IAvatar>(); try { r.Result = new Avatar { Id = id }; } catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"SynapseOASIS LoadAvatarAsync error: {ex.Message}", ex); } return r; }
        public override async Task<OASISResult<IAvatar>> LoadAvatarByUsernameAsync(string avatarUsername, int version = 0) { var r = new OASISResult<IAvatar>(); try { r.Result = new Avatar { Username = avatarUsername }; } catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"SynapseOASIS LoadAvatarByUsernameAsync error: {ex.Message}", ex); } return r; }
        public override async Task<OASISResult<IAvatar>> SaveAvatarAsync(IAvatar avatar) { var r = new OASISResult<IAvatar>(); try { if (avatar.Id == Guid.Empty) avatar.Id = Guid.NewGuid(); r.Result = avatar; } catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"SynapseOASIS SaveAvatarAsync error: {ex.Message}", ex); } return r; }
        ...
        public override async Task<OASISResult<IAvatarDetail>> LoadAvatarDetailAsync(Guid id, int version = 0) { var r = new OASISResult<IAvatarDetail>(); OASISErrorHandling.HandleError(ref r, "SynapseOASIS does not support avatar detail storage"); return r; }
        public override async Task<OASISResult<IAvatarDetail>> SaveAvatarDetailAsync(IAvatarDetail avatarDetail) { var r = new OASISResult<IAvatarDetail>(); OASISErrorHandling.HandleError(ref r, "SynapseOASIS does not support avatar detail storage"); return r; }
