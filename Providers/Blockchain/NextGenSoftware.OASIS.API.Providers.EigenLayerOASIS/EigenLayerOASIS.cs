...
namespace NextGenSoftware.OASIS.API.Providers.EigenLayerOASIS
{
    public class EigenLayerOASIS : OASISStorageProviderBase, IOASISStorageProvider, IOASISNETProvider, IOASISBlockchainStorageProvider
    {
        ...
        public override async Task<OASISResult<bool>> ActivateProviderAsync()
        {
            ...
            try { await GetAsync("operators?limit=1"); _isActivated = true; r.Result = true; r.Message = "EigenLayerOASIS activated"; }
        ...
        public override async Task<OASISResult<IAvatar>> LoadAvatarByProviderKeyAsync(string address, int v = 0)
        {
            ...
            try
            {
                ...
                avatar.MetaData["shares"] = json["shares"]?.ToString(Newtonsoft.Json.Formatting.None) ?? "[]";
                r.Result = avatar;
            }
            catch (Exception ex) { OASISErrorHandling.HandleError(ref r, $"EigenLayer LoadAvatar failed: {ex.Message}", ex); }
            return r;
        }
        ...
        public override async Task<OASISResult<IHolon>> SaveHolonAsync(IHolon h, bool sc = true, bool rec = true, int md = 0, bool coe = true, bool scop = false) { if (h.Id == Guid.Empty) h.Id = Guid.NewGuid(); return new OASISResult<IHolon> { Result = h, Message = "EigenLayer is a read-only data API; data not persisted externally." }; }
