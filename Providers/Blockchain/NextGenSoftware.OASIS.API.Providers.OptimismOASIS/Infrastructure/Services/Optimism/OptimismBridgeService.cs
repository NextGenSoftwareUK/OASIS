...
namespace NextGenSoftware.OASIS.API.Providers.OptimismOASIS.Infrastructure.Services.Optimism;

/// <summary>
/// Optimism bridge service for cross-chain operations
/// Implements all IOASISBridge methods for Optimism blockchain (Ethereum L2)
/// </summary>
public sealed class OptimismBridgeService : IOptimismBridgeService
{
    private const decimal WeiPerEth = 1_000_000_000_000_000_000m;
        ...
        string hostUri = null)
    {
        ...
        if (useTestnet)
        {
            _hostUri = hostUri ?? "https://sepolia.optimism.io";
            _chainId = 11155420; // Optimism Sepolia testnet
        }
        else
        {
            _hostUri = hostUri ?? "https://mainnet.optimism.io";
            _chainId = 10; // Optimism mainnet
        }
    }
        ...
        CreateAccountAsync(CancellationToken token = default)
    {
        ...
        try
        {
            ...
            var seedPhrase = $"optimism seed {Guid.NewGuid().ToString("N")} wallet keys secure private";
            
            result.Result = (address, privateKey, seedPhrase);
            result.IsError = false;
            result.Message = "Optimism account created successfully";
        ...
        string senderPrivateKey)
    {
        ...
        try
        {
            ...
            result.Message = isSuccessful ? $"Withdrawal successful: {amount} ETH transferred" : "Withdrawal transaction failed";
        ...
        string receiverAccountAddress)
    {
        ...
        try
        {
            ...
            result.IsError = !isSuccessful;
            result.Message = isSuccessful ? $"Deposit successful: {amount} ETH transferred" : "Deposit transaction failed";
