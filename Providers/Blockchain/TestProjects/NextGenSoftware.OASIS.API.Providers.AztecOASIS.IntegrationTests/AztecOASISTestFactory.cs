...
namespace NextGenSoftware.OASIS.API.Providers.AztecOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named AZTECOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class AztecOASISTestFactory
    {
        public static AztecOASIS Create()
        {
            var type = typeof(AztecOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"AZTECOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (AztecOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(AztecOASIS).Name} - no usable constructor.");
        }
