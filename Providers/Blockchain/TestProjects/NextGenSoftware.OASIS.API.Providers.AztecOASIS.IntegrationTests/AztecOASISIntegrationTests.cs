...
namespace NextGenSoftware.OASIS.API.Providers.AztecOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from AZTECOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class AztecOASISIntegrationTests
    {
        private AztecOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = AztecOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live AztecOASIS backend configured - set the AZTECOASIS_* environment variables to run this.");
        }
