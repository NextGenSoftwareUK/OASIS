...
namespace NextGenSoftware.OASIS.API.Providers.XRPLOASIS.TestHarness
{
    public class XRPLOASISTestHarness
    {
        private static XRPLOASIS _provider;

        public static async Task Main(string[] args)
        {
            Console.WriteLine("=== XRPLOASIS Test Harness ===");
            Console.WriteLine("Testing XRPLOASIS integration with OASIS...\n");
        ...
        private static async Task TestAvatarOperations()
        {
            ...
            {
                Id = Guid.NewGuid(),
                Username = "XRPLOASISTestUser",
                Email = "xrploasistest@example.com",
                FirstName = "XRPLOASIS",
                LastName = "Tester",
        ...
        private static async Task TestHolonOperations()
        {
            ...
            {
                Id = Guid.NewGuid(),
                Name = "XRPLOASISTestHolon",
                Description = "Test Holon for XRPLOASIS integration",
                CreatedDate = DateTime.UtcNow
            };
        ...
        private static async Task TestSearchOperations()
        {
            ...
            Console.WriteLine($"Searching with query: '{searchParams.SearchQuery}'");
            var searchResult = await _provider.SearchAsync(searchParams);
            Console.WriteLine($"Search Result: {(searchResult.IsError ? "Failed" : "Success")}");
            if (searchResult.IsError)
                Console.WriteLine($"Error: {searchResult.Message}");
            else
                Console.WriteLine($"Found {searchResult.Result?.NumberOfResults ?? 0} results");
