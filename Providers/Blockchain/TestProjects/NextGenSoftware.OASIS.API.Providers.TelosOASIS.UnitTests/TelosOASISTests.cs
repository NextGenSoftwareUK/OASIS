...
namespace NextGenSoftware.OASIS.API.Providers.TelosOASIS.UnitTests
{
    [TestClass]
    public class TelosOASISTests
    {
        ...
        public async Task SaveAvatar_ShouldReturnNotSupported()
        {
            ...
            Assert.IsTrue(result.Message.Contains("not supported yet by Telos provider"));
        }
        ...
        public void GetPlayersNearMe_ShouldReturnNotSupported()
        {
            ...
            // Assert
            Assert.IsFalse(result.IsSuccess);
            Assert.IsTrue(result.Message.Contains("not supported by Telos provider"));
        }
