...
namespace NextGenSoftware.OASIS.API.Providers.AbstractOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named ABSTRACTOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class AbstractOASISTestFactory
    {
        public static AbstractOASIS Create()
        {
            var type = typeof(AbstractOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"ABSTRACTOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (AbstractOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(AbstractOASIS).Name} - no usable constructor.");
        }
