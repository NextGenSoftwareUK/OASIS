using System;
using NextGenSoftware.OASIS.API.Providers.AlgorandOASIS;

namespace NextGenSoftware.OASIS.API.Providers.AlgorandOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named ALGORANDOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class AlgorandOASISTestFactory
    {
        public static AlgorandOASIS Create()
        {
            var type = typeof(AlgorandOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"ALGORANDOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (AlgorandOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(AlgorandOASIS).Name} - no usable constructor.");
        }
