...
namespace NextGenSoftware.OASIS.API.Providers.AlgorandOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from ALGORANDOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class AlgorandOASISIntegrationTests
    {
        private AlgorandOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = AlgorandOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live AlgorandOASIS backend configured - set the ALGORANDOASIS_* environment variables to run this.");
        }
