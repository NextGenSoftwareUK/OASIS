...
namespace NextGenSoftware.OASIS.API.Providers.BerachainOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named BERACHAINOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class BerachainOASISTestFactory
    {
        public static BerachainOASIS Create()
        {
            var type = typeof(BerachainOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"BERACHAINOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (BerachainOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(BerachainOASIS).Name} - no usable constructor.");
        }
