using System;
using NextGenSoftware.OASIS.API.Providers.BasechainOASIS;

namespace NextGenSoftware.OASIS.API.Providers.BasechainOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named BASECHAINOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class BasechainOASISTestFactory
    {
        public static BasechainOASIS Create()
        {
            var type = typeof(BasechainOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"BASECHAINOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (BasechainOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(BasechainOASIS).Name} - no usable constructor.");
        }
