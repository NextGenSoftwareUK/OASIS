...
namespace NextGenSoftware.OASIS.API.Providers.BasechainOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from BASECHAINOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class BasechainOASISIntegrationTests
    {
        private BasechainOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = BasechainOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live BasechainOASIS backend configured - set the BASECHAINOASIS_* environment variables to run this.");
        }
