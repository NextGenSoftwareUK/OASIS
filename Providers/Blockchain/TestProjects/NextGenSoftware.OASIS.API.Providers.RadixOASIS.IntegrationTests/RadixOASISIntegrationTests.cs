...
namespace NextGenSoftware.OASIS.API.Providers.RadixOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from RADIXOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class RadixOASISIntegrationTests
    {
        private RadixOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = RadixOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live RadixOASIS backend configured - set the RADIXOASIS_* environment variables to run this.");
        }
