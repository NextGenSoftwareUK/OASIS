...
namespace NextGenSoftware.OASIS.API.Providers.RadixOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named RADIXOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class RadixOASISTestFactory
    {
        public static RadixOASIS Create()
        {
            var type = typeof(RadixOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"RADIXOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (RadixOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(RadixOASIS).Name} - no usable constructor.");
        }
