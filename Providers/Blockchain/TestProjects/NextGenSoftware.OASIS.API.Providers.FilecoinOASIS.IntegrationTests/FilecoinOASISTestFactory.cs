using System;
using NextGenSoftware.OASIS.API.Providers.FilecoinOASIS;

namespace NextGenSoftware.OASIS.API.Providers.FilecoinOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named FILECOINOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class FilecoinOASISTestFactory
    {
        public static FilecoinOASIS Create()
        {
            var type = typeof(FilecoinOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"FILECOINOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (FilecoinOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(FilecoinOASIS).Name} - no usable constructor.");
        }
