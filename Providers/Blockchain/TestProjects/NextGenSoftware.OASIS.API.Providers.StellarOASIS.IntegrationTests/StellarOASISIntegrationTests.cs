...
namespace NextGenSoftware.OASIS.API.Providers.StellarOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from STELLAROASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class StellarOASISIntegrationTests
    {
        private StellarOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = StellarOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live StellarOASIS backend configured - set the STELLAROASIS_* environment variables to run this.");
        }
