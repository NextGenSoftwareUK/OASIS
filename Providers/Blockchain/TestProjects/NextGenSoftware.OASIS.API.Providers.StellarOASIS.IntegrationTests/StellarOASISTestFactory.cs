using System;
using NextGenSoftware.OASIS.API.Providers.StellarOASIS;

namespace NextGenSoftware.OASIS.API.Providers.StellarOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named STELLAROASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class StellarOASISTestFactory
    {
        public static StellarOASIS Create()
        {
            var type = typeof(StellarOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"STELLAROASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (StellarOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(StellarOASIS).Name} - no usable constructor.");
        }
