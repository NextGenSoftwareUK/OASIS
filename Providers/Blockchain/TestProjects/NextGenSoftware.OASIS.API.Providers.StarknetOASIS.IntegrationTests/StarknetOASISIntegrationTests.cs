...
namespace NextGenSoftware.OASIS.API.Providers.StarknetOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from STARKNETOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class StarknetOASISIntegrationTests
    {
        private StarknetOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = StarknetOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live StarknetOASIS backend configured - set the STARKNETOASIS_* environment variables to run this.");
        }
