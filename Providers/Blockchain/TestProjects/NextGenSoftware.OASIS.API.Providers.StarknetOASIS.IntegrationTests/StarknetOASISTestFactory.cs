...
namespace NextGenSoftware.OASIS.API.Providers.StarknetOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named STARKNETOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class StarknetOASISTestFactory
    {
        public static StarknetOASIS Create()
        {
            var type = typeof(StarknetOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"STARKNETOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (StarknetOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(StarknetOASIS).Name} - no usable constructor.");
        }
