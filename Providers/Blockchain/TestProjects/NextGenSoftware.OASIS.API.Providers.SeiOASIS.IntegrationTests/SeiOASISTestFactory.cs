...
namespace NextGenSoftware.OASIS.API.Providers.SeiOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named SEIOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class SeiOASISTestFactory
    {
        public static SeiOASIS Create()
        {
            var type = typeof(SeiOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"SEIOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (SeiOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(SeiOASIS).Name} - no usable constructor.");
        }
