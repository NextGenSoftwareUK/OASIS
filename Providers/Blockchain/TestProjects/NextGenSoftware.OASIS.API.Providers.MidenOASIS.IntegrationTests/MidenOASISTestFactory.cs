...
namespace NextGenSoftware.OASIS.API.Providers.MidenOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named MIDENOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class MidenOASISTestFactory
    {
        public static MidenOASIS Create()
        {
            var type = typeof(MidenOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"MIDENOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (MidenOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(MidenOASIS).Name} - no usable constructor.");
        }
