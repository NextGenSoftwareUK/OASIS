...
namespace NextGenSoftware.OASIS.API.Providers.MidenOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from MIDENOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class MidenOASISIntegrationTests
    {
        private MidenOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = MidenOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live MidenOASIS backend configured - set the MIDENOASIS_* environment variables to run this.");
        }
