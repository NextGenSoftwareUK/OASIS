using System;
using NextGenSoftware.OASIS.API.Providers.CeramicOASIS;

namespace NextGenSoftware.OASIS.API.Providers.CeramicOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named CERAMICOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class CeramicOASISTestFactory
    {
        public static CeramicOASIS Create()
        {
            var type = typeof(CeramicOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"CERAMICOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (CeramicOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(CeramicOASIS).Name} - no usable constructor.");
        }
