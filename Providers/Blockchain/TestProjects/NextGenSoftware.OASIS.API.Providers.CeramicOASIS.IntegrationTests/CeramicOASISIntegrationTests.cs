...
namespace NextGenSoftware.OASIS.API.Providers.CeramicOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from CERAMICOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class CeramicOASISIntegrationTests
    {
        private CeramicOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = CeramicOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live CeramicOASIS backend configured - set the CERAMICOASIS_* environment variables to run this.");
        }
