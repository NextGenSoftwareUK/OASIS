...
namespace NextGenSoftware.OASIS.API.Providers.MonadOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named MONADOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class MonadOASISTestFactory
    {
        public static MonadOASIS Create()
        {
            var type = typeof(MonadOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"MONADOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (MonadOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(MonadOASIS).Name} - no usable constructor.");
        }
