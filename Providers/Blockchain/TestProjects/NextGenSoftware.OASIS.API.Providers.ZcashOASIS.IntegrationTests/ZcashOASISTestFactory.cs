...
namespace NextGenSoftware.OASIS.API.Providers.ZcashOASIS.IntegrationTests
{
    /// <summary>
    ...
    /// variable named ZCASHOASIS_&lt;PARAM&gt;, falling back to a harmless default.
    /// Keeping it in one place means the tests do not need per-provider wiring.
    /// </summary>
    internal static class ZcashOASISTestFactory
    {
        public static ZcashOASIS Create()
        {
            var type = typeof(ZcashOASIS);
            ...
            foreach (var ctor in SortByFewestParameters(type))
            {
                ...
                for (var i = 0; i < ps.Length; i++)
                {
                    var env = Environment.GetEnvironmentVariable(
                        $"ZCASHOASIS_{ps[i].Name!.ToUpperInvariant()}");
                ...
                if (!ok) continue;
                try { return (ZcashOASIS)ctor.Invoke(args); } catch { /* try the next overload */ }
            }

            throw new InvalidOperationException(
                $"Could not construct {typeof(ZcashOASIS).Name} - no usable constructor.");
        }
