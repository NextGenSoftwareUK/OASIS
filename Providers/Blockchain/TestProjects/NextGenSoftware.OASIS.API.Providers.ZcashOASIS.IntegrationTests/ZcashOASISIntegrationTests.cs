...
namespace NextGenSoftware.OASIS.API.Providers.ZcashOASIS.IntegrationTests
{
    ...
    ///
    /// Connection details come from ZCASHOASIS_&lt;PARAM&gt; environment variables. When the
    ...
    /// </summary>
    [TestClass]
    public class ZcashOASISIntegrationTests
    {
        private ZcashOASIS _provider = null!;
        private bool _live;

        [TestInitialize]
        public void Setup()
        {
            _provider = ZcashOASISTestFactory.Create();
        ...
        private void RequireLive()
        {
            if (!_live)
                Assert.Inconclusive(
                    "No live ZcashOASIS backend configured - set the ZCASHOASIS_* environment variables to run this.");
        }
